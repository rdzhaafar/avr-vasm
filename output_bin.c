...
static char* copyright =
	"vasm binary output module 2.0 (c) 2002-2020 Volker Barthelmann and Frank Wille";
...
{
	if (ULLTADDR((*(section**)sec1)->org) >
	    ULLTADDR((*(section**)sec2)->org))
		return 1;
	if (ULLTADDR((*(section**)sec1)->org) <
	    ULLTADDR((*(section**)sec2)->org))
		return -1;
	...
	for (slp = seclist; nsecs > 0; nsecs--) {
		...
		default:
			...
			if (s != seclist[0] && ULLTADDR(s->org) > pc)
				fwalignpattern(f, ULLTADDR(s->org) - pc, s->pad,
					       s->padbytes);
			break;
		}
...
int init_output_bin(char** cp, void (**wo)(FILE*, section*, symbol*),
		    int (**oa)(char*))
{
	*cp = copyright;
	*wo = write_output;
