   ...
   when label is not starting at first column. */
{
	...
	if (isspace((unsigned char)*s)) {
		s = skip(s);
		needcolon =
			1; /* colon required, when label doesn't start at 1st column */
	}
	...
	if (f = locate_file(filename, "rb", NULL)) {
		...
		if (size > 0) {
			if (nbskip >= 0 && nbskip <= size) {
				...
				if (nbkeep > (unsigned long)(size - nbskip) ||
				    nbkeep == 0)
				...
				if (fread(db->data, 1, db->size, f) != db->size)
					general_error(
						29, filename); /* read error */
				add_atom(0, new_data_atom(db, 1));
			} else
				general_error(
					46); /* bad file-offset argument */
		}
		fclose(f);
	}
	myfree(filename);
...
/* check if a directive from the list matches the current source location */
{
	...
	if (!ISIDSTART(*s) ||
	    (!isspace((unsigned char)*(s - 1)) && *(s - 1) != '\0'))
...
void new_repeat(int rcnt, char* name, char* vals, struct namelen* reptlist,
		struct namelen* endrlist)
{
...
int find_macarg_name(source* src, char* name, size_t len)
{
	...
	if (src->macro != NULL) {
		for (idx = 0, ma = src->argnames;
		     ma != NULL && idx < maxmacparams;
		     idx++, ma = ma->argnext) {
			/* @@@ case-sensitive comparison? */
			if (ma->arglen == len &&
			    strncmp(ma->argname, name, len) == 0)
				return idx;
		}
	}
	...
	if (src->irpname != NULL) {
		if (strlen(src->irpname) == len &&
		    strncmp(src->irpname, name, len) == 0) {
		 ...
		 char* args)
{
	...
	if (cur_macro == NULL && cur_src != NULL && enddir_list == NULL) {
		...
		if (cur_src->defsrc)
			general_error(
				26,
				cur_src->name); /* macro definition inside macro */
		m->defsrc = cur_src;
		...
		if (find_name_nc(mnemohash, name, &data)) {
			...
			for (idx = data.idx;
			     idx < mnemonic_cnt &&
			     !stricmp(mnemonics[idx].name, name);
			     idx++) {
				if (MNEMONIC_VALID(idx)) {
					m->text = NULL;
					general_error(
						51); /* name conflicts with mnemonic */
					break;
				}
			}
		...
		if (args) {
			...
			while (args != NULL && !ISEOL(args)) {
				...
				if (end != NULL && end - args != 0 &&
					...
					m->num_argnames++;
					args = skip(end);
					if (end = MACRO_ARG_OPTS(
						    m, n, ma->argname, args))
						args = skip(
							end); /* got defaults and qualifiers */
					else
						addmacarg(
							&m->defaults, args,
					...
					general_error(
						42); /* illegal macro argument */
					break;
				}
				args = MACRO_ARG_SEP(
					args); /* check for separator between names */
			}
		}
	} else
		ierror(0);
		  ...
		  char* s)
{
	...
	if (m->recursions >= maxmacrecurs) {
		general_error(
			56,
			maxmacrecurs); /* maximum macro recursions reached */
		return 0;
	}
	m->recursions++;
	...
	for (n = 0, ma = m->defaults; n < maxmacparams; n++) {
		if (ma != NULL) {
			src->param[n] = ma->arglen == MACARG_REQUIRED ?
						      NULL :
						      ma->argname;
			src->param_len[n] =
				ma->arglen == MACARG_REQUIRED ? 0 : ma->arglen;
			ma = ma->argnext;
		} else {
	...
	while (!ISEOL(s) && n < maxmacparams) {
		if (n >= 0 && m->vararg == n) {
			...
			if (arg.len) {
				...
				if (n <= 0) {
					n = find_macarg_name(src, arg.name,
							     arg.len);
					if (n >= 0) {
						...
						general_error(
							72); /* undefined macro argument name */
					n = -1;
				} else
					general_error(
						71); /* cannot mix positional and keyword arguments */
			} else {
		...
		s = skip(s);
		if (!(s = MACRO_PARAM_SEP(
			      s))) /* check for separator between params. */
			break;
	}
	...
	for (n = 0; n < maxmacparams; n++) {
		if (src->param[n] == NULL) {
			...
			general_error(
				73,
				n + 1); /* required macro argument was left out */
		}
	}
...
int new_structure(char* name)
{
	...
	if (cur_struct) {
		general_error(
			48); /* cannot declare structure within structure */
		return 0;
	}
...
char* read_next_line(void)
{
	...
	for (;;) {
		...
		if (cur_src->srcptr >= srcend || *(cur_src->srcptr) == '\0') {
			if (--cur_src->repeat > 0) {
				...
				cur_src->srcptr =
					cur_src->text; /* back to start */
				cur_src->line = 0;
				if (cur_src->irpname != NULL &&
				    (irpval = cur_src->irpvals) != NULL) {
				...
				cur_src =
					cur_src->parent; /* return to parent source */
#ifdef CARGSYM
				if (cur_src->cargexp) {
					symbol* carg = internal_abs(CARGSYM);
					carg->expr =
						cur_src->cargexp; /* restore parent CARG */
				}
#endif
#ifdef REPTNSYM
				set_internal_abs(
					REPTNSYM,
					cur_src->reptn); /* restore parent REPTN */
#endif
			}
		} else
			break;
	...
	*d++ = 0;
	...
	if (enddir_list != NULL && (srcend - s) > enddir_minlen) {
		...
		while (s <= (srcend - enddir_minlen)) {
			if (dir = dirlist_match(s, srcend, enddir_list)) {
				...
				s += dir->len;
				   ...
				   (dir = dirlist_match(
					    s, srcend, reptdir_list)) != NULL) {
				s += dir->len;
				rept_nest++;
			}
#ifdef MACRO_IN_MACRO_CHECK /* caution: misdetection in operands possible */
			else if (cur_macro != NULL &&
				 (dir = dirlist_match(s, srcend,
						      macrdir_list)) != NULL) {
				general_error(
					26,
					cur_macro->name); /* macro definition inside macro */
			}
#endif
		...
		if (enddir_list) {
			if (cur_macro)
				general_error(
					25,
					cur_macro->name); /* missing ENDM directive */
			else
	...
	if (nparam < 0 && cur_src->irpname != NULL)
		nparam =
			0; /* expand current repeat-iterator symbol into source */
	...
	while (s < srcend && *s != '\0') {
		...
		if (nparam >= 0)
			nc = expand_macro(cur_src, &s, d,
					  len); /* try macro arg. expansion */
		else
			nc = 0;
		...
		if (nc < 0) {
			...
			cur_src->linebuf =
				myrealloc(cur_src->linebuf, cur_src->bufsize);
