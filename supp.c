...
void* mymalloc(size_t sz)
{
	...
	if (debug) {
		...
		*p++ = sz;
		memset(p, 0xdd,
		       sz); /* make it crash, when using uninitialized memory */
	} else {
		p = malloc(sz ? sz : 1);
		if (!p)
			general_error(17);
...
void myfree(void* p)
{
	if (p) {
		if (debug) {
			...
			size_t sz = *(--myp);
			memset(p, 0xff,
			       sz); /* make it crash, when reusing deallocated memory */
			free(--myp);
		} else
			free(p);
	}
...
void* setval_signext(int be, void* dest, size_t extsz, size_t valsz,
		     int64_t val)
...
uint64_t readbits(int be, void* p, unsigned bfsize, unsigned offset,
		  unsigned size)
...
/* @@@ Warning: precision is lost! Converting to double precision. */
{
	...
	if (conv.x == 0) {
		...
		man = ((conv.x & 0xfffffffffffffLL) << 11) |
