...
static char* expskip(char* s)
{
	s = skip(s);
	if (*s == commentchar)
		*s = '\0'; /* rest of line is commented out */
	return s;
}
#endif
...
expr* copy_tree(expr* old)
{
	...
	if (old) {
		new = make_expr(old->type, copy_tree(old->left),
				copy_tree(old->right));
		new->c = old->c;
	}
	return new;
...
static expr* primary_expr(void)
{
	...
	if (name = get_local_label(&s)) {
		...
		if (!sym)
			sym = new_import(name);
		sym->flags |= USED;
		new = (sym->type != EXPRESSION) ? new_sym_expr(sym) :
							copy_tree(sym->expr);
		myfree(name);
		return new;
	}
	...
	if (name = parse_identifier(&s)) {
		...
		if (!sym) {
			...
			if (!strcmp(name, NARGSYM)) {
				...
				new->type = NUM;
				new->c.val =
					cur_src->num_params; /*@@@ check for macro mode? */
				return new;
			}
#endif
			sym = new_import(name);
...
int type_of_expr(expr* tree)
{
	...
	if (ltype == SYM) {
		...
		ltype = tree->c.sym->type == EXPRESSION ?
				      type_of_expr(tree->c.sym->expr) :
				      NUM;
		tree->c.sym->flags &= ~INEVAL;
		return ltype;
...
expr** find_sym_expr(expr** ptree, char* name)
{
	...
	if (*ptree == NULL)
		return NULL;
	if ((*ptree)->left != NULL && (*ptree)->left->type == SYM &&
	    !strcmp((*ptree)->left->c.sym->name, name))
		return &(*ptree)->left;
	if (psym = find_sym_expr(&(*ptree)->left, name))
		return psym;
	if ((*ptree)->right != NULL && (*ptree)->right->type == SYM &&
	    !strcmp((*ptree)->right->c.sym->name, name))
		return &(*ptree)->right;
...
void simplify_expr(expr* tree)
{
	...
	if (tree->type == SUB && tree->right->type == SYM &&
	    tree->left->type == SUB && tree->left->left->type != SYM &&
	    tree->left->right->type == SYM) {
	...
	if (tree->left) {
		if (tree->left->type == NUM || tree->left->type == HUG ||
		    tree->left->type == FLT)
			type = tree->left->type;
		else
			return;
	...
	if (tree->right) {
		if (tree->right->type == NUM || tree->right->type == HUG ||
		    tree->right->type == FLT) {
	...
	if (type == NUM) {
		...
		case DIV:
			if (tree->right->c.val == 0) {
				general_error(41);
				ival = 0;
			} else if (tree->left->c.val == taddrmin &&
				   tree->right->c.val == -1) {
				general_error(
					21,
					sizeof(taddr) *
		...
		default:
			...
			if (tree->left &&
			    EXT_UNARY_EVAL(tree->type, tree->left->c.val, &ival,
					   1))
				break;
#endif
			return;
		...
		if (tree->left) {
			...
			case NUM:
				lval = (tfloat)tree->left->c.val;
				break;
			case HUG:
				lval = huge_to_float(tree->left->c.huge);
				break;
			case FLT:
				lval = tree->left->c.flt;
				break;
			}
		}
		if (tree->right) {
			...
			case NUM:
				rval = (tfloat)tree->right->c.val;
				break;
			case HUG:
				rval = huge_to_float(tree->right->c.huge);
				break;
			case FLT:
				rval = tree->right->c.flt;
				break;
			}
		}
		switch (tree->type) {
...
void print_expr(FILE* f, expr* p)
{
	...
	simplify_expr(p);
	if (p->type == NUM)
		fprintf(f, "%lld=0x%llx", (long long)p->c.val,
			ULLTADDR(p->c.val));
	else if (p->type == HUG)
		fprintf(f, "0x%016llx%016llx", (long long)p->c.huge.hi,
			(long long)p->c.huge.lo);
	else if (p->type == FLT)
...
static int _find_base(expr* p, symbol** base, section* sec, taddr pc)
{
	...
	if (p->type == SUB) {
		...
		if (_find_base(p->left, base, sec, pc) == BASE_OK &&
			...
			if (LOCREF(pcsym) && pcsym->sec == sec &&
			    (LOCREF(*base) || EXTREF(*base)))
				return BASE_PCREL;
		}
	}
	return BASE_ILLEGAL;
