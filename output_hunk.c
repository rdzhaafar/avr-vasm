   ...
   Remember the total number of sections left in sec_cnt. */
{
	...
	for (sec_cnt = 0, sec = first_sec, first_nonbss = NULL; sec != NULL;
	     sec = sec->next) {
		/* ignore empty sections without symbols, unless -keepempty was given */
		if (keep_empty_sects || get_sec_size(sec) != 0 ||
		...
		for (a = sec->first; a; a = a->next) {
			if (a->type == DATA) {
				for (rl = a->content.db->relocs; rl;
				     rl = rl->next) {
					if (rl->type == REL_ABS ||
					    rl->type == REL_PC) {
						if (((nreloc*)rl->reloc)->size ==
						    32) {
							symbol* s =
								((nreloc*)rl
									 ->reloc)
		...
		if (a->type == DATA) {
			...
			if (a->content.db->relocs) {
				...
				for (d = a->content.db->data;
				     d <
				     a->content.db->data + a->content.db->size;
				     d++) {
					if (*d) {
						zerodata = 0;
						break;
			...
			if (a->content.sb->relocs) {
				...
				for (d = a->content.sb->fill;
				     d <
				     a->content.sb->fill + a->content.sb->size;
				     d++) {
					if (*d) {
						zerodata = 0;
						break;
	...
	for (a = sec->first; a; a = a->next) {
		...
		if (a->type == SPACE) {
			...
			if (sb->flags & SPC_DATABSS) {
				...
				output_atom_error(
					13,
					a); /* warn about data following a DX directive */
				dxpc = 0;
			}
		} else if (a->type == DATA) {
			if (dxpc != 0) {
	...
	if (rl->type <= LAST_STANDARD_RELOC
		...
		if (EXTREF(r->sym)) {
			...
			case REL_ABS:
				...
				switch (r->size) {
				case 8:
					type = kick1 ? EXT_RELREF8 :
							     EXT_ABSREF8;
					break;
				case 16:
					type = kick1 ? EXT_RELREF16 :
							     EXT_ABSREF16;
					break;
				case 32:
					if (com) {
...
/* convert an atom's rlist into relocations and xrefs */
{
	...
	do {
		...
		if (hr != NULL &&
		    (xreflist != NULL || hr->hunk_id == HUNK_ABSRELOC32 ||
			...
			if (xref) {
				if (xreflist)
					addtail(xreflist,
						&xref->n); /* add new external reference */
				else
					output_atom_error(8, a, xref->name,
							  sec->name,
							  xref->offset,
							  rl->type);
			} else
				unsupp_reloc_error(
					rl); /* reloc not supported */
		}
	} while (rl = rl->next);
}
		...
		if (n > 0) {
			if (bytes == 0) {
				if (shrt && type == HUNK_ABSRELOC32)
					fw32(f, HUNK_DREL32,
					     1); /* RELOC32SHORT is DREL32 for OS2.0 */
				else
					fw32(f, type, 1);
				bytes = 4;
			}
...
static void linedebug_hunk(FILE* f, struct list* ldblist)
{
	...
	while (ldbblk = (struct linedb_block*)remhead(ldblist)) {
		...
		if (!abs_path(ldbblk->filename)) {
			...
			snprintf(abspathbuf, MAXPATHLEN, "%s%s", cwd,
				 ldbblk->filename);
			myfree(cwd);
		} else
...
/* write all external references from a section into a HUNK_EXT hunk */
{
	while (xreflist->first->next) {
		...
		for (n = 0, x = (struct hunkxref*)xreflist->first; x->n.next;
		     x = (struct hunkxref*)x->n.next) {
	...
	if (sec) {
		for (; sec; sec = sec->next) {
			if (!(sec->flags & SEC_DELETED)) {
				...
				ext_refs(f, &xreflist);
				if (sec->idx ==
				    0) /* absolute definitions into first hunk */
					ext_defs(f, EXPRESSION, EXPORT, 0,
						 EXT_ABS);
...
static void write_exec(FILE* f, section* sec, symbol* sym)
{
	...
	if (sec_cnt) {
		fw32(f, sec_cnt,
		     1); /* number of sections - no overlay support */
		...
		for (; sec; sec = sec->next) {
			if (!(sec->flags & SEC_DELETED)) {
				...
				if (type != HUNK_BSS) {
					...
					size = databss ? file_size(sec) :
							       sect_size(sec);
					fw32(f, (size + 3) >> 2, 1);
					for (a = sec->first, pc = 0;
					     a != NULL && pc < size;
					     a = a->next) {
						npc = fwpcalign(f, a, sec, pc);
				...
				if (!kick1)
					reloc_hunk(f, HUNK_ABSRELOC32, 1,
						   &reloclist);
				reloc_hunk(f, HUNK_ABSRELOC32, 0, &reloclist);
				if (!kick1) /* RELRELOC32 works with short 16-bit offsets only! */
					reloc_hunk(f, HUNK_RELRELOC32, 1,
						   &reloclist);
				report_bad_relocs(&reloclist);
		     ...
		     int (**oa)(char*))
{
	...
	*oa = exec_out ? exec_args : object_args;
	secname_attr =
		1; /* attribute is used to differentiate between sections */
	return 1;
}
