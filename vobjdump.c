...
static const char* reloc_name[] = { "NONE",    "ABS",	  "PC",	     "GOT",
				    "GOTPC",   "GOTOFF",  "GLOBDAT", "PLT",
				    "PLTPC",   "PLTOFF",  "SD",	     "UABS",
				    "LOCALPC", "LOADREL", "COPY",    "JMPSLOT",
				    "SECOFF",  NULL };
static const char* type_name[] = { "", "obj", "func", "sect", "file", NULL };
...
static void read_section(struct vobj_section* vsect, struct vobj_symbol* vsym,
			 int nsyms)
{
	const char* attr;
	unsigned long flags;
	...
	for (i = 0; i < nrelocs; i++) {
		...
		if (type < sizeof(reloc_name) / sizeof(reloc_name[0])) {
			...
			rsize = read_number(
				0); /* size of special relocation entry */
			if (rsize < 0) {
...
static const char* def_name(struct vobj_symbol* vs, struct vobj_section* sec,
			    int nsecs)
{
	switch (vs->type) {
	case EXPRESSION:
...
static int vobjdump(void)
{
	...
	if (vlen > 4 && p[0] == 0x56 && p[1] == 0x4f && p[2] == 0x42 &&
		...
		printf("VOBJ %s (%s endian), %d bits per byte, %d bytes per word.\n"
		       "%d symbol%s.\n%d section%s.\n",
		       cpu_name, endian_name[endian - 1], bpb, bpt, nsyms,
		       nsyms == 1 ? emptystr : sstr, nsecs,
		       nsecs == 1 ? emptystr : sstr);

		/* read symbols */
		if (nsyms) {
			if (vsymbols = malloc(nsyms *
					      sizeof(struct vobj_symbol))) {
				for (i = 0; i < nsyms; i++)
					read_symbol(&vsymbols[i]);
			} else {
				fprintf(stderr,
					"Cannot allocate %ld bytes for symbols!\n",
					(long)(nsyms *
					       sizeof(struct vobj_symbol)));
				return 1;
			}
		...
		if (vsect = malloc(nsecs * sizeof(struct vobj_section))) {
			...
			fprintf(stderr,
				"Cannot allocate %ld bytes for sections!\n",
		...
		for (i = 0; i < nsyms; i++) {
			...
			printf("%08llx: %-4s %08x %-4s %8.8s %8llx %s\n",
			       BPTMASK(vs->offs), bind_name(vs->flags),
			       (unsigned)vs->size, type_name[TYPE(vs)],
