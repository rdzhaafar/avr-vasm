...
static utaddr get_reloc_type(rlist** rl, utaddr* roffset, taddr* addend,
			     symbol** refsym)
{
	rlist* rl2;
	utaddr mask;
...
/* convert all of an atom's relocations into ELF32/ELF64 relocs */
{
	...
	if (rl) {
		do {
			...
			if (rtype = get_reloc_type(&rl, &offset, &addend,
						   &refsym)) {
				if (LOCREF(refsym)) {
					/* this is a local relocation */
					addrel(pc + offset, addend,
					       refsym->sec->idx, rtype);
					ro += elfrelsize;
				} else if (EXTREF(refsym)) {
					/* this is an external symbol reference */
					unsigned idx =
						findelfsymbol(refsym->name);

					if (idx == 0) {
						/* create a new symbol, which can be referenced */
						idx = symindex;
						newsym(refsym->name, 0, 0,
					   ...
					   uint8_t, unsigned))
{
	...
	for (secp = sec; secp; secp = secp->next) {
		if (keep_empty_sects || get_sec_size(secp) != 0 ||
		    (secp->flags & HAS_SYMBOLS)) {
			...
			secp->idx = shdrindex;
			makeshdr(addString(&shstrlist, secp->name), type,
				 get_sec_flags(secp->attr), soffset,
	...
	if (!no_symbols && bits == 32 && nlist != NULL) {
		...
		while (nlist != NULL) {
			stablen++;
			if (nlist->type == N_SO && nlist->name.ptr != NULL &&
			    *nlist->name.ptr != '\0')
		...
		stabidx = shdrindex;
		shn = makeshdr(addString(&shstrlist, stabname), SHT_PROGBITS, 0,
			       soffset, (stablen + 1) * sizeof(struct nlist32),
			       0, 4, sizeof(struct nlist32));
		soffset += (stablen + 1) * sizeof(struct nlist32);
		setval(be, shn->s.sh_link, 4,
		       shdrindex); /* associated .stabstr section */
		/* make .stabstr section */
		makeshdr(addString(&shstrlist, ".stabstr"), SHT_STRTAB, 0,
			 soffset, stabstrlist.index, 0, 1, 0);
	...
	if (!no_symbols) /* symbols with local binding first */
		for (symp = first; symp; symp = symp->next)
			if (*symp->name != '.' && *symp->name != ' ' &&
			    !(symp->flags & VASMINTERN))
				if (symp->type != IMPORT &&
				    !(symp->flags & (EXPORT | WEAK)))
					newsym(symp->name, get_sym_value(symp),
					       get_sym_size(symp), STB_LOCAL,
					       get_sym_info(symp),
					       get_sym_index(symp));
	...
	for (symp = first; symp; symp = symp->next)
		if (*symp->name != '.' && !(symp->flags & VASMINTERN))
			if ((symp->type != IMPORT &&
			     (symp->flags & (EXPORT | WEAK))) ||
			    (symp->type == IMPORT &&
			     (symp->flags & (COMMON | WEAK))))
				newsym(symp->name, get_sym_value(symp),
				       get_sym_size(symp),
				       (symp->flags & WEAK) ? STB_WEAK :
								    STB_GLOBAL,
...
static void make_reloc_sections(section* sec,
				void (*newsym)(char*, elfull, elfull, uint8_t,
					       uint8_t, unsigned),
				void (*addrel)(elfull, elfull, elfull, elfull),
				void* (*makeshdr)(elfull, elfull, elfull,
						  elfull, elfull, elfull,
						  elfull, elfull))
{
	struct stabdef* nlist = first_nlist;
	utaddr roffset = 0;
	...
	for (secp = sec; secp; secp = secp->next) {
		if (secp->idx) {
			...
			for (a = secp->first, basero = roffset, pc = 0; a;
			     a = a->next) {
				npc = pcalign(a, pc);
				if (a->type == DATA)
					roffset += make_relocs(
						a->content.db->relocs, npc,
						newsym, addrel);
				if (a->type == SPACE)
					roffset += make_relocs(
						a->content.sb->relocs, npc,
						newsym, addrel);
				pc = npc + atom_size(a, secp, npc);
			}

			if (basero != roffset) /* were there any relocations? */
				make_relsechdr(secp->name, basero,
					       roffset - basero, secp->idx,
					       makeshdr);
		}
	}
	...
	if (!no_symbols) {
		...
		while (nlist != NULL) {
			if (nlist->base != NULL)
				roffset += make_stabreloc(pc, nlist, newsym,
							  addrel);
			nlist = nlist->next;
		...
		if (basero != roffset)
			make_relsechdr(stabname, basero, roffset - basero,
				       stabidx, makeshdr);
	}
}
...
static void write_ELF64(FILE* f, section* sec, symbol* sym)
{
	...
	elfrelsize =
		RELA ? sizeof(struct Elf64_Rela) : sizeof(struct Elf64_Rel);
	...
	makeShdr64(shstrtabidx, SHT_STRTAB, 0, soffset, shstrlist.index, 0, 1,
		   0);
	...
	setval(be, header.e_shoff, 8,
	       soffset); /* remember offset of Shdr table */
	...
	shn = makeShdr64(symtabidx, SHT_SYMTAB, 0, soffset,
			 symindex * sizeof(struct Elf64_Sym), firstglobal, 8,
			 sizeof(struct Elf64_Sym));
	setval(be, shn->s.sh_link, 4,
	       shdrindex); /* associated .strtab section */
	...
	while (shn = (struct Shdr64Node*)remhead(&shdrlist)) {
		if (readval(be, shn->s.sh_type, 4) == shtreloc) {
			...
			setval(be, shn->s.sh_offset, 8,
			       readval(be, shn->s.sh_offset, 8) + soffset);
			setval(be, shn->s.sh_link, 4,
			       shdrindex - 2); /* index of associated symtab */
		}
...
static void write_ELF32(FILE* f, section* sec, symbol* sym)
{
	...
	elfrelsize =
		RELA ? sizeof(struct Elf32_Rela) : sizeof(struct Elf32_Rel);
	...
	/* ".shstrtab" section header string table */
	makeShdr32(shstrtabidx, SHT_STRTAB, 0, soffset, shstrlist.index, 0, 1,
		   0);
	soffset += shstrlist.index;
	align1 = balign(soffset, 4);
	soffset += align1;
	...
	shn = makeShdr32(symtabidx, SHT_SYMTAB, 0, soffset,
			 symindex * sizeof(struct Elf32_Sym), firstglobal, 4,
	...
	while (shn = (struct Shdr32Node*)remhead(&shdrlist)) {
		if (readval(be, shn->s.sh_type, 4) == shtreloc) {
			/* set correct offset and link to symtab */
			setval(be, shn->s.sh_offset, 4,
			       readval(be, shn->s.sh_offset, 4) + soffset);
			setval(be, shn->s.sh_link, 4,
			       shdrindex - 2); /* index of associated symtab */
		}
...
static void write_output(FILE* f, section* sec, symbol* sym)
{
	...
	if (bits == 32 && cpu != EM_NONE)
		...
		output_error(1,
			     cpuname); /* output module doesn't support cpu */
