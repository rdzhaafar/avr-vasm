...
static char* copyright =
	"vasm vobj output module 1.0 (c) 2002-2020 Volker Barthelmann";
...
static int count_relocs(rlist* rl)
{
	...
	for (nrelocs = 0; rl; rl = rl->next) {
		if (rl->type >= FIRST_STANDARD_RELOC &&
		    rl->type <= LAST_STANDARD_RELOC) {
...
static void get_section_sizes(section* sec, taddr* rsize, taddr* rdata,
			      taddr* rnrelocs)
{
...
static void write_rlist(FILE* f, section* sec, rlist* rl)
{
	...
	for (; rl; rl = rl->next) {
		if (rl->type >= FIRST_STANDARD_RELOC &&
			...
			nreloc* rel = rl->reloc;
			if (!(idx = rel->sym->idx)) {
				if (rel->sym->type == IMPORT &&
				    !sym_valid(rel->sym))
					continue;
				idx = rel->sym->sec
					      ->idx; /* symbol does not exist, use section-symbol */
			}
...
static void write_output(FILE* f, section* sec, symbol* sym)
{
	...
	for (nsecs = 1, secp = sec, first = sym, last = NULL; secp;
	     secp = secp->next) {
		secp->idx =
			nsecs++; /* symp->idx will become equal to secp->idx */
...
int init_output_vobj(char** cp, void (**wo)(FILE*, section*, symbol*),
		     int (**oa)(char*))
{
	*cp = copyright;
	*wo = write_output;
