...
/* For xtern=-1, return true when this relocation requires a base symbol. */
{
	...
	if (nr = (nreloc*)(*rl)->reloc) {
		...
		if (nr->bitoffset == 0 &&
		    (nr2 == NULL || nr2->byteoffset != nr->byteoffset) &&
		...
		if (b != 0 && s < 4) {
			if (b > 0)
				setbits(be, &r, sizeof(r) << 3, (unsigned)b, 1,
					1);
			setbits(be, &r, sizeof(r) << 3, RSTDB_length,
				RSTDS_length, s);
			setbits(be, &r, sizeof(r) << 3, RSTDB_extern,
				RSTDS_extern, xtern ? 1 : 0);
			return readbits(be, &r, sizeof(r) << 3, RELB_reloc,
					RELS_reloc);
		}
	}
...
static void aout_initwrite(section* firstsec)
{
	...
	aoutstrlist.hashtab =
		mycalloc(STRHTABSIZE * sizeof(struct StrTabNode*));
	aoutstrlist.nextoffset = 4; /* first string is always at offset 4 */
	initlist(&aoutsymlist.l);
	aoutsymlist.hashtab =
		mycalloc(SYMHTABSIZE * sizeof(struct SymbolNode*));
       ...
       we would have to calculate it from the atoms and store it there */
		if (get_sec_size(sec) > 0 || (sec->flags & HAS_SYMBOLS)) {
			...
			if (i == S_MISS) {
				output_error(
					3,
					sec->attr); /* section attributes not supported */
				i = S_TEXT;
			}
			if (i == S_ABS)
			...
			if (!sections[i]) {
				...
				sec->idx =
					i; /* section index 0:text, 1:data, 2:bss */
			} else
	...
	for (sec = firstsec; sec; sec = sec->next) {
		if (sec->flags & ABSOLUTE)
			secsize[S_TEXT] += balign(secsize[S_TEXT], sec->align) +
					   get_sec_size(sec);
	}
...
static uint32_t aout_addsymhash(char* name, taddr value, int bind, int info,
				int type, int desc, int be)
/* add a new symbol, return its symbol table index */
{
	struct SymbolNode **chain, *sym;

	chain = &aoutsymlist.hashtab[hashcode(name ? name : emptystr) %
				     SYMHTABSIZE];
	while (sym = *chain)
		chain = &sym->hashchain;
	...
	*chain = sym =
		aout_addsym(name, type, ((bind & 0xf) << 4) | (info & 0xf),
			    desc, value, be);
	return sym->index;
}
...
/* find a symbol by its name, return symbol table index or -1 */
{
	struct SymbolNode** chain =
		&aoutsymlist.hashtab[hashcode(name) % SYMHTABSIZE];
...
/* common symbol */
      ...
      type = N_COMM | ext;
			...
			if (!(sym->sec->flags & ABSOLUTE)) {
				...
				val += secoffs
					[sym->sec->idx]; /* a.out requires to add sec. offset */
	...
	if (size) {
		...
		aout_addsymhash(sym->name, size, symbind, syminfo, N_SIZE, 0,
				be);
	}
}
...
/* add stabs to the a.out symbol list */
{
	...
	while (nlist != NULL) {
		...
		aout_addsym(nlist->name.ptr, nlist->type, nlist->other,
			    nlist->desc, val, be);
		nlist = nlist->next;
	}
}
...
static uint32_t
aout_convert_rlist(int be, atom* a, int secid, struct list* rlst, taddr pc,
...
/* convert all of an atom's relocs into a.out relocations */
{
	...
	do {
		...
		if (LOCREF(refsym)) {
			...
			aout_addreloclist(
				rlst, pc + r->byteoffset, sectype[rsecid],
			...
			if ((symidx = aout_findsym(refsym->name, be)) == -1)
				symidx = aout_addsymhash(refsym->name, 0, 0, 0,
							 N_UNDF | N_EXT, 0, be);
			aout_addreloclist(rlst, pc + r->byteoffset, symidx,
					  getrinfo(&rl, 1,
						   sections[secid]->name, be),
					  be);
			rsize += sizeof(struct relocation_info);
		} else
			ierror(0);
		...
		if (rl->type == REL_PC)
			val -= pc + r->byteoffset;
		if (a->type == DATA)
			setval(be, a->content.db->data + r->byteoffset,
			       r->size >> 3, val + add);
		else if (a->type == SPACE && a->content.sb->space != 0) {
			setval(be, a->content.sb->fill, r->size >> 3,
			       val + add);
			a->content.sb->space =
				0; /* we only need to patch 'fill' once */
		}
	} while (rl = rl->next);
...
/* creates a.out relocations for a single section (.text or .data) */
{
	...
	if (sections[secid]) {
		...
		for (a = sections[secid]->first; a; a = a->next) {
			npc = pcalign(a, pc);
			rtabsize += aout_convert_rlist(be, a, secid, rlst, npc,
						       getrinfo);
...
static void aout_header(FILE* f, uint32_t mag, uint32_t flag, uint32_t tsize,
			uint32_t dsize, uint32_t bsize, uint32_t syms,
...
static void write_output(FILE* f, section* sec, symbol* sym)
{
	...
	aout_header(f, OMAGIC, isPIC ? EX_PIC : 0,
		    ...
		    secsize[S_BSS],
		    aoutsymlist.nextindex * sizeof(struct nlist32), 0, trsize,
		    drsize, be);
