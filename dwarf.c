...
static const unsigned char stdopclengths[] = { 0, 1, 1, 1, 1, 0, 0, 0, 1 };

static unsigned char dw2_compile_unit_abbrev[] = { DW_TAG_compile_unit,
						   0, /* no children */
						   DW_AT_producer,
						   DW_FORM_string,
						   DW_AT_language,
...
static unsigned char dw3_compile_unit_abbrev[] = { DW_TAG_compile_unit,
						   0, /* no children */
						   DW_AT_producer,
						   DW_FORM_string,
						   DW_AT_language,
...
static void make_file_lists(struct source_file* first_source)
{
	...
	for (srcnode = first_source; srcnode; srcnode = srcnode->next) {
		...
		if (pathbuf[0]) {
			...
			if (first_dwinc) {
				for (i = 1, dwinc = first_dwinc;;
				     i++, dwinc = dwinc->next) {
					if (!filenamecmp(newpath,
							 dwinc->name)) {
						myfree(newpath);
						newfil->incidx =
							i; /* use existing include index */
						break;
					}
					if (dwinc->next == NULL) {
						if (include_idx != i)
				...
				newfil->incidx =
					++include_idx; /* should be index 1 */
			}
		} else
			newfil->incidx =
				0; /* no path, file is in current work directory */
...
void dwarf_init(struct dwarf_info* dinfo, struct include_path* first_incpath,
		struct source_file* first_source)
{
	...
	dinfo->max_lnadvance_hipc = 255 -
				    dinfo->max_pcadvance * dinfo->line_range -
				    dinfo->opcode_base;
	...
	dinfoatom =
		add_data_atom(dsec, 4, 1, 0); /* 32-bit reloc to .debug_info */
	...
	add_data_atom(dsec, 2, 1, dinfo->version);
	dabbratom = add_data_atom(dsec, 4, 1,
				  0); /* 32-bit reloc to .debug_abbrev */
	add_data_atom(dsec, 1, 1, dinfo->addr_len);
	...
	dlineatom =
		add_data_atom(dsec, 4, 1, 0); /* 32-bit reloc to .debug_line */
	...
	if (dinfo->version < 3) {
       ...
       is to set low_pc=0 and high_pc=~0 to cover the whole address space. */
		...
		dinfo->highpc_atom =
			add_data_atom(dsec, dinfo->addr_len, 1, ~0);
	} else {
		/* address ranges defined in .debug_ranges since DWARF3 */
		drangatom = add_data_atom(
			dsec, 4, 1, 0); /* 32-bit reloc to .debug_ranges */
	...
	if (dinfo->version >= 3) {
		dinfo->rsec = dsec =
			new_section(".debug_ranges", "r", 2 * dinfo->addr_len);

		/* make 32-bit reloc for ranges-start label */
		tmpsym = new_tmplabel(dsec);
		add_extnreloc(&drangatom->content.db->relocs, tmpsym,
			      tmpsym->pc, REL_ABS, 0, 32, 0);
...
void dwarf_end_sequence(struct dwarf_info* dinfo, section* sec)
{
	if (!dinfo->end_sequence) {
		...
		add_data_atom(dinfo->asec, dinfo->addr_len, dinfo->addr_len,
			      sec->pc);

		if (dinfo->highpc_atom) {
			if (dinfo->code_sections > 1)
				/* low_pc=0, high_pc=~0 for multiple sections as workaround */
				set_atom_val(dinfo->highpc_atom,
					     dinfo->addr_len, ~0);
			else
				/* we can set high_pc, when there is just a single code section */
				set_atom_label(dinfo->highpc_atom,
					       dinfo->addr_len, sym);
		}

		if (dinfo->rsec) {
			/* enter end-of-section label reference into the ranges table */
			a = add_data_atom(dinfo->rsec, dinfo->addr_len,
					  dinfo->addr_len, sym->pc);
			add_extnreloc(&a->content.db->relocs, sym, sym->pc,
				      REL_ABS, 0, dinfo->addr_len << 3, 0);
		}
	}
}
...
void dwarf_line(struct dwarf_info* dinfo, section* sec, int file, int line)
{
	...
	if (dinfo->end_sequence) {
		...
		a = add_data_atom(dinfo->asec, dinfo->addr_len, dinfo->addr_len,
				  0);
		add_extnreloc(&a->content.db->relocs, sym, sym->pc, REL_ABS, 0,
			      dinfo->addr_len << 3, 0);
		...
		if (dinfo->lowpc_atom) {
			if (dinfo->code_sections > 1)
				/* low_pc=0, high_pc=~0 for multiple sections as workaround */
				set_atom_val(dinfo->lowpc_atom, dinfo->addr_len,
					     0);
			else
				/* we can set low_pc, when there is just a single code section */
				set_atom_label(dinfo->lowpc_atom,
					       dinfo->addr_len, sym);
		}
