...
#define RFLD(p, s)                                                             \
	(BIGENDIAN ? (size == (s) && (pos & 15) == (p)) :                      \
			   (size == (s) && (pos & 15) == (16 - (p) - (s))))
	...
	*roffset = r->byteoffset;
	...
	case REL_ABS:
		if (pos == 0 && mask == ~0) {
			...
			if ((mask == 0xffff0000 && r2->mask == 0xffff &&
			     r2->size == 16 && r2->bitoffset == 0) ||
			    (mask == 0xffff && r2->mask == 0xffff0000 &&
			     r2->size == 16 && r2->bitoffset == 16)) {
				t = R_JAG_ABS32SWP;
				*rl = (*rl)->next;
			}
		}
