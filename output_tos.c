...
static int write_reloc68k(atom* a, rlist* rl, int signedval, taddr val)
{
	...
	if (field_overflow(signedval, nrel->size, val)) {
		output_atom_error(12, a, rl->type, (unsigned long)nrel->mask,
				  nrel->sym->name, (unsigned long)nrel->addend,
				  nrel->size);
		return 0;
	}
	...
	if (a->type == DATA)
		...
		p = (char*)a->content.sb
			    ->fill; /* @@@ ignore offset completely? */
	else
		return 1;

	setbits(1, p, (nrel->bitoffset + nrel->size + 7) & ~7, nrel->bitoffset,
		nrel->size, val);
	return 1;
}
...
static void write_dri_sym(FILE* f, char* name, int type, taddr value)
{
	...
	setval(1, stab.type, sizeof(stab.type),
	       longname ? (type | STYP_LONGNAME) : type);
	...
	if (longname) {
		...
		strncpy(rest_of_name, name + DRI_NAMELEN,
			sizeof(struct DRIsym));
       ...
       no matter if global or local. But no equates or other types. */
		if (!(sym->flags & (VASMINTERN | COMMON)) &&
			...
			if (sym->flags & WEAK)
				output_error(
					10,
					sym->name); /* weak symbol not supported */
			t = labtype[sym->sec->idx] | STYP_DEFINED | STYP_GLOBAL;
			write_dri_sym(f, sym->name, t,
				      tos_sym_value(sym, textbasedsyms));
		}
	}
}
	...
	if (sec) {
		...
		for (a = sec->first; a; a = a->next) {
			...
			if (nrel) {
				...
				if (nrel > 1)
					qsort(sortoffs, nrel, sizeof(int),
					      offscmp);
