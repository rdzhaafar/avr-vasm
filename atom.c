...
instruction* new_inst(char* inst, int len, int op_cnt, char** op, int* op_len)
{
      ...
      mnemo_opcnt = op_cnt<MAX_OPERANDS ? op_cnt : MAX_OPERANDS;
			...
			for (j = 0; j < MAX_OPERANDS; j++)
				if (mnemonics[i].operand_type[j] == 0)
					break;
			mnemo_opcnt =
				j; /* number of expected operands for this mnemonic */
#endif
			inst_found = 2;
			save_symbols(); /* make sure we can restore symbols to this point */

			for (j = k = omitted = skipped = 0, again = -1;
			     j < mnemo_opcnt; j++) {
			...
			return new;
			 ...
			 !strnicmp(mnemonics[i].name, inst, len) &&
	...
	case 2:
		...
		break;
	default:
		general_error(1, cnvstr(inst,
					len)); /* completely unknown mnemonic */
		break;
	}
	myfree(new);
	return 0;
	...
	if (final_pass && sb->fill_exp) {
		if (sb->size <= sizeof(taddr)) {
			...
			if (!eval_expr(sb->fill_exp, &fill, sec, pc)) {
				if (find_base(sb->fill_exp, &base, sec, pc) ==
				    BASE_ILLEGAL)
					general_error(
			...
			if (base && !sb->relocs) {
				...
				for (i = 0; i < space; i++)
					add_extnreloc(&sb->relocs, base, fill,
						      REL_ABS, 0, sb->size << 3,
						      sb->size * i);
			}
		} else
...
void add_atom(section* sec, atom* a)
{
	...
	if (sec->last) {
		...
		if (pa->type == LABEL && pa->line == a->line &&
		    (a->type == INSTRUCTION || a->type == DATADEF ||
		     a->type == SPACE))
			pa->align = a->align;
	} else
		sec->first = a;
...
size_t atom_size(atom* p, section* sec, taddr pc)
{
	...
	case INSTRUCTION:
		return p->content.inst->code >= 0 ?
				     instruction_size(p->content.inst, sec, pc) :
				     0;
	case SPACE:
...
static void print_instruction(FILE* f, instruction* p)
{
	...
	printf("inst %d(%s) ", p->code,
	       p->code >= 0 ? mnemonics[p->code].name : "deleted");
#if MAX_OPERANDS != 0
	for (i = 0; i < MAX_OPERANDS; i++)
...
void print_atom(FILE* f, atom* p)
{
	...
	case SPACE:
		fprintf(f, "space(%lu,fill=",
			(unsigned long)(p->content.sb->space *
					p->content.sb->size));
		for (i = 0; i < p->content.sb->size; i++)
			fprintf(f, "%02x%c",
				(unsigned char)p->content.sb->fill[i],
				(i == p->content.sb->size - 1) ? ')' : ' ');
		for (rl = p->content.sb->relocs; rl; rl = rl->next)
			print_reloc(f, rl->type, rl->reloc);
		break;
	case DATADEF:
		fprintf(f, "datadef(%lu bits)",
			(unsigned long)p->content.defb->bitsize);
		break;
	case LINE:
		fprintf(f, "line: %d of %s", p->content.srcline,
			getdebugname());
		break;
#if HAVE_CPU_OPTS
	case OPTS:
	...
	case ROFFS:
		...
		break;
	case RORG:
		fprintf(f, "rorg: relocate to 0x%llx",
			ULLTADDR(*p->content.rorg));
		break;
	case RORGEND:
		fprintf(f, "rorg end");
		break;
	case ASSERT:
		fprintf(f, "assert: %s (message: %s)\n",
			...
			p->content.assert->msgstr ? p->content.assert->msgstr :
							  emptystr);
		break;
	case NLIST:
		fprintf(f, "nlist: %s (type %d, other %d, desc %d) with value ",
			p->content.nlist->name != NULL ?
				      p->content.nlist->name :
				      "<NULL>",
