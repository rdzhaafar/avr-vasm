...
char* syntax_copyright =
	"vasm madmac syntax module 0.4g (c) 2015-2020 Frank Wille";
hashtable* dirhash;
char commentchar = ';';
...
static struct namelen macro_dirlist[] = { { 6, &macroname[0] },
					  { 5, &macroname[1] },
					  { 0, 0 } };
static struct namelen endm_dirlist[] = { { 5, &endmname[0] },
					 { 4, &endmname[1] },
					 { 0, 0 } };
static struct namelen rept_dirlist[] = { { 5, &reptname[0] },
					 { 4, &reptname[1] },
					 { 0, 0 } };
static struct namelen endr_dirlist[] = { { 5, &endrname[0] },
					 { 4, &endrname[1] },
					 { 0, 0 } };
	...
	for (;;) {
		...
		if (!db) {
			...
			if (parse_operand(opstart, s - opstart, op,
					  DATA_OPERAND(sz)))
				add_atom(0,
					 new_datadef_atom(OPSZ_BITS(sz), op));
			else
...
static void handle_endm(char* s)
{
	syntax_error(12, endmname,
		     macroname); /* unexpected endm without macro */
}
			...
			while (*s == '/') {
				...
				if (s = skip_identifier(s)) {
					...
					default:
						syntax_error(
							7,
							f); /* unknown print format flag */
						break;
					}
				} else {
					syntax_error(
						9); /* print format corrupted */
					break;
				}
				s = skip(s);
			}
...
struct {
	char* name;
	void (*func)(char*);
} directives[] = { "equ",     handle_equ,     "=",	  handle_equ,
		   "set",     handle_set,     "if",	  handle_if,
		   "else",    handle_else,    "endif",	  handle_endif,
		   "rept",    handle_rept,    "endr",	  handle_endr,
		   "macro",   handle_macro,   "endm",	  handle_endm,
		   "exitm",   handle_exitm,   "macundef", handle_macundef,
		   "text",    handle_text,    "data",	  handle_data,
		   "bss",     handle_bss,     "org",	  handle_org,
		   "68000",   handle_68000,   "globl",	  handle_globl,
		   "extern",  handle_globl,   "assert",	  handle_assert,
		   "dc",      handle_d16,     "dc.b",	  handle_d8,
		   "dc.w",    handle_d16,     "dc.l",	  handle_d32,
		   "dc.i",    handle_i32,     "dcb",	  handle_blk16,
		   "dcb.b",   handle_blk8,    "dcb.w",	  handle_blk16,
		   "dcb.l",   handle_blk32,   "ds",	  handle_spc16,
		   "ds.b",    handle_spc8,    "ds.w",	  handle_spc16,
		   "ds.l",    handle_spc32,   "end",	  handle_end,
		   "even",    handle_even,    "long",	  handle_long,
		   "phrase",  handle_phrase,  "dphrase",  handle_dphrase,
		   "qphrase", handle_qphrase, "include",  handle_include,
		   "incbin",  handle_incbin,  "list",	  handle_list,
		   "nlist",   handle_nlist,   "nolist",	  handle_nlist,
		   "print",   handle_print,   "abs",	  handle_offset,
...
static int macro_arg_defined(source* src, char* argstart, char* argend, char* d)
{
	...
	if (n >= 0) {
		/* valid argument name */
		*d++ = (n < src->num_params && n < maxmacparams &&
			src->param_len[n] > 0) ?
				     '1' :
				     '0';
		return 1;
...
int expand_macro(source* src, char** line, char* d, int dlen)
{
	...
	if (*s++ == '\\') {
		...
		if (*s == '\\') {
			...
			if (dlen > 1) {
				*d++ = '.';
				if ((nc = copy_macro_qual(src, 0, d, dlen)) >=
				    0)
					nc++;
				s++;
			} else
			...
			if ((end = skip_identifier(s + 1)) != NULL) {
				...
				if ((nc = macro_arg_defined(src, s + 1, end,
							    d)) >= 0)
					s = end;
			} else if (*(s + 1) == '{' &&
				   (end = skip_identifier(s + 2)) != NULL) {
				if (*end == '}') {
					/* \?{argname} : 1 when argument defined, 0 when missing or empty */
					if ((nc = macro_arg_defined(
						     src, s + 2, end, d)) >= 0)
						s = end + 1;
				}
			}
		} else if (*s == '{' &&
			   (end = skip_identifier(s + 1)) != NULL) {
			if (*end == '}') {
				if ((n = find_macarg_name(
					     src, s + 1, end - (s + 1))) >= 0) {
