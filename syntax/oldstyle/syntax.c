...
char* syntax_copyright =
	"vasm oldstyle syntax module 0.15 (c) 2002-2020 Frank Wille";
hashtable* dirhash;
...
static struct namelen macro_dirlist[] = { { 5, &macroname[1] },
					  { 3, &macname[1] },
					  { 0, 0 } };
static struct namelen endm_dirlist[] = { { 4, &endmname[1] },
					 { 6, &endmname[1] },
					 { 8, &endmname[1] },
					 { 0, 0 } };
static struct namelen rept_dirlist[] = { { 4, &reptname[1] },
					 { 6, &repeatname[1] },
					 { 0, 0 } };
static struct namelen endr_dirlist[] = { { 4, &endrname[1] },
					 { 6, &endrname[1] },
					 { 9, &endrname[1] },
					 { 0, 0 } };
static struct namelen dmacro_dirlist[] = { { 6, &macroname[0] },
					   { 4, &macname[0] },
					   { 0, 0 } };
static struct namelen dendm_dirlist[] = { { 5, &endmname[0] },
					  { 7, &endmname[0] },
					  { 9, &endmname[0] },
					  { 0, 0 } };
static struct namelen drept_dirlist[] = { { 5, &reptname[0] },
					  { 7, &repeatname[0] },
					  { 0, 0 } };
static struct namelen dendr_dirlist[] = { { 5, &endrname[0] },
					  { 7, &endrname[0] },
					  { 10, &endrname[0] },
					  { 0, 0 } };
	...
	for (;;) {
		...
		/* For the Z80 ignore ' behind a letter, as it may be a register */
			...
			s = skip_string(s, c, NULL) - 1;
			 ...
			 c == ',' && par_cnt == 0)
			break;
		else if (instoper && OPERSEP_BLANK &&
...
static void handle_data_mod(char* s, int size, expr* tree)
{
	...
	if (tree) {
		...
		char* modname = make_local_label(NULL, 0, local_modif_name,
		...
		if (db == NULL) {
			...
			if (mod != NULL) {
				...
				add_atom(
					...
					new_data_atom(
						db,
						1)); /* add dblock-string unmodified */
		}
...
static void handle_str(char* s) /* GMGM entire handle_str function */
{
	...
	handle_data(s, 8);
	add_atom(0,
		 new_space_atom(number_expr(1), 1, fill)); /* terminating CR */
}
...
static void handle_org(char* s)
{
	if (*s == current_pc_char) {
		...
		if (*s == '#')
			s = skip(
				s +
				1); /* some strange assemblers allow ORG #<addr> */
		if (dsect_active)
...
static void handle_rept(char* s)
{
	...
	eol(s);
	new_repeat((int)cnt, NULL, NULL, dotdirs ? drept_dirlist : rept_dirlist,
		   dotdirs ? dendr_dirlist : endr_dirlist);
}

static void handle_endr(char* s)
{
	syntax_error(12, &endrname[1],
		     &repeatname[1]); /* unexpected endr without rept */
}
...
{
	syntax_error(12, &endmname[1],
		     &macroname[1]); /* unexpected endm without macro */
}
...
static void handle_inline(char* s)
{
	...
	if (inline_stack_index < INLSTACKSIZE) {
		...
		syntax_error(
			16,
			INLSTACKSIZE); /* maximum inline nesting depth exceeded */
}
...
static void handle_einline(char* s)
{
	if (inline_stack_index > 0) {
		if (--inline_stack_index == 0) {
			...
			sprintf(inl_lab_name, INLLABFMT,
				inline_stack[inline_stack_index - 1]);
...
struct {
	...
	"org",	     handle_org,       "rorg",	       handle_rorg,
	"rend",	     handle_rend,      "phase",	       handle_rorg,
	"dephase",   handle_rend,      "roffs",	       handle_roffs,
	"align",     handle_align,     "even",	       handle_even,
	"byte",	     handle_d8,	       "db",	       handle_d8,
	"dfb",	     handle_d8,	       "defb",	       handle_d8,
	"asc",	     handle_d8,	       "data",	       handle_d8,
	"defm",	     handle_text,      "text",	       handle_text,
	"wor",	     handle_d16,       "word",	       handle_d16,
	"addr",	     handle_d16,       "dw",	       handle_d16,
	"dfw",	     handle_d16,       "defw",	       handle_d16,
	"dd",	     handle_d32,
#if defined(VASM_CPU_650X) || defined(VASM_CPU_Z80) || defined(VASM_CPU_6800)
	"abyte",     handle_d8_mod,
#endif
	"ds",	     handle_spc8,      "dsb",	       handle_spc8,
	"fill",	     handle_spc8,      "reserve",      handle_spc8,
	"spc",	     handle_spc8,      "dsw",	       handle_spc16,
	"blk",	     handle_spc8,      "blkw",	       handle_spc16,
	"dc",	     handle_spc8,      "byt",	       handle_fixedspc1,
	"wrd",	     handle_fixedspc2, "assert",       handle_assert,
#if defined(VASM_CPU_TR3200) /* Clash with IFxx instructions of TR3200 cpu */
	"if_def",    handle_ifd,       "if_ndef",      handle_ifnd,
	"if_eq",     handle_ifeq,      "if_ne",	       handle_ifne,
	"if_gt",     handle_ifgt,      "if_ge",	       handle_ifge,
	"if_lt",     handle_iflt,      "if_le",	       handle_ifle,
	"if_used",   handle_ifused,    "if_nused",     handle_ifnused,
#else
	"ifdef",     handle_ifd,       "ifndef",       handle_ifnd,
	"ifd",	     handle_ifd,       "ifnd",	       handle_ifnd,
	"ifeq",	     handle_ifeq,      "ifne",	       handle_ifne,
	"ifgt",	     handle_ifgt,      "ifge",	       handle_ifge,
	"iflt",	     handle_iflt,      "ifle",	       handle_ifle,
	"ifused",    handle_ifused,    "ifnused",      handle_ifnused,
#endif
	"if",	     handle_ifne,      "else",	       handle_else,
	"el",	     handle_else,      "endif",	       handle_endif,
#if !defined(VASM_CPU_Z80) && !defined(VASM_CPU_6800)
	"ei",	     handle_endif, /* Clashes with z80 opcode */
#endif
	"fi",	     handle_endif, /* GMGM */
	"incbin",    handle_incbin,    "mdat",	       handle_incbin,
	"incdir",    handle_incdir,    "include",      handle_include,
	"rept",	     handle_rept,      "repeat",       handle_rept,
	"endr",	     handle_endr,      "endrep",       handle_endr,
	"endrepeat", handle_endr,      "mac",	       handle_macro,
	"macro",     handle_macro,     "endm",	       handle_endm,
	"endmac",    handle_endm,      "endmacro",     handle_endm,
	"end",	     handle_end,       "fail",	       handle_fail,
	"section",   handle_section,   "dsect",	       handle_dsect,
	"dend",	     handle_dend,      "binary",       handle_incbin,
	"defs",	     handle_spc8,      "defp",	       handle_d24,
	"defl",	     handle_d32,       "defc",	       handle_defc,
	"xdef",	     handle_global,    "xref",	       handle_global,
	"lib",	     handle_global,    "xlib",	       handle_global,
	"global",    handle_global,    "extern",       handle_global,
	"local",     handle_local,     "weak",	       handle_weak,
	"ascii",     handle_string,    "asciiz",       handle_string,
	"string",    handle_string,    "str",	       handle_str, /* GMGM */
	"list",	     handle_list,      "nolist",       handle_nolist,
	"struct",    handle_struct,    "structure",    handle_struct,
	"endstruct", handle_endstruct, "endstructure", handle_endstruct,
	"inline",    handle_inline,    "einline",      handle_einline,
#if !defined(VASM_CPU_650X)
	"rmb",	     handle_spc8,
#endif
	"fcc",	     handle_text,      "fcs",	       handle_fcs,
	"fcb",	     handle_d8,	       "fdb",	       handle_d16,
	"bsz",	     handle_spc8,      "zmb",	       handle_spc8,
	"nam",	     handle_listttl,   "subttl",       handle_listsubttl,
	"page",	     handle_listpage,  "space",	       handle_listspace
};
...
static int execute_struct(char* name, int name_len, char* s)
{
	...
	for (p = str->first; p; p = p->next) {
		...
		if (p->type == DATA || p->type == SPACE || p->type == DATADEF) {
				...
				if (p->type == DATADEF) {
					...
					if (parse_operand(
						    ...
						    DATA_OPERAND(
							    p->content.defb
								    ->bitsize))) {
						new = new_datadef_atom(
							p->content.defb->bitsize,
							op);
						new->align = p->align;
						add_atom(0, new);
					} else
						syntax_error(
							8); /* invalid data operand */
				} else if (p->type == SPACE) {
					/* parse the fill expression for this space */
					new = clone_atom(p);
					new->content.sb = new_sblock(
						p->content.sb->space_exp,
						p->content.sb->size,
						parse_expr_tmplab(&opp));
					new->content.sb->space =
						p->content.sb->space;
					add_atom(0, new);
				} else {
			...
			syntax_error(
				23); /* skipping instruction in struct init */
		...
		if (labname = parse_label_or_pc(&s)) {
			...
			if (equlen) {
				...
				if (*labname == current_pc_char) {
					...
					s = skip(s + equlen);
					label = new_equate(
						labname, parse_expr_tmplab(&s));
				}
			} else if (!strnicmp(s, setname + !dotdirs,
					     3 + dotdirs) &&
				   isspace((unsigned char)*(s + 3 + dotdirs))) {
				...
				if (*labname == current_pc_char) {
					...
					s = skip(s + 3 + dotdirs);
					label = new_abs(labname,
							parse_expr_tmplab(&s));
				}
			} else if (!strnicmp(s, macname + !dotdirs,
					     3 + dotdirs) &&
					   (isspace((unsigned char)*(
						    s + 3 + dotdirs)) ||
					    *(s + 3 + dotdirs) == '\0') ||
				   !strnicmp(s, macroname + !dotdirs,
					     5 + dotdirs) &&
					   (isspace((unsigned char)*(
						    s + 5 + dotdirs)) ||
					    *(s + 5 + dotdirs) == '\0')) {
				char* params =
					skip(s + (*(s + 3 + dotdirs) == 'r' ?
								5 + dotdirs :
								3 + dotdirs));
				...
				if (!(labname = parse_identifier(&s)))
					ierror(0);
				new_macro(labname,
					  dotdirs ? dmacro_dirlist :
							  macro_dirlist,
					  dotdirs ? dendm_dirlist :
							  endm_dirlist,
					  params);
				myfree(labname);
				continue;
...
int expand_macro(source* src, char** line, char* d, int dlen)
{
	...
	if (*s++ == '\\') {
		...
		if (*s == '\\') {
			...
			nc = copy_macro_param(src, *s == '0' ? 0 : *s - '1', d,
					      dlen);
			s++;
...
{
	if (ISIDSTART(*p) ||
	    isdigit((unsigned char)*p)) { /* may start with digit */
		p++;
		while (ISIDCHAR(*p))
			p++;
	} else
...
/* Local labels start with a '.' or end with '$': "1234$", ".1" */
{
	...
	if (p != NULL && *p == '.' && ISIDCHAR(*(p + 1)) && ISIDSTART(*s) &&
	    *s != '.' && *(p - 1) != '$') {
