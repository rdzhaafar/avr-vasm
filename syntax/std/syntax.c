...
#if defined(VASM_CPU_C16X) || defined(VASM_CPU_M68K) ||                        \
	defined(VASM_CPU_650X) || defined(VASM_CPU_ARM) ||                     \
	defined(VASM_CPU_Z80) || defined(VASM_CPU_6800) ||                     \
	defined(VASM_CPU_JAGRISC) || defined(VASM_CPU_QNICE) ||                \
	defined(VASM_CPU_6809)
char commentchar = ';';
#else
char commentchar = '#';
#endif
...
static struct namelen dmacro_dirlist[] = { { 6, &macroname[0] }, { 0, 0 } };
static struct namelen dendm_dirlist[] = { { 5, &endmname[0] }, { 0, 0 } };
static struct namelen drept_dirlist[] = { { 5, &reptname[0] },
					  { 4, &irpname[0] },
					  { 5, &irpcname[0] },
					  { 0, 0 } };
static struct namelen dendr_dirlist[] = { { 5, &endrname[0] }, { 0, 0 } };
static struct namelen endm_dirlist[] = { { 4, &endmname[1] }, { 0, 0 } };
static struct namelen macro_dirlist[] = { { 5, &macroname[1] }, { 0, 0 } };
static struct namelen rept_dirlist[] = { { 4, &reptname[1] },
					 { 3, &irpname[1] },
					 { 4, &irpcname[1] },
					 { 0, 0 } };
static struct namelen endr_dirlist[] = { { 4, &endrname[1] }, { 0, 0 } };
...
char* chkidend(char* start, char* end)
{
	if ((end - start) > 2 && *(end - 2) == '.') {
		...
		if (c == 'b' || c == 'w' || c == 'l')
			return end -
			       2; /* .b/.w/.l extension is not part of M68k-identifier */
	}
	return end;
}
#endif
...
static char* skip_macroparam(char* s)
{
	...
	while (1) {
		...
		if (ISEOL(s) ||
		    ((c == ',' || isspace((unsigned char)c)) && par_cnt == 0))
			break;
		s++;
	}
	return s;
...
static void handle_section(char* s)
{
	...
	if (*s == ',') {
					...
					if (new = parse_identifier(&s)) {
						if (!strcmp(new, "nobits")) {
							...
							if (strcmp(new,
								   "progbits"))
								syntax_error(
									14); /* invalid sectiont type ignored */
							myfree(new);
						}
					}
				} else {
		...
		if (!strcmp(name, dataname))
			attr = dataattr;
		if (!strcmp(name, sdataname))
			attr = sdataattr;
		if (!strcmp(name, sdata2name))
			attr = sdata2attr;
		if (!strcmp(name, rodataname))
			attr = rodataattr;
		if (!strcmp(name, bssname))
			attr = bssattr;
		if (!strcmp(name, sbssname))
			attr = sbssattr;
		if (!strcmp(name, tocdname))
			attr = tocdattr;
		if (!strcmp(name, dpagename))
			attr = dpageattr;
	}
...
static void handle_org(char* s)
{
	if (*s == current_pc_char) { /*  "* = * + <expr>" reserves bytes */
		...
		if (*s == '+') {
			add_atom(0,
				 new_space_atom(parse_expr_tmplab(&s), 1, 0));
		} else {
...
static void handle_data(char* s, int size, int noalign)
{
	for (;;) {
		...
		if ((OPSZ_BITS(size) == 8 || OPSZ_BITS(size) == 16) &&
		    *s == '\"') {
		...
		if (!db) {
			op = new_operand();
			s = skip_operand(s);
			if (parse_operand(opstart, s - opstart, op,
					  DATA_OPERAND(size))) {
				atom* a;
...
static void do_binding(char* s, int bind)
{
	...
	while (1) {
		...
		if (((sym->flags & (EXPORT | WEAK | LOCAL)) != 0 &&
			...
			general_error(
				62, sym->name,
				get_bind_name(sym)); /* binding already set */
		else
			sym->flags |= bind;
		s = skip(s);
		if (*s != ',')
...
static void handle_rept(char* s)
{
	...
	eol(s);
	new_repeat(cnt, NULL, NULL, nodotneeded ? rept_dirlist : drept_dirlist,
...
{
	syntax_error(12, endmname,
		     ".macro"); /* unexpected endm without macro */
}
...
static void handle_8bit(char* s)
{
	handle_data(s, 8, 0);
}
static void handle_16bit(char* s)
{
	handle_data(s, 16, 0);
}
static void handle_32bit(char* s)
{
	handle_data(s, 32, 0);
}
static void handle_64bit(char* s)
{
	handle_data(s, 64, 0);
}
static void handle_16bit_noalign(char* s)
{
	handle_data(s, 16, 1);
}
static void handle_32bit_noalign(char* s)
{
	handle_data(s, 32, 1);
}
static void handle_64bit_noalign(char* s)
{
	handle_data(s, 64, 1);
}
static void handle_single(char* s)
{
	handle_data(s, OPSZ_FLOAT | 32, 0);
}
static void handle_double(char* s)
{
	handle_data(s, OPSZ_FLOAT | 64, 0);
}
#if VASM_CPU_OIL
...
static void handle_string(char* s)
{
	handle_data(s, 8, 0);
	add_atom(0,
		 new_space_atom(number_expr(1), 1, 0)); /* terminating zero */
}
#endif
static void handle_texts(char* s)
{
	handle_section(textname);
	eol(s);
}
static void handle_datas(char* s)
{
	handle_section(dataname);
	eol(s);
}
static void handle_sdatas(char* s)
{
	handle_section(sdataname);
	eol(s);
}
static void handle_sdata2s(char* s)
{
	handle_section(sdata2name);
	eol(s);
}
static void handle_rodatas(char* s)
{
	handle_section(rodataname);
	eol(s);
}
static void handle_sbsss(char* s)
{
	handle_section(sbssname);
	eol(s);
}
static void handle_tocds(char* s)
{
	handle_section(tocdname);
	eol(s);
}
static void handle_dpages(char* s)
{
	handle_section(dpagename);
	eol(s);
}
...
struct {
	...
	"org",	       handle_org,	     "section",	   handle_section,
	"pushsection", handle_pushsection,   "popsection", handle_popsection,
	"string",      handle_string,	     "byte",	   handle_8bit,
	"ascii",       handle_8bit,	     "asciz",	   handle_string,
	"short",       handle_16bit,	     "half",	   handle_16bit,
	"word",	       handle_16bit,	     "int",	   handle_32bit,
	"long",	       handle_32bit,	     "quad",	   handle_64bit,
	"2byte",       handle_16bit_noalign, "uahalf",	   handle_16bit_noalign,
	"4byte",       handle_32bit_noalign, "uaword",	   handle_16bit_noalign,
	"ualong",      handle_32bit_noalign, "8byte",	   handle_64bit_noalign,
	"uaquad",      handle_64bit_noalign, "float",	   handle_single,
	"single",      handle_single,	     "double",	   handle_double,
	"text",	       handle_texts,	     "data",	   handle_datas,
	"bss",	       handle_bsss,	     "rodata",	   handle_rodatas,
	"sdata",       handle_sdatas,	     "sdata2",	   handle_sdata2s,
	"sbss",	       handle_sbsss,	     "tocd",	   handle_tocds,
	"dpage",       handle_dpages,	     "equ",	   handle_set,
	"set",	       handle_set,	     "equiv",	   handle_equiv,
	"global",      handle_global,	     "globl",	   handle_global,
	"extern",      handle_global,	     "weak",	   handle_weak,
	"local",       handle_local,	     "even",	   handle_even,
	"align",       handle_align,	     "balign",	   handle_balign,
	"balignw",     handle_balignw,	     "balignl",	   handle_balignl,
	"p2align",     handle_p2align,	     "p2alignw",   handle_p2alignw,
	"p2alignl",    handle_p2alignl,	     "space",	   handle_space,
	"skip",	       handle_space,	     "zero",	   handle_space,
	"comm",	       handle_comm,	     "lcomm",	   handle_lcomm,
	"size",	       handle_size,	     "type",	   handle_type,
	"file",	       handle_file,	     "stabs",	   handle_stabs,
	"stabn",       handle_stabn,	     "stabd",	   handle_stabd,
	"incdir",      handle_incdir,	     "include",	   handle_include,
	"incbin",      handle_incbin,	     "rept",	   handle_rept,
	"irp",	       handle_irp,	     "irpc",	   handle_irpc,
	"endr",	       handle_endr,	     "macro",	   handle_macro,
	"endm",	       handle_endm,	     "ifdef",	   handle_ifd,
	"ifndef",      handle_ifnd,	     "ifb",	   handle_ifb,
	"ifnb",	       handle_ifnb,	     "if",	   handle_ifne,
	"ifeq",	       handle_ifeq,	     "ifne",	   handle_ifne,
	"ifgt",	       handle_ifgt,	     "ifge",	   handle_ifge,
	"iflt",	       handle_iflt,	     "ifle",	   handle_ifle,
	"else",	       handle_else,	     "endif",	   handle_endif,
	"abort",       handle_abort,	     "err",	   handle_err,
	"fail",	       handle_fail,	     "title",	   handle_title,
	"ident",       handle_ident,	     "list",	   handle_list,
	"nolist",      handle_nolist,	     "swbeg",	   handle_swbeg,
...
void parse(void)
{
	char *s, *line, *ext[MAX_QUALIFIERS ? MAX_QUALIFIERS : 1],
		*op[MAX_OPERANDS];
	char *labname, *start;
	int inst_len, ext_len[MAX_QUALIFIERS ? MAX_QUALIFIERS : 1],
		op_len[MAX_OPERANDS];
	int ext_cnt, op_cnt;
	instruction* ip;
...
char* macro_arg_opts(macro* m, int argno, char* name, char* s)
{
	...
	if (*s == '=') {
		...
		if (end = skip_operand(s)) {
			if (req)
				syntax_error(
					13,
					name); /* pointless default value for req. parameter */
	...
	if (new) {
		if (req)
			addmacarg(&m->defaults, NULL,
				  NULL); /* mark as required - no default! */
		else
			addmacarg(&m->defaults, new,
				  new); /* empty string as default */
	}
	return new; /* NULL means: set an empty string as default value */
}

/* parse next macro argument */
char* parse_macro_arg(struct macro* m, char* s, struct namelen* param,
		      struct namelen* arg)
{
