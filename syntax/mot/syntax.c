...
static struct namelen macro_dirlist[] = { { 5, "macro" }, { 0, 0 } };
static struct namelen endm_dirlist[] = { { 4, "endm" }, { 0, 0 } };
static struct namelen rept_dirlist[] = { { 4, "rept" }, { 0, 0 } };
static struct namelen endr_dirlist[] = { { 4, "endr" }, { 0, 0 } };
static struct namelen erem_dirlist[] = { { 4, "erem" }, { 0, 0 } };
...
char* chkidend(char* start, char* end)
{
	if (dot_idchar && (end - start) > 2 && *(end - 2) == '.') {
		...
		if (c == 'b' || c == 'w' || c == 'l')
			return end -
			       2; /* .b/.w/.l extension is not part of identifier */
	}
	return end;
}
#endif
...
static symbol* new_setoffset_size(char* equname, char* symname, char** s,
				  int dir, taddr size)
{
	symbol *sym, *equsym;
	expr* new, *old;
	...
	if (!ISEOL(*s)) {
       ...
       Perform even alignment when requested. */
		...
		if (devpac_compat)
			general_error(
				9); /* Devpac requires an expression here */
	}

	/* assign expression to equ-symbol and change exp. of the offset-symbol */
	if (equname)
		equsym = new_equate(equname,
				    dir > 0 ? copy_tree(old) : copy_tree(new));
	else
		equsym = NULL;
...
static void handle_cnop(char* s)
{
	...
	if (!devpac_compat && align > 3 &&
	    (current_section == NULL ||
	     strchr(current_section->attr, 'c') != NULL))
...
{
	new_repeat((utaddr)parse_constexpr(&s), NULL, NULL, rept_dirlist,
		   endr_dirlist);
}
...
struct {
	...
	"import",
	0,
	handle_global, /* modifictation: pink-rg: handle purec syntax */
	"export",
	0,
	handle_global, /* modifictation: pink-rg: handle purec syntax */
	"weak",
...
static int offs_directive(char* s, char* name)
{
	...
	return !strnicmp(s, name, len) &&
		...
		(*d == '.' &&
		 (isspace((unsigned char)*(d + 2)) || ISEOL(d + 2))));
}
...
void parse(void)
{
	...
	while (line = read_next_line()) {
		...
		if (labname = parse_labeldef(&s, 0)) {
			...
			if (!strnicmp(s, "equ", 3) &&
			    isspace((unsigned char)*(s + 3))) {
				s = skip(s + 3);
				label = new_equate(labname,
						   parse_expr_tmplab(&s));
			} else if (!strnicmp(s, "fequ.", 5) &&
				   isspace((unsigned char)*(s + 6))) {
				s += 5;
				label = fequate(labname, &s);
			} else if (phxass_compat && !strnicmp(s, "equ.", 4) &&
				...
				if (!(labname = parse_identifier(&s)))
					ierror(0);
				new_macro(labname, macro_dirlist, endm_dirlist,
					  NULL);
				myfree(labname);
				continue;
			}
...
/* copy macro parameter #CARG to line buffer, increment or decrement CARG */
{
	...
	if (inc) {
		expr* new = make_expr(inc > 0 ? ADD : SUB,
				      copy_tree(carg->expr), number_expr(1));
	...
	if (*s++ == '\\') {
		...
		if (*s == '\\') {
			...
			if (*s == '@') {
				...
				if (id_stack_index <= 0) {
					syntax_error(
						17); /* id pull without matching push */
					return 0;
				} else
					unique_id =
						id_stack[id_stack_index - 1];
			} else
				unique_id = src->id;
			...
			if (nc < dlen) {
				...
				case '!':
					...
					if (id_stack_index >= IDSTACKSIZE) {
						syntax_error(
							16); /* id stack overflow */
						return 0;
					} else
						id_stack[id_stack_index++] =
							unique_id;
					s++;
					break;
				case '?':
			...
			nc = copy_macro_param(
				src, tolower((unsigned char)*s) - 'a' + 9, d,
				dlen);
			s++;
		}
...
/* Motorola local labels start with a '.' or end with '$': "1234$", ".1" */
{
	...
	if (p != NULL && *p == '\\' && ISIDSTART(*s) && *s != local_char &&
	    *(p - 1) != '$') {
		/* skip local part of global\local label */
		s = p + 1;
		if (p = skip_local(s)) {
			name = make_local_label(*start, (s - 1) - *start, s,
						*(p - 1) == '$' ? (p - 1) - s :
									p - s);
			*start = skip(p);
		} else
		   ...
		   p > (s + 1)) { /* identifier with at least 2 characters */
		if (*s == local_char) {
			/* .label */
...
int init_syntax()
{
	...
	refer_symbol(sym,
		     so_name); /* SO is only an additional reference to RS */
