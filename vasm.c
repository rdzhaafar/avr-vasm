	...
	if (eval_expr(p->content.roffs->offset, &space, sec, sec->pc) &&
		...
		if (space >= 0) {
			if (p->content.roffs->fillval == NULL) {
				...
				padbytes =
					make_padding(n, padding, MAXPADBYTES);

			if (space >= padbytes) {
				n = balign(
					sec->pc,
					padbytes); /* alignment is automatic */
				space -= n;
				sec->pc +=
					n; /* Important! Fix the PC for new alignment. */
				sb = new_sblock(number_expr(space / padbytes),
						padbytes, 0);
				...
				if (space > 0) {
					...
					atom* a = new_space_atom(
						number_expr(space), 1, 0);
					a->next = p->next;
					p->next = a;
				}
			} else {
				p->type = SPACE;
				p->content.sb =
					new_sblock(number_expr(space), 1, 0);
			}
		} else
...
void vasmdebug(const char* f, section* s, atom* a)
{
	if (a->next != NULL) {
		a = a->next;
		printf("%s: (%s+0x%llx) %2d:%lu(%u) ", f, s->name,
		       ULLTADDR(s->pc), a->type, (unsigned long)a->lastsize,
		       a->changes);
		print_atom(stdout, a);
		putchar('\n');
	}
...
static int resolve_section(section* sec)
{
	...
	do {
		...
		for (p = sec->first; p; p = p->next) {
				...
				if (label->pc != sec->pc) {
					if (debug)
						printf("moving label %s from %lu to %lu\n",
						       label->name,
						       (unsigned long)label->pc,
						       (unsigned long)sec->pc);
					done = 0;
					label->pc = sec->pc;
				}
			} else if (p->type == VASMDEBUG)
				vasmdebug("resolve_section", sec, p);
			if (pass > fastphase && !done &&
			    p->type == INSTRUCTION) {
			...
			if (size != p->lastsize) {
				...
				if (pass > fastphase)
					...
					extrapass =
						0; /* no extra pass, when an atom became larger */
				p->lastsize = size;
			}
			sec->pc += size;
		}
		if (rorg) {
			sec->pc = org_pc + (sec->pc - rorg_pc);
			sec->flags &=
				~ABSOLUTE; /* workaround for misssing RORGEND */
		}
...
static void assemble(void)
{
	...
	if (dwarf) {
		dinfo.version = dwarf;
		dinfo.producer = cnvstr(copyright,
					strchr(copyright, '(') - copyright - 1);
		...
		for (p = sec->first; p; p = p->next) {
			...
			if (sec->pc != basepc) {
				...
				if (aa->type == INSTRUCTION)
					general_error(
						50); /* instruction has been auto-aligned */
				else if (aa->type == DATA ||
					 aa->type == DATADEF)
					general_error(
						57); /* data has been auto-aligned */
			}
			if (p->type == RORG) {
				rorg_pc = *p->content.rorg;
				org_pc = sec->pc;
				...
				if (rorg) {
					...
					general_error(
						44); /* reloc org was not set */
			} else if (p->type == INSTRUCTION) {
				dblock* db;
				cur_listing = p->list;
				db = eval_instruction(p->content.inst, sec,
						      sec->pc);
				if (pic_check)
				...
				dblock* db;
				cur_listing = p->list;
				db = eval_data(p->content.defb->op,
					       p->content.defb->bitsize, sec,
					       sec->pc);
				if (pic_check)
					do_pic_check(db->relocs);
				cur_listing = 0;
				...
				if (ast->assert_exp != NULL) {
					eval_expr(ast->assert_exp, &val, sec,
						  sec->pc);
					if (val == 0)
						general_error(
							47, ast->expstr,
				...
				if (lasterrsrc != p->src ||
					...
					if (sec->flags & UNALLOCATED) {
						if (warn_unalloc_ini_dat)
							general_error(
								54); /* initialized data in offset section */
					} else
						general_error(
							31); /* initialized data in bss */
					lasterrsrc = p->src;
					lasterrline = p->line;
				}
			}
...
static void undef_syms(void)
{
	...
	for (sym = first_symbol; sym; sym = sym->next) {
		if (!auto_import && sym->type == IMPORT &&
		    !(sym->flags & (EXPORT | COMMON | WEAK)))
	...
	for (sym = first_symbol; sym; sym = sym->next) {
		...
		/* expressions which are based on a label are turned into a new label */
			...
			if (!eval_expr(sym->expr, &val, NULL, 0)) {
				if (find_base(sym->expr, &base, NULL, 0) ==
				    BASE_OK) {
					...
					general_error(
						53,
						sym->name); /* non-relocatable expr. in equate */
			}
		}
	}
}
...
{
	if (!strcmp(fmt, "test"))
		return init_output_test(&output_copyright, &write_object,
					&output_args);
	if (!strcmp(fmt, "elf"))
		return init_output_elf(&output_copyright, &write_object,
				       &output_args);
	if (!strcmp(fmt, "bin"))
		return init_output_bin(&output_copyright, &write_object,
				       &output_args);
	if (!strcmp(fmt, "srec"))
		return init_output_srec(&output_copyright, &write_object,
					&output_args);
	if (!strcmp(fmt, "vobj"))
		return init_output_vobj(&output_copyright, &write_object,
					&output_args);
	if (!strcmp(fmt, "hunk"))
		return init_output_hunk(&output_copyright, &write_object,
					&output_args);
	if (!strcmp(fmt, "aout"))
		return init_output_aout(&output_copyright, &write_object,
					&output_args);
	...
	if (!strcmp(fmt, "tos")) {
		exec_out = 1; /* executable format */
		return init_output_tos(&output_copyright, &write_object,
				       &output_args);
	}
	if (!strcmp(fmt, "xfile")) {
		exec_out = 1; /* executable format */
		return init_output_xfile(&output_copyright, &write_object,
					 &output_args);
	}
	if (!strcmp(fmt, "cdef"))
		return init_output_cdef(&output_copyright, &write_object,
					&output_args);
	return 0;
}
...
int main(int argc, char** argv)
{
	...
	if (verbose)
		printf("%s\n%s\n%s\n%s\n", copyright, cpu_copyright,
		       syntax_copyright, output_copyright);
	for (i = 1; i < argc; i++) {
		if (argv[i][0] == 0)
			continue;
		if (argv[i][0] != '-') {
			...
			if (def) {
				...
				if (ISIDSTART(*s)) {
					...
					if (*s)
						general_error(
							23,
							'D'); /* trailing garbage after option */
					new_abs(def, val);
					myfree(def);
					continue;
				}
		...
		if (!strncmp("-depend=", argv[i], 8) ||
		    !strncmp("-dependall=", argv[i], 11)) {
			depend_all = argv[i][7] != '=';
			if (!strcmp("list", &argv[i][depend_all ? 11 : 8])) {
				depend = DEPEND_LIST;
				continue;
			} else if (!strcmp("make",
					   &argv[i][depend_all ? 11 : 8])) {
				depend = DEPEND_MAKE;
				continue;
			}
		}
	...
	nostdout =
		depend &&
		dep_filename == NULL; /* dependencies to stdout nothing else */
	include_main_source();
	...
	if (errors == 0) {
		if (depend && dep_filename == NULL) {
			...
			if (!outfile)
				...
				write_object(outfile, first_section,
					     first_symbol);
		}
	}
	leave();
	...
	if (nptr != NULL) {
		...
		if (f = locate_file(filename, "r", &ipath)) {
			...
			if (feof(f)) {
				...
				*nptr = srcfile;
				cur_src = newsrc = new_source(filename, srcfile,
							      text, size);
			} else
		...
		cur_src = newsrc = new_source(srcfile->name, srcfile,
					      srcfile->text, srcfile->size);
	}
	return newsrc;
}
...
source* new_source(char* srcname, struct source_file* srcfile, char* text,
		   size_t size)
{
...
void set_section(section* s)
{
	...
	if (current_section != NULL &&
	    !(current_section->flags & UNALLOCATED)) {
	...
	if (s != NULL && !(s->flags & UNALLOCATED))
		cpu_opts_init(
			s); /* set initial cpu opts before the first atom */
#endif
	current_section = s;
}
...
void start_rorg(taddr rorg)
{
	...
	if (!(s->flags & ABSOLUTE)) {
		s->flags &= ~PREVABS;
		s->flags |=
			ABSOLUTE; /* make section absolute during the ROrg-block */
	} else
		s->flags |= PREVABS;
}
...
void set_list_title(char* p, int len)
{
	...
	listtitlelines = myrealloc(listtitlelines,
				   listtitlecnt * sizeof(*listtitlelines));
...
void write_listing(char* listname)
{
	...
	for (p = first_listing; p; p = p->next) {
		...
		fprintf(f, "%4d ", p->line);
		a = p->atom;
		while (a && a->type != DATA && a->next &&
		       a->next->line == a->line && a->next->src == a->src)
			a = a->next;
		if (a && a->type == DATA) {
			int size = a->content.db->size;
			char* dp = a->content.db->data;
			pc = p->pc;
			fprintf(f, "%05lX %d ", (unsigned long)pc,
				(int)(p->sec ? p->sec->idx : 0));
			for (i = 0; i < 8; i++) {
				if (i == 4)
					fprintf(f, " ");
				if (i < size) {
				...
				if (i == size - 1 && i < 7 && a->next &&
				    a->next->align <= a->align &&
				    a->next->type == DATA &&
				    a->next->line == a->line &&
				    a->next->src == a->src) {
					a = a->next;
			...
			if (a->content.db->relocs) {
				symbol* s =
					((nreloc*)(a->content.db->relocs->reloc))
						->sym;
				if (s->type == IMPORT)
					rel = 'X';
				else
		...
		while (a) {
			...
			if (a->next && a->next->line == a->line &&
			    a->next->src == a->src) {
				a = a->next;
				pc = pcalign(a, pc);
				if (a->type == DATA && a->content.db->relocs) {
					symbol* s =
						((nreloc*)(a->content.db->relocs
								   ->reloc))
							->sym;
					if (s->type == IMPORT)
						rel = 'X';
...
void write_listing(char* listname)
{
	...
	for (p = first_listing; p; p = p->next) {
		...
		if (p->src && p->src->id > maxsrc)
			maxsrc = p->src->id;
		fprintf(f, "F%02d:%04d %s %s", (int)(p->src ? p->src->id : 0),
			p->line, err, p->txt);
		a = p->atom;
		pc = p->pc;
		while (a) {
