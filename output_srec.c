...
static char* default_start =
	"start"; /* name of default execution address symbol
 ...
 */
{
	...
	if (data_size == 0 &&
	    type != 0) /* allow S0 record to have data size of 0 */
	...
	if (type == 3) {
		write_hex_byte(
			f,
			data_size +
				5); /* count: 4 bytes for address + checksum */
		...
		write_hex_byte(
			f,
			data_size +
				4); /* count: 3 bytes for address + checksum */
	...
	else if (type <= 1) /* type is uint, so no need to check if <0 */
	{
		write_hex_byte(
			f,
			data_size +
				3); /* count: 2 bytes for address + checksum */
 ...
 * mode */
{
	...
	fw8(f, 'S');
	fw8(f, (10 - srecfmt) +
		       '0'); /* writes S header depending on S19/S28/S37 */
...
/* modified from fwpcalign() in supp.c */
{
	...
	if (a->type == SPACE &&
	    a->content.sb->space == 0) { /* space align atom */
		if (a->content.sb->maxalignbytes != 0 &&
		    n > a->content.sb->maxalignbytes)
			return;
...
static void write_output(FILE* f, section* sec, symbol* sym)
{
	...
	for (; sym; sym = sym->next) {
		...
		if (start_sym != NULL &&
		    !strcmp(sym->name, start_sym)) /* look for start symbol */
		{
			start_addr = sym->pc;
			start_sym =
				NULL; /* we use this to indicate that we found the symbol */
		}
	}

	if (start_sym !=
	    NULL) /* if start_name is not NULL, then it means we set it
	...
	{
		for (data_size = 0;
		     (*(s->name + data_size) != '\0') && (data_size < 32);
		     data_size++)
		...
		for (p = s->first; p; p = p->next) /* iterate through atoms */
		{
			...
			if (p->type == DATA)
				for (i = 0; i < p->content.db->size; i++)
					put_byte_in_buffer(
						f,
						(uint8_t)p->content.db->data[i]);
			else if (p->type == SPACE) {
				for (i = 0; i < p->content.sb->space; i++) {
					for (j = 0; j < p->content.sb->size;
					     j++) {
						put_byte_in_buffer(
							f,
							p->content.sb->fill[j]);
		...
		write_data_buffer(
			f,
			srecfmt); /* now that we're done iterating through atoms */
...
int init_output_srec(char** cp, void (**wo)(FILE*, section*, symbol*),
		     int (**oa)(char*))
{
	*cp = copyright;
	*wo = write_output;
