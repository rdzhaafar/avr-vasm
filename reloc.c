   ...
   byteoffset and bitoffset from offset. */
{
	...
	if (rl = add_extnreloc(relocs, sym, addend, type, bitoffs, size,
			       byteoffs)) {
		r = rl->reloc;
		r->mask = mask;
	}
...
void print_reloc(FILE* f, int type, nreloc* p)
{
	if (type <= LAST_STANDARD_RELOC) {
		static const char* rname[] = { "none",	  "abs",     "pc",
					       "got",	  "gotrel",  "gotoff",
					       "globdat", "plt",     "pltrel",
					       "pltoff",  "sd",	     "uabs",
					       "localpc", "loadrel", "copy",
					       "jmpslot", "secoff" };
		...
		static const char* rname[] = { "sd2", "sd21", "sdi16", "drel",
					       "brel" };
