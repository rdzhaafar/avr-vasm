...
int bytespertaddr = 2;
char* cpu_copyright =
	"vasm 6800/6801/68hc11 cpu backend 0.4 (c) 2013-2016,2019 Esben Norby";
char* cpuname = "6800";
...
static size_t eval_oper(operand* op, section* sec, taddr pc, taddr offs,
			dblock* db)
{
	size_t size = 0;
	symbol* base = NULL;
	...
	if (size > 0 && db != NULL) {
		...
		if (op->type == REL && base == NULL) {
			...
			if (!is_pc_reloc(base, sec)) {
				...
				add_extnreloc(&db->relocs, base, val, REL_PC, 0,
					      8, offs);
