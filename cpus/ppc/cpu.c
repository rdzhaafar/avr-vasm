...
static uint64_t cpu_type =
	CPU_TYPE_PPC | CPU_TYPE_ALTIVEC | CPU_TYPE_32 | CPU_TYPE_ANY;
...
{
	if (n & OPSZ_FLOAT)
		return OPSZ_BITS(n) > 32 ? OP_F64 : OP_F32;
	if (OPSZ_BITS(n) <= 8)
		return OP_D8;
	if (OPSZ_BITS(n) <= 16)
		return OP_D16;
...
/* Check if mnemonic is available for selected cpu_type. */
{
	...
	if ((avail & cpu_type) != 0) {
		if ((avail & cpu_type & ~datawidth) != 0 ||
		    (cpu_type & CPU_TYPE_ANY) != 0) {
			if (avail & datawidth)
				return (avail & datawidth) ==
					       (cpu_type & datawidth) ||
...
static int get_reloc_type(operand* op)
{
	...
	if (OP_DATA(op->type)) { /* data relocs */
		...
		const struct powerpc_operand* ppcop =
			&powerpc_operands[op->type];

		if (ppcop->shift == 0) {
			if (ppcop->bits == 16 || ppcop->bits == 26) {
				if (ppcop->flags &
				    OPER_RELATIVE) { /* a relative branch */
					switch (op->attr) {
					case REL_NONE:
						rtype = REL_PC;
						break;
					...
					default:
						cpu_error(
							11); /* reloc attribute not supported by operand */
						break;
					}
				}

				else if (ppcop->flags &
					 OPER_ABSOLUTE) { /* absolute branch */
					switch (op->attr) {
					case REL_NONE:
						rtype = REL_ABS;
						break;
...
static taddr make_reloc(int reloctype, operand* op, section* sec, taddr pc,
			rlist** reloclist)
...
/* checks if a value fits the allowed range for this operand field */
{
	...
	if (db) {
		if (db->relocs) {
			...
			case REL_PPCEABI_SDA21:
				force_signopt =
					1; /* relocation allows full positive range */
				break;
			}
		}
	}
...
size_t eval_operands(instruction* ip, section* sec, taddr pc, uint32_t* insn,
		     dblock* db)
   ...
   return size of instruction */
{
	...
	for (i = 0; i < MAX_OPERANDS && ip->op[i] != NULL; i++) {
		...
		if ((reloctype = get_reloc_type(&op)) != REL_NONE) {
			if (db != NULL) {
				val = make_reloc(reloctype, &op, sec, pc,
						 &db->relocs);
			} else {
				if (!eval_expr(op.value, &val, sec, pc)) {
					if (reloctype == REL_PC)
			...
			if (!eval_expr(op.value, &val, sec, pc))
				if (insn != NULL)
					cpu_error(
						2); /* constant integer expression required */
		}
			...
			if (reloctype == REL_PC &&
             ...
             a "B<!cc> ; B" combination */
					if (insn != NULL) {
						...
						*insn = B(
							18, 0,
							0); /* set B instruction opcode */
						val -= 4;
					}
					ppcop = &powerpc_operands
							[LI]; /* set oper. for B instruction */
					isize = 8;
				}
			}
		}
		...
		if (ppcop->flags & OPER_PARENS) {
			if (op.basereg) {
				...
				if (db != NULL && op.mode == OPM_NONE &&
				    op.attr == REL_NONE) {
					if (eval_expr(op.basereg, &reg, sec,
						      pc)) {
						if (reg ==
						    sdreg) /* is it a small data reference? */
							fix_reloctype(db,
								      REL_SD);
						else if (reg ==
							 sd2reg) /* EABI small data 2 */
							fix_reloctype(
								db,
								REL_PPCEABI_SDA2);
					}
...
int cpu_args(char* p)
{
	...
	if (!strncmp(p, "-m", 2)) {
		...
		if (!strcmp(p, "pwrx") || !strcmp(p, "pwr2"))
			cpu_type =
				CPU_TYPE_POWER | CPU_TYPE_POWER2 | CPU_TYPE_32;
			...
			cpu_type = CPU_TYPE_PPC | CPU_TYPE_7450 | CPU_TYPE_32 |
				   CPU_TYPE_ALTIVEC;
		else if (!strncmp(p, "74", 2) || !strcmp(p, "avec") ||
			 !strcmp(p, "altivec"))
			cpu_type =
				CPU_TYPE_PPC | CPU_TYPE_32 | CPU_TYPE_ALTIVEC;
		else if (!strcmp(p, "403"))
			cpu_type = CPU_TYPE_PPC | CPU_TYPE_403 | CPU_TYPE_32;
		else if (!strcmp(p, "405"))
			cpu_type = CPU_TYPE_PPC | CPU_TYPE_403 | CPU_TYPE_405 |
				   CPU_TYPE_32;
		else if (!strncmp(p, "44", 2) || !strncmp(p, "46", 2))
			cpu_type = CPU_TYPE_PPC | CPU_TYPE_440 |
				   CPU_TYPE_BOOKE | CPU_TYPE_ISEL |
				   CPU_TYPE_RFMCI | CPU_TYPE_32;
		else if (!strcmp(p, "821") || !strcmp(p, "850") ||
			 !strcmp(p, "860"))
			...
			cpu_type = CPU_TYPE_PPC | CPU_TYPE_E500 |
				   CPU_TYPE_BOOKE | CPU_TYPE_ISEL |
				   CPU_TYPE_SPE | CPU_TYPE_EFS | CPU_TYPE_PMR |
				   CPU_TYPE_RFMCI | CPU_TYPE_32;
