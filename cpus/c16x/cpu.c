...
char* cpu_copyright =
	"vasm c16x/st10 cpu backend 0.2c (c) in 2002-2005 Volker Barthelmann";
char* cpuname = "c16x";
...
int parse_operand(char* p, int len, operand* op, int requires)
{
	...
	if ((p[0] == 'r' || p[0] == 'R') && p[1] >= '0' && p[1] <= '9' &&
	...
	} else if ((p[0] == 'r' || p[0] == 'R') && p[1] == '1' && p[2] >= '0' &&
		   p[2] <= '5' && (len == 3 || p[3] == '.')) {
		op->type = OP_GPR;
	...
	} else if (len == 3 && (p[0] == 'r' || p[0] == 'R') &&
		   (p[1] == 'l' || p[1] == 'L') && p[2] >= '0' && p[2] <= '7') {
		op->type = OP_BGPR;
		op->reg = (p[2] - '0') * 2;
		op->regsfr = op->reg + 0xf0;
	} else if (len == 3 && (p[0] == 'r' || p[0] == 'R') &&
		   (p[1] == 'h' || p[1] == 'H') && p[2] >= '0' && p[2] <= '7') {
		op->type = OP_BGPR;
		op->reg = (p[2] - '0') * 2 + 1;
		op->regsfr = op->reg + 0xf0;
	} else if (p[0] == '#') {
		op->type = OP_IMM16;
		p = skip(p + 1);
		if ((!strncmp("SOF", p, 3) || !strncmp("sof", p, 3)) &&
		    isspace((unsigned char)p[3])) {
			op->mod = MOD_SOF;
			p = skip(p + 3);
		}
		if ((!strncmp("SEG", p, 3) || !strncmp("seg", p, 3)) &&
		    isspace((unsigned char)p[3])) {
			op->mod = MOD_SEG;
			p = skip(p + 3);
		}
		if ((!strncmp("DPP0:", p, 5) || !strncmp("dpp0:", p, 5))) {
			op->mod = MOD_DPP0;
			p = skip(p + 5);
		}
		if ((!strncmp("DPP1:", p, 5) || !strncmp("dpp1:", p, 5))) {
			op->mod = MOD_DPP1;
			p = skip(p + 5);
		}
		if ((!strncmp("DPP2:", p, 5) || !strncmp("dpp2:", p, 5))) {
			op->mod = MOD_DPP2;
			p = skip(p + 5);
		}
		if ((!strncmp("DPP3:", p, 5) || !strncmp("dpp3:", p, 5))) {
			op->mod = MOD_DPP3;
			p = skip(p + 5);
		}
		if ((!strncmp("DPPX:", p, 5) || !strncmp("dppx:", p, 5))) {
			op->mod = MOD_DPPX;
			p = skip(p + 5);
		}
			...
			if (find_namelen(sfrhash, name, p - name, &data)) {
				...
				if (sfr->flags & ISBIT) {
					...
					if (requires == OP_SFR ||
					    ...
					    requires == OP_BWORD) {
						op->offset =
							number_expr(sfr->saddr);
						op->type = requires;
					} else if (requires == OP_BADDR &&
						   *p == '.') {
						op->offset =
							number_expr(sfr->saddr);
						p = skip(p + 1);
						op->boffset = parse_expr(&p);
						op->type = OP_BADDR;
					} else if (requires == OP_ABS ||
						   requires == OP_BABS) {
						op->type = requires;
						op->offset = number_expr(
							(2 * sfr->saddr) +
							(sfr->laddr << 8));
					}
				}
	...
	if (requires == OP_BWORD && op->type == OP_SFR)
		return 1;
	if (op->type == OP_IMM16 &&
	    (requires >= OP_IMM2 && requires <= OP_IMM16))
		return 1;
...
static taddr absoffset2(expr* tree, int mod, section* sec, taddr pc,
			rlist** relocs, int roffset, int size, taddr mask)
{
	...
	if (mod == MOD_DPP0 || mod == MOD_DPP1 || mod == MOD_DPP2 ||
	    mod == MOD_DPP3 || mod == MOD_DPPX) {
		if (mask != 0xffffffff && mask != 0xffff)
			cpu_error(7);
		mask = 0x3fff;
	}
	if (!eval_expr(tree, &val, sec, pc)) {
		...
		if (mod == MOD_DPPX) {
			...
			if (dppsym->type == EXPRESSION) {
				...
				add_nreloc_masked(relocs, dppsym, 0, REL_ABS, 2,
						  roffset + 14, 0x3);
			}
		}
		add_nreloc_masked(relocs, base, addend, REL_ABS, size, roffset,
				  mask);
		return val;
	}
	val &= mask;
...
static taddr absoffset(expr* tree, int mod, section* sec, taddr pc,
		       rlist** relocs, int roffset, int size)
{
	/* taddr mask=size==32?0xffffffff:((((taddr)1)<<size)-1);*/
	return absoffset2(tree, mod, sec, pc, relocs, roffset, size,
			  0xffffffff);
}
...
static int translate(instruction* p, section* sec, taddr pc)
{
	...
	if (c == JMP ||
	    (!notrans && (c == JMPA || c == JMPR || c == JB || c == JNB))) {
	...
	if (mnemonics[c].operand_type[1] == OP_IMM3) {
		if (!eval_expr(p->op[1]->offset, &val, sec, pc) || val < 0 ||
		    val > 7) {
...
dblock* eval_instruction(instruction* p, section* sec, taddr pc)
{
	...
	if ((mnemonics[p->code].operand_type[0] == OP_GPR &&
	     mnemonics[c].operand_type[0] == OP_SFR) ||
	    (mnemonics[p->code].operand_type[0] == OP_BGPR &&
	     mnemonics[c].operand_type[0] == OP_BSFR))
	...
	case 6:
		code = opcode | p->op[0]->reg << 4 |
		       absval(p->op[1]->offset, sec, pc, 3);
		break;
	case 7:
		/* fall through */
	case 8:
		code = opcode << 16 |
		       absoffset(p->op[0]->offset, p->op[0]->mod, sec, pc,
				 &relocs, 20, 8)
			       << 16 |
		       absoffset(p->op[1]->offset, p->op[1]->mod, sec, pc,
				 &relocs, 16, 16);
		break;
	case 9:
		code = opcode | p->op[0]->reg |
		       absval(p->op[1]->offset, sec, pc, 4) << 4;
		break;
	case 10:
		...
		code = opcode << 16 |
		       ...
		       absoffset(p->op[0]->offset, p->op[0]->mod, sec, pc,
				 &relocs, 16, 8)
			       << 0 |
		       absoffset(p->op[1]->boffset, 0, sec, pc, &relocs, 24, 4)
			       << 12 |
		       absoffset(p->op[0]->boffset, 0, sec, pc, &relocs, 28, 4)
			       << 8;
		break;
	case 11:
		code = opcode |
		       absoffset(p->op[0]->boffset, 0, sec, pc, &relocs, 0, 4)
			       << 12 |
		       absoffset(p->op[0]->offset, p->op[0]->mod, sec, pc,
				 &relocs, 8, 8);
		break;
	case 12:
		code = opcode << 16 |
		       absoffset(p->op[0]->offset, p->op[0]->mod, sec, pc,
				 &relocs, 8, 8)
			       << 16 |
		       absoffset(p->op[2]->offset, p->op[2]->mod, sec, pc,
				 &relocs, 16, 8)
			       << 8 |
		       absoffset(p->op[1]->offset, p->op[1]->mod, sec, pc,
				 &relocs, 24, 8);
	...
	case 17:
		if (p->op[0]->type == OP_CC) {
			...
			code = opcode << 16 |
			       absoffset2(p->op[1]->offset, 0, sec, pc, &relocs,
					  8, 8, 0xffff0000)
				       << 16 |
			       absoffset2(p->op[1]->offset, 0, sec, pc, &relocs,
					  16, 16, 0xffff);
			...
			code = opcode << 16 |
			       absoffset2(p->op[0]->offset, 0, sec, pc, &relocs,
					  8, 8, 0xffff0000)
				       << 16 |
			       absoffset2(p->op[0]->offset, 0, sec, pc, &relocs,
					  16, 16, 0xffff);
	...
	case 18:
		/* fall through */
	case 19:
		code = opcode << 16 | 0xf << 20 | p->op[0]->reg << 16 |
		       absoffset(p->op[1]->offset, p->op[1]->mod, sec, pc,
				 &relocs, 16, 16);
		break;
	case 20:
	...
	case 22:
		if (!jmpconv) {
			val = ((reloffset(p->op[1]->offset, sec, pc) - 4) >>
			       1) &
			      255;
			code = opcode << 16 |
			       absoffset(p->op[0]->offset, p->op[0]->mod, sec,
					 pc, &relocs, 8, 8)
				       << 16 |
			       absoffset(p->op[0]->boffset, 0, sec, pc, &relocs,
					 24, 4)
				       << 12 |
			       val;
		} else {
	...
	case 23:
		if (!jmpconv) {
			val = ((reloffset(p->op[1]->offset, sec, pc) - 2) >>
			       1) &
			      255;
		...
		break;
	case 24:
		code = opcode << 16 | p->op[0]->reg << 20 |
		       p->op[1]->reg << 16 |
		       absoffset(p->op[1]->offset, p->op[1]->mod, sec, pc,
				 &relocs, 16, 16);
		break;
	case 25:
		code = opcode << 16 | p->op[0]->reg << 16 |
		       absoffset(p->op[1]->offset, p->op[1]->mod, sec, pc,
				 &relocs, 16, 16);
		break;
	case 26:
		code = opcode | absoffset(p->op[0]->offset, p->op[0]->mod, sec,
					  pc, &relocs, 8, 8);
		break;
	case 27:
	...
	case 29:
		...
		break;
	case 30:
		code = opcode << 16 | p->op[1]->reg << 16 |
		       absoffset(p->op[0]->offset, p->op[0]->mod, sec, pc,
				 &relocs, 16, 16);
		break;
	case 31:
		code = opcode |
		       ((absval(p->op[0]->offset, sec, pc, 2) - 1) << 4);
		break;
	case 32:
		code = opcode | p->op[0]->reg |
		       ((absval(p->op[1]->offset, sec, pc, 2) - 1) << 4);
		break;
	case 34:
		code = opcode << 16 |
		       ((absval(p->op[1]->offset, sec, pc, 2) - 1) << 20) |
		       absoffset(p->op[0]->offset, p->op[0]->mod, sec, pc,
				 &relocs, 16, 8);
		break;
	case 33:
	...
	if (jmpconv) {
		*d++ = 0xfa;
		*d++ = absoffset2(jmpaddr->offset, 0, sec, pc, &relocs,
				  8 + 8 * osize, 8, 0xffff0000);
		val = absoffset2(jmpaddr->offset, 0, sec, pc, &relocs,
				 16 + 8 * osize, 16, 0xffff);
		*d++ = val >> 8;
		*d++ = val;
	}
	db->relocs = relocs;
	...
	if (ISIDSTART(*s)) {
		...
		if (s - name == 4 && !strncmp(name, ".sfr", 4)) {
			...
			if (find_namelen(sfrhash, name, s - name, &data))
				...
				add_hashentry(sfrhash, cnvstr(name, s - name),
					      data);
				new->next = first_sfr;
				first_sfr = new;
			}
			...
			if (tree->c.val == 0xfe || tree->c.val == 0xf0) {
				...
				if (tree->c.val >= 0xfe00)
					...
					new->laddr = 0xf0;
				new->saddr =
					(tree->c.val - (new->laddr << 8)) / 2;
				if ((new->laddr << 8) + 2 * new->saddr !=
				    tree->c.val)
					ierror(0);
				free_expr(tree);
			}
