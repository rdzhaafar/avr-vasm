...
char* cpu_copyright =
	"vasm ARM cpu backend 0.4h (c) 2004,2006,2010,2011,2014-2020 Frank Wille";
char* cpuname = "ARM";
int bitsperbyte = 8;
int bytespertaddr = 4;
...
static const char* addrmode_strings[] = { "da", "ia", "db", "ib", "fa",
					  "fd", "ea", "ed", "bt", "tb",
					  "sb", "sh", "t",  "b",  "h",
					  "s",	"l",  "p",  NULL, "<none>" };
enum {
...
static const char* shift_strings[NUM_SHIFTTYPES] = { "LSL", "LSR", "ASR",
						     "ROR", "RRX", "ASL" };
...
/* parse instruction and save extension locations */
{
	...
	if (thumb_mode) { /* no qualifiers in THUMB code */
		*inst_len = s - inst;
		...
		if (len > 2) {
			if (c == 'b' && strnicmp(inst, "bic", 3) &&
			    (len == 3 || len == 4)) {
				*inst_len = len - 2;
			} else if ((c == 'u' || c == 's') &&
				   tolower((unsigned char)*(inst + 1)) == 'm' &&
				   len >= 5) {
				*inst_len = 5;
			} else
				*inst_len = 3;
...
/* Parses operands, reads expressions and assigns relocation types */
{
	...
	if (optype == DATA64_OP) {
		...
		if (THUMBOPER(optype)) { /* Thumb instruction */
			...
			if (optype == R19PO || optype == R3UD1 ||
			    optype == IMUD1 || optype == IMCP1) {
				p = skip(p);
				if (*p++ != ']')
					return PO_NOMATCH;
			}
			if (optype == R19WB || optype == R3UD1 ||
			    optype == IMUD1 || optype == IMCP1) {
				if (*p == '!') {
					p++;
					op->flags |= OFL_WBACK;
			...
			if (list >= 0) {
				...
				if (*p == '^') {
					p++;
					op->flags |=
						OFL_FORCE; /* set "load PSR / force user mode" flag */
				}
			} else
				return PO_NOMATCH;
		}
   ...
   return size of instruction */
{
	...
	for (opcnt = 0; opcnt < MAX_OPERANDS && ip->op[opcnt] != NULL;
	     opcnt++) {
		taddr val;
		symbol* base = NULL;
		int btype;
		...
		if (op.type == TPCLW || THBRANCH(op.type)) {
			...
			if ((base != NULL && btype == BASE_OK &&
				...
				if (op.type == TBRHL) {
					...
					isize +=
						2; /* we need two instructions for a 23-bit branch */
				} else if (op.type == TPCLW) {
					...
					cpu_error(
						22); /* operation not allowed on external symbols */
			} else if (insn)
				cpu_error(
					22); /* operation not allowed on external symbols */
		}
			...
			if (THREGOPER(op.type)) {
				...
				*insn |= val;
				...
				case TUIM3:
					if (val >= 0 && val <= 7) {
						*insn |= val << 6;
						...
						cpu_error(
							25, 3,
							(long)val); /* immediate offset out of range */
					break;
				case TUIM5:
				case TUI5I:
					if (val >= 0 && val <= 0x1f) {
					...
					if (val >= 0 && val <= 0x3e) {
						if ((val & 1) == 0)
							*insn |= (val & 0x3e)
							...
							cpu_error(
								26,
								2); /* offset has to be a multiple of 2 */
					} else
						cpu_error(
							25, 6,
							(long)val); /* immediate offset out of range */
					...
					if (val >= 0 && val <= 0x7c) {
						if ((val & 3) == 0)
							*insn |= (val & 0x7c)
							...
							cpu_error(
								26,
								4); /* offset has to be a multiple of 4 */
					} else
						cpu_error(
							25, 7,
							(long)val); /* immediate offset out of range */
				...
				if (base != NULL && db != NULL) {
					if (btype == BASE_OK) {
						if (op.type == TUIM5 ||
							...
							cpu_error(
								6); /* constant integer expression required */
					} else
						general_error(
				...
				if (val & 1)
					cpu_error(
						8,
						(long)val); /* branch to unaligned address */
				*insn |= (val >> 1) & 0xff;
			}
				...
				if (val < -0x800 || val > 0x7fe)
					cpu_error(
						3,
						(long)val); /* branch offset is out of range */
				if (val & 1)
					cpu_error(
						8,
						(long)val); /* branch to unaligned address */
...
static int negated_rot_immediate(uint32_t val, mnemonic* mnemo, uint32_t* insn)
...
{
	static uint32_t masks[] = { 0x000000ff, 0xc000003f, 0xf000000f,
				    0xfc000003, 0xff000000, 0x3fc00000,
				    0x0ff00000, 0x03fc0000, 0x00ff0000,
				    0x003fc000, 0x000ff000, 0x0003fc00,
				    0x0000ff00, 0x00003fc0, 0x00000ff0,
				    0x000003fc };
	uint32_t a, m;
	int i;
   ...
   return size of instruction */
{
		...
		*insn = mnemo->ext.opcode | get_condcode(ip);
		...
		if ((mnemo->ext.flags & SETPSR) != 0 && am == AM_P) {
			...
			*insn |= 0x0000f000;
			if (cpu_type & ~AA2)
				cpu_error(
					28); /* deprecated on 32-bit architectures */
		}

		if (!strcmp(mnemo->name, "ldr") ||
		    !strcmp(mnemo->name, "str")) {
			if (am == AM_T || am == AM_B || am == AM_BT ||
			    am == AM_TB) { /* std. ldr/str */
				if (am != AM_B) {
					*insn |=
						0x00200000; /* W-flag for post-indexed mode */
					*insn &=
						~0x01000000; /* force post-indexed */
				}
				if (am != AM_T)
					*insn |=
						0x00400000; /* B-flag for byte-transfer */
			} else if (am == AM_SB || am == AM_H ||
				   am == AM_SH) { /* arch.4 ldr/str */
				if (cpu_type & AA4UP) {
					...
					*insn = (*insn & 0xf1100000) |
					...
					if (am != AM_H) {
						if (*insn &
						    0x00100000) /* load */
							*insn |=
								0x40; /* signed transfer */
						else
							cpu_error(
								18,
								addrmode_strings
									[am]); /* illegal addr. mode */
					}
					if (am != AM_SB)
						*insn |=
							0x20; /* halfword-transfer */
					aa4ldst = 1;
				} else
					cpu_error(
	...
	for (opcnt = 0; opcnt < MAX_OPERANDS && ip->op[opcnt] != NULL;
		...
		if (op.type == PCL12 || op.type == PCLRT || op.type == PCLCP ||
		    op.type == BRA24) {
			...
			if ((base != NULL && btype == BASE_OK &&
				...
				case PCL12:
					if ((!aa4ldst && val < 0x1000 &&
					     val > -0x1000) ||
					    (aa4ldst && val < 0x100 &&
					     val > -0x100)) {
						op.type =
							IMUD2; /* handle as normal #+/-Imm12 */
						if (val < 0)
							val = -val;
						else
							op.flags |= OFL_UP;
						...
						if (opt_ldrpc &&
							...
							if (insn) {
								...
								*(insn + 1) =
									*insn;
								*insn &=
									0xf0000000; /* clear all except cond. */
								if (val < 0) {
									v = -val;
									*insn |=
										0x024f0a00; /* SUB */
									*(insn +
									  1) &= ~0x00800000; /* clear U-bit */
								} else {
									v = val;
									*insn |=
										0x028f0a00; /* ADD */
									*(insn +
									  1) |= 0x00800000; /* set U-bit */
								}
								if (aa4ldst)
									*insn |=
										(*(insn +
							...
							if (insn)
								cpu_error(
									4,
									val); /* PC-relative ldr/str out of range */
						}
					}
					break;
				...
				case PCLCP:
					if (val < 0x400 && val > -0x400) {
						op.type =
							IMCP2; /* handle as normal #+/-Imm10>>2 */
						if (val < 0)
							val = -val;
						else
							op.flags |= OFL_UP;
						...
						if (insn)
							cpu_error(
								4,
								val); /* PC-relative ldc/stc out of range */
					}
					break;
				...
				case PCLRT:
					...
					if (am == AM_L && val == 0) { /* ADRL */
						isize +=
							4; /* always reserve two ADD instructions */
						if (insn != NULL &&
						    db != NULL) {
							*(insn + 1) = *insn &
								      ~0xf0000;
			...
			if (base == NULL) {
				if ((rotval = rotated_immediate(val)) !=
				    ROTFAIL) {
					if (insn)
						*insn |= rotval;
				} else if (!negated_rot_immediate(val, mnemo,
								  insn)) {
					/* rotation, negation and inversion failed - try a 2nd operation */
					isize += 4;
					if (insn) {
						if (!negated_double_rot_immediate(
							    val, insn))
							cpu_error(
								7,
								(uint32_t)
									val); /* const not suitable */
					}
				}
			} else if (insn)
				cpu_error(
		...
		if (insn) {
			if (REGOPER(op.type)) {
				...
				if (op.type == R3UD1 && !(*insn & 0x01000000))
					cpu_error(
						21); /* post-indexed addressing mode expected */
				...
				if ((mnemo->ext.flags & NOPC) && val == 15)
					cpu_error(
						10); /* PC (r15) not allowed in this mode */
				if ((mnemo->ext.flags & NOPCR03) && val == 15 &&
				    REG03OPER(op.type))
					cpu_error(
						11); /* PC (r15) not allowed for offset register Rm */
				if ((mnemo->ext.flags & NOPC) && val == 15 &&
				    (op.flags & OFL_WBACK))
					cpu_error(
						12); /* PC (r15) not allowed with write-back */

				/* check for illegal double register specifications */
				if (((mnemo->ext.flags & DIFR03) &&
				     REG03OPER(op.type)) ||
				    ((mnemo->ext.flags & DIFR11) &&
				     REG11OPER(op.type)) ||
				    ((mnemo->ext.flags & DIFR15) &&
				     REG15OPER(op.type)) ||
				    ((mnemo->ext.flags & DIFR19) &&
				     REG19OPER(op.type))) {
					if (chkreg != -1) {
						if (val == chkreg)
							cpu_error(
								13,
								(long)val); /* register was used multiple times */
					} else
						chkreg = val;
				}
			}
				...
				*insn |= (val >> 2) & 0xffffff;
				...
				if (op.type == IMUD1 && !(*insn & 0x01000000))
					cpu_error(
						21); /* post-indexed addressing mode exptected */
				if (op.flags & OFL_WBACK)
					*insn |=
						0x00200000; /* set write-back flag */
				if (op.flags & OFL_UP)
				...
				if (val >= 0 && val <= 0x3ff) {
					if ((val & 3) == 0)
						*insn |= val >> 2;
						...
						cpu_error(
							23); /* ldc/stc offset has to be a multiple of 4 */
				} else
					cpu_error(
						20, 10,
						(long)val); /* immediate offset out of range */
				...
				if (val >= 0 && val < 0x1000000) {
					*insn |= val;
					...
					cpu_error(
						16); /* 24-bit unsigned immediate expected */
				if (base)
					cpu_error(
						6); /* constant integer expression required */
			}
				...
				if (val >= 0 && val <= 30 && (val & 1) == 0)
					*insn |= val << 7;
					...
					cpu_error(
						29,
						(long)val); /* must be even number between 0 and 30 */
				if (base)
					cpu_error(
						6); /* constant integer expression required */
			}
				...
				if (val >= 0 && val < 0x100 && base == NULL)
					*insn |= val;
					...
					cpu_error(
						30, 8,
						(long)val); /* 8-bit unsigned constant required */
			}
				...
				if (aa4ldst)
					cpu_error(
						19); /* signed/halfword ldr/str doesn't support shifts */
				...
				if (op.flags & OFL_IMMEDSHIFT) {
					if (sh_op == 1 ||
					    sh_op ==
						    2) { /* lsr/asr permit shift-count #32 */
						if (val == 32)
							val = 0;
					}
					if (base == NULL && val >= 0 &&
						...
						*insn |= (val << 7) |
						...
						cpu_error(
							14,
							(long)val); /* illegal immediate shift count */
					...
					case CPOP3:
						if (val >= 0 && val < 8)
							*insn |= val << 21;
							...
							cpu_error(
								24,
								val); /* illegal coprocessor operation */
						break;
					case CPOP4:
						if (val >= 0 && val < 16)
							*insn |= val << 20;
   ...
   if necessary. */
{
	...
	inst_type =
		(mnemonics[ip->code].ext.flags & THUMB) ? TYPE_THUMB : TYPE_ARM;
...
int cpu_args(char* p)
{
	if (!strncmp(p, "-m", 2)) {
		...
		if (!strcmp(p, "2"))
			cpu_type = ARM2;
		else if (!strcmp(p, "250"))
			cpu_type = ARM250;
		else if (!strcmp(p, "3"))
			cpu_type = ARM3;
		else if (!strcmp(p, "6"))
			cpu_type = ARM6;
		else if (!strcmp(p, "600"))
			cpu_type = ARM600;
		else if (!strcmp(p, "610"))
			cpu_type = ARM610;
		else if (!strcmp(p, "7"))
			cpu_type = ARM7;
		else if (!strcmp(p, "710"))
			cpu_type = ARM710;
		else if (!strcmp(p, "7500"))
			cpu_type = ARM7500;
		else if (!strcmp(p, "7d"))
			cpu_type = ARM7d;
		else if (!strcmp(p, "7di"))
			cpu_type = ARM7di;
		else if (!strcmp(p, "7dm"))
			cpu_type = ARM7dm;
		else if (!strcmp(p, "7dmi"))
			cpu_type = ARM7dmi;
		else if (!strcmp(p, "7tdmi"))
			cpu_type = ARM7tdmi;
		else if (!strcmp(p, "8"))
			cpu_type = ARM8;
		else if (!strcmp(p, "810"))
			cpu_type = ARM810;
		else if (!strcmp(p, "9"))
			cpu_type = ARM9;
		else if (!strcmp(p, "920"))
			cpu_type = ARM920;
		else if (!strcmp(p, "920t"))
			cpu_type = ARM920t;
		else if (!strcmp(p, "9tdmi"))
			cpu_type = ARM9tdmi;
		else if (!strcmp(p, "sa1"))
			cpu_type = SA1;
		else if (!strcmp(p, "strongarm"))
			cpu_type = STRONGARM;
		else if (!strcmp(p, "strongarm110"))
			cpu_type = STRONGARM110;
		else if (!strcmp(p, "strongarm1100"))
			cpu_type = STRONGARM1100;
		else
			return 0;
