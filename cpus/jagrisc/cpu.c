...
char* cpu_copyright =
	"vasm Jaguar RISC cpu backend 0.4d (c) 2014-2017,2020 Frank Wille";
...
/* condition codes */
static regsym cc_regsyms[] = { { "t", RTYPE_CC, 0, 0x00 },
			       { "a", RTYPE_CC, 0, 0x00 },
			       { "ne", RTYPE_CC, 0, 0x01 },
			       { "eq", RTYPE_CC, 0, 0x02 },
			       { "cc", RTYPE_CC, 0, 0x04 },
			...
			*start = skip_line(s);
			...
			if ((ccexp = parse_cc(&s)) != NULL) {
				if (eval_expr(ccexp, &val, NULL, 0))
					new_regsym(0, 0, labname, RTYPE_CC, 0,
						   (int)val);
				else
					general_error(
						30); /* expression must be a constant */
			} else
				general_error(9); /* @@@ */
			eol(s);
			*start = skip_line(s);
...
int parse_operand(char* p, int len, operand* op, int required)
{
	...
	case DATAI_OP:
		if (required == IMM1S) {
			op->val =
				make_expr(SUB, number_expr(32), parse_expr(&p));
	...
	case IR15D: /* (R15+d) */
		if (*p++ != '(')
			return PO_NOMATCH;
		reg = parse_reg(&p);
		if ((required == IR14D && reg != 14) ||
		    (required == IR15D && reg != 15))
			return PO_NOMATCH;
		if (*p++ != '+')
			return PO_NOMATCH;
		p = skip(p);
	...
	case IR15R: /* (R15+Rn) */
		if (*p++ != '(')
			return PO_NOMATCH;
		reg = parse_reg(&p);
		if ((required == IR14R && reg != 14) ||
		    (required == IR15R && reg != 15))
			return PO_NOMATCH;
		if (*p++ != '+')
			return PO_NOMATCH;
		op->reg = parse_reg(&p);
...
static int32_t eval_oper(instruction* ip, operand* op, section* sec, taddr pc,
			 dblock* db)
{
	symbol* base = NULL;
	int optype = op->type;
	...
	case CC:
		...
		if (optype == IMM0 || optype == CC || optype == IMM1 ||
		    optype == SIMM) {
			if (base != NULL) {
				loval = -32;
				hival = 32;
			...
			if ((base != NULL && btype == BASE_OK &&
				...
				add_extnreloc_masked(&db->relocs, base, val - 2,
						     REL_PC,
						     jag_big_endian ? 6 : 5, 5,
						     0, 0x3e);
			}
			base = NULL;
		} else
		...
		if (base != NULL)
			general_error(
				38); /* bad or unhandled reloc: illegal relocation */
