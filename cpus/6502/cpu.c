...
char* cpu_copyright =
	"vasm 6502 cpu backend 0.9 (c) 2002,2006,2008-2012,2014-2020 Frank Wille";
char* cpuname = "6502";
int bitsperbyte = 8;
int bytespertaddr = 2;
		...
		if ((op = ip->op[i]) != NULL) {
			if (op->value != NULL) {
				if (eval_expr(op->value, &val, sec, pc))
					...
					find_base(op->value, &base, sec,
						  pc); /* get base-symbol */

				if ((op->type == ABS || op->type == ABSX ||
				     op->type == ABSY) &&
				    ((base == NULL &&
				      ((val >= 0 && val <= 0xff) ||
				       ((utaddr)val >= op->dp &&
					(utaddr)val <= op->dp + 0xff))) ||
				     (base != NULL &&
				      (base->flags & ZPAGESYM))) &&
					...
					if ((bd < -0x80 || bd > 0x7f) &&
					    branchopt) {
						if (mnemo->ext.opcode == 0x80 ||
						    mnemo->ext.opcode == 0x12) {
...
dblock* eval_instruction(instruction* ip, section* sec, taddr pc)
{
	...
	optimize_instruction(ip, sec, pc,
			     1); /* really execute optimizations now */
			...
			if (op->value != NULL) {
				...
				case ABSY:
					if (!*(db->data)) /* STX/STY allow only ZeroPage addressing mode */
						cpu_error(
							5,
							8); /* operand doesn't fit into 8 bits */
				case ABS:
				case INDIR:
				case INDIRX:
					*d++ = val & 0xff;
	...
	for (i = 0; i < mnemonic_cnt; i++) {
		if (mnemonics[i].ext.available & cpu_type) {
			if (!strcmp(mnemonics[i].name, "jmp") &&
			    mnemonics[i].operand_type[0] == ABS)
				OC_JMPABS = i;
