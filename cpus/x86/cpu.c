...
enum codetype { CODE_32BIT, CODE_16BIT, CODE_64BIT };

static enum codetype mode_flag = CODE_32BIT; /* 32 bit is default */

/* operand types for printing */
static const char* operand_type_str[] = {
	"Reg8",	    "Reg16",	 "Reg32",	"Reg64",     "Imm8",
	"Imm8S",    "Imm16",	 "Imm32",	"Imm32S",    "Imm64",
	"Imm1",	    "Disp8",	 "Disp16",	"Disp32",    "Disp32S",
	"Disp64",   "BaseIndex", "ShiftCntReg", "IOPortReg", "CtrlReg",
	"DebugReg", "TestReg",	 "Acc",		"SegReg2",   "SegReg3",
	"MMXReg",   "XMMReg",	 "FloatReg",	"FloatAcc",  "EsSeg",
	"JmpAbs",   "InvMem"
};
...
{
	if (n & OPSZ_FLOAT)
		return OPSZ_BITS(n) > 32 ? Float64 : Float32;
	if (OPSZ_BITS(n) <= 8)
		return Data8;
	if (OPSZ_BITS(n) <= 16)
		return Data16;
...
static int add_seg_prefix(instruction* ip, int segrn)
{
	switch (segrn) {
	case ESEG_REGNUM:
		return add_ip_prefix(ip, OC_ESEG_PREFIX);
	case CSEG_REGNUM:
		return add_ip_prefix(ip, OC_CSEG_PREFIX);
	case SSEG_REGNUM:
		return add_ip_prefix(ip, OC_SSEG_PREFIX);
	case DSEG_REGNUM:
		return add_ip_prefix(ip, OC_DSEG_PREFIX);
	case FSEG_REGNUM:
		return add_ip_prefix(ip, OC_FSEG_PREFIX);
	case GSEG_REGNUM:
		return add_ip_prefix(ip, OC_GSEG_PREFIX);
	default:
		ierror(0);
	}
	return 0;
...
static void chk_byte_reg(instruction* ip)
{
	...
	for (i = 0; i < MAX_OPERANDS; i++) {
		...
		if (ot & (AnyReg | FloatReg | FloatAcc)) {
			...
			cpu_error(10, ip->op[i]->basereg->reg_name,
				  ip->qualifiers[0][0]);
			suffix_from_reg(ip);
			break;
		}
...
static void chk_word_reg(instruction* ip)
{
	...
	for (i = 0; i < MAX_OPERANDS; i++) {
		...
		if (ot & (AnyReg | FloatReg | FloatAcc)) {
			...
			if ((ot & Reg8) &&
			    (mnemo->operand_type[i] & (Reg16 | Reg32 | Acc))) {
				cpu_error(10, ip->op[i]->basereg->reg_name,
					  ip->qualifiers[0][0]);
				suffix_from_reg(ip);
				break;
				...
				/* warning: using register x instead of y due to suffix */
				cpu_error(9, ip->op[i]->basereg->reg_name,
					  oldreg->reg_name,
					  ip->qualifiers[0][0]);
				ip->op[i]->type &= ~Reg;
				ip->op[i]->type |= Reg16;
...
static int assign_suffix(instruction* ip)
{
	...
	if (suffix == '\0' &&
	    (mnemo->ext.opcode_modifier &
	     (b_Illegal | w_Illegal | l_Illegal | s_Illegal | x_Illegal |
	      q_Illegal)) == (b_Illegal | w_Illegal | l_Illegal | s_Illegal |
			      x_Illegal | q_Illegal)) {
	...
	if (mnemo->ext.opcode_modifier & (Size16 | Size32 | Size64)) {
		...
		/* check if register operands match the given suffix */
		   ...
		   (mnemo->ext.opcode_modifier & DefaultSize)) {
		/* not needed for i386 */
	}
...
static int process_suffix(instruction* ip, int final)
{
	...
	if (suffix != 'b') {
		...
		if (suffix != 'q' &&
		    !(mnemo->ext.opcode_modifier & IgnoreSize)) {
			if ((suffix == 'w' && mode_flag == CODE_32BIT) ||
				...
				if (!add_ip_prefix(ip, code)) {
					if (!final)
						cpu_error(
							2); /* same type of prefix used twice */
					return 0;
				}
			}
		}
...
/* apply suffix to immediate operand */
{
	...
	if ((ot & (Imm8 | Imm8S | Imm16 | Imm32 | Imm32S)) && ot != Imm8 &&
	    ot != Imm8S && ot != Imm16 && ot != Imm32 && ot != Imm32S &&
	    ot != Imm64) {
		switch (suffix) {
		case 'b':
			ot &= Imm8 | Imm8S;
		...
		if (ot != Imm8 && ot != Imm8S && ot != Imm16 && ot != Imm32 &&
		    ot != Imm32S && ot != Imm64) {
			if (final)
				cpu_error(
					24); /* can't determine imm. op. size w/o suffix */
			return 0;
		}
	}
	return ot;
...
static void optimize_imm(instruction* ip, operand* op, section* sec, taddr pc,
			 int final)
/* determine smallest type which can hold the immediate mode */
{
	...
	if (nt)
		...
		cpu_error(
			23); /* instruction doesn't support these operand sizes */
}

static void optimize_jump(instruction* ip, operand* op, section* sec, taddr pc,
			  int final)
{
	mnemonic* mnemo = &mnemonics[ip->code];
	...
	if (mod & JmpByte) {
		...
		if (final && label_in_sec) {
			...
			if (diff < -0x80 || diff > 0x7f)
				cpu_error(
					22,
					diff); /* jump destination out of range */
		}
		...
		if (label_in_sec && ip->ext.last_size >= 0) {
			...
			diff = val - (pc + ip->ext.last_size);
			if (diff >= -0x80 && diff <= 0x7f &&
			    (ip->ext.flags & OPTFAILED) != OPTFAILED) {
				op->type = Disp8;
				return;
			} else if (final) {
				if (mode_flag == CODE_16BIT) {
	...
	if (!eval_expr(op->value, &val, sec, pc)) {
		...
		if (mode_flag == CODE_16BIT) {
			...
			if (final && !(op->type & Disp16))
				cpu_error(
					21,
					16); /* need at least n bits for a relocatable symbol */
		} else { /* CODE_32BIT */
			op->type &= ~(Disp8 | Disp16);
			if (final && !(op->type & (Disp32 | Disp32S | Disp64)))
				cpu_error(
					21,
					32); /* need at least n bits for a relocatable symbol */
		}
	} else {
		...
		if (mode_flag == CODE_16BIT) {
			...
			op->type &=
				~Disp16; /* no 16bit displacements possible */
			if (val >= -0x80000000LL && val <= 0xffffffffLL &&
			    (op->type & Disp32)) {
				op->type &= ~Disp64;
...
static int make_modrm(instruction* ip, mnemonic* mnemo, int final)
{
	...
	if (reg_operands == 2) {
		i = (ip->op[0]->type & AnyReg) ? 0 :
						       1; /* first register operand */
		ip->ext.rm.mode = 3;
		...
		if (mem_operands) {
			operand* memop =
				ip->op[(ip->op[0]->type & AnyMem) ?
						     0 :
						     ((ip->op[1]->type & AnyMem) ? 1 :
										   2)];

			if (final && (cpudebug & 8)) {
				if (memop->basereg) {
					printf("\tbase=%%%s(",
					       memop->basereg->reg_name);
					print_operand_type(
						memop->basereg->reg_type, '|');
					putchar(')');
				} else
					printf("\tbase=none");
				if (memop->indexreg) {
					printf(" index=%%%s(",
					       memop->indexreg->reg_name);
					print_operand_type(
						memop->indexreg->reg_type, '|');
					putchar(')');
				} else
					printf(" index=none");
				printf(" scale=%d\n",
			...
			if (memop->basereg == NULL) {
				...
				default: /* (%si) or (%di) */
					ip->ext.rm.regmem =
						memop->basereg->reg_num - 2;
					break;
				}
				ip->ext.rm.mode =
					mode_from_disp_size(memop->type);
			}
				...
				if (memop->basereg->reg_num == EBP_REGNUM) {
					...
					if (memop->value == NULL) {
						memop->type |=
							Disp8; /* fake 8-bit zero-displacement */
						if (final)
							memop->value =
								number_expr(0);
					}
				} else if (memop->basereg->reg_num ==
					   ESP_REGNUM) {
					defaultseg = SSEG_REGNUM;
				}
				ip->ext.sib.scale = memop->log2_scale;
		...
		if (reg_operands) {
			operand* regop =
				ip->op[(ip->op[0]->type & AnyReg) ?
						     0 :
						     ((ip->op[1]->type & AnyReg) ? 1 :
										   2)];

			if (final && (cpudebug & 8)) {
				printf("\tregister=%%%s(",
				       regop->basereg->reg_name);
	...
	if (final && (cpudebug & 8)) {
		printf("\tMod/RM: mode=%d reg=%d regmem=%d\n",
		       (int)ip->ext.rm.mode, (int)ip->ext.rm.reg,
		       (int)ip->ext.rm.regmem);
		if (ip->ext.flags & SIB_BYTE) {
			printf("\tSIB:    scale=%d index=%d base=%d\n",
			       (int)ip->ext.sib.scale, (int)ip->ext.sib.index,
			       (int)ip->ext.sib.base);
		}
	}
	return defaultseg;
	...
	if ((mod & ShortForm) && ip->op[0]) {
		...
		if ((mod & Deprecated) && final) {
			if (ip->op[1]) {
				...
				cpu_error(16, mnemo->name,
					  ip->op[1]->basereg->reg_name,
					  ip->op[0]->basereg->reg_name);
			} else {
				/* translating to fxxxx %reg */
				cpu_error(17, mnemo->name,
					  ip->op[0]->basereg->reg_name);
			}
		}
	}
		...
		if (mnemo->ext.base_opcode == OC_POP_SREG2 &&
		    ip->op[0]->basereg && final) {
			if (ip->op[0]->basereg->reg_num == CSEG_REGNUM) {
				cpu_error(
					15,
					ip->op[0]
						->basereg
						->reg_name); /* cannot pop %cs */
				return 0;
			}
		}
...
static size_t finalize_instruction(instruction* ip, section* sec, taddr pc,
				   int final)
   ...
   modrm- and sib-bytes, and return its size in bytes */
{
	...
	for (i = 0; i < MAX_OPERANDS; i++) {
		if (ip->op[i]) {
			...
			if (ip->op[i]->type & Imm) {
				...
				if (mnemo->ext.opcode_modifier &
				    (Jmp | JmpByte | JmpDword)) {
					/* check jump-distances and try to use 8-bit form */
					optimize_jump(ip, ip->op[i], sec, pc,
						      final);
				} else {
					/* use the smallest type for absolute displacements */
					optimize_disp(ip->op[i], sec, pc,
						      final);
				}
			}
			...
			if (final && ip->op[i]->scalefactor != NULL) {
				...
				if (!eval_expr(ip->op[i]->scalefactor, &sf, sec,
					       pc)) {
					cpu_error(
						18); /* absolute scale factor required */
	...
	if (cpudebug & 16)
		printf("%08lx: (%u) %s", (unsigned long)pc, (unsigned)size,
		       mnemo->name);
...
static unsigned char* output_opcodes(unsigned char* d, instruction* ip)
{
	...
	*d++ = oc & 0xff;
	...
	if (ip->ext.flags & MODRM_BYTE)
		*d++ = make_byte233(ip->ext.rm.mode, ip->ext.rm.reg,
				    ip->ext.rm.regmem);

	if (ip->ext.flags & SIB_BYTE)
		*d++ = make_byte233(ip->ext.sib.scale, ip->ext.sib.index,
				    ip->ext.sib.base);

	return d;
}

static unsigned char* output_disp(dblock* db, unsigned char* d, instruction* ip,
		...
		if (op = ip->op[i]) {
			if (op->type & Disp) {
				...
				if (!eval_expr(op->value, &val, sec, pc)) {
					...
					if (find_base(op->value, &base, sec,
						      pc) == BASE_OK) {
						if ((mnemo->ext.opcode_modifier &
						     (Jmp | JmpByte |
						      JmpDword)) ||
...
static unsigned char* output_imm(dblock* db, unsigned char* d, instruction* ip,
				 section* sec, taddr pc)
{
	int i, ot;
	size_t bits;
...
/* parse a register name, return pointer to regsym when successful */
{
	...
	if (*p++ == '%') {
		...
		if (ISIDSTART(*p)) {
			...
			if (p - start == 2 && (!strncmp(start, "st(", 3) ||
					       !strncmp(start, "ST(", 3))) {
  ...
  given_type = op->parsed_type; */
	...
	if (given_type == 0) {
		...
		if (*p == '%') {
			...
			p = skip(p);
			if (sreg != NULL &&
			    (sreg->reg_type & (SegReg2 | SegReg3)) &&
			    *p++ == ':') {
				op->segoverride = sreg;
				need_mem_oper =
					1; /* only memory operands may follow */
				p = skip(p);
			} else {
		...
		if (*p == '%') {
			...
			if (given_type & JmpAbs) {
				cpu_error(
					3); /* immediate operand illegal with absolute jump */
				return PO_CORRUPT;
			}
			p++;
			op->value = parse_expr(&p);
			given_type =
				Imm; /* exact size is not available at this stage */
		}
			...
			if (*p != '(') {
				...
				given_type |=
					Disp; /* exact size is not available at this stage */
			...
			if (*p == '(') {
				...
				if (*p == ',') {
					...
					if (!(op->indexreg = parse_reg(&p))) {
						cpu_error(
							5); /* scale factor without index register */
						return PO_CORRUPT;
					}
					p = skip(p);
					if (*p == ',') {
				...
				if (*p++ != ')') {
					cpu_error(
						6); /* missing ')' in baseindex addressing mode */
					return PO_CORRUPT;
				}
	...
	if ((overlap & ~JmpAbs) && (given_type & (BaseIndex | JmpAbs)) ==
   ...
   to the data created by eval_instruction. */
{
	...
	if (!(realip->ext.flags & SUFFIX_CHECKED)) {
		...
		if ((mnemo->ext.opcode_modifier & suffix_flag(realip)) ||
			...
			if (!find_next_mnemonic(realip))
				cpu_error(
					0); /* instruction not supported on selected arch. */
		}
	}
	...
	if (realip->ext.last_size >= 0 &&
	    (diff = realip->ext.last_size - (int)size) != 0) {
		if (diff > 0) {
			if (cpudebug & 16)
...
int init_cpu()
{
	...
	for (i = 0; i < mnemonic_cnt; i++) {
		if (!strcmp(mnemonics[i].name, "addr16"))
			OC_ADDR_PREFIX =
				(unsigned char)mnemonics[i].ext.base_opcode;
...
int cpu_args(char* p)
{
	if (!strncmp(p, "-cpudebug=", 7)) {
		...
		if (!strcmp(p, "8086"))
			cpu_type = CPU086;
		else if (!strcmp(p, "i186"))
			cpu_type = CPU086 | CPU186;
		else if (!strcmp(p, "i286"))
			cpu_type = CPU086 | CPU186 | CPU286;
		else if (!strcmp(p, "i386"))
			cpu_type = CPU086 | CPU186 | CPU286 | CPU386;
		else if (!strcmp(p, "i486"))
			cpu_type = CPU086 | CPU186 | CPU286 | CPU386 | CPU486;
		else if (!strcmp(p, "i586"))
			cpu_type = CPU086 | CPU186 | CPU286 | CPU386 | CPU486 |
				   CPU586 | CPUMMX;
		else if (!strcmp(p, "i686"))
			cpu_type = CPU086 | CPU186 | CPU286 | CPU386 | CPU486 |
				   CPU586 | CPU686 | CPUMMX | CPUSSE;
		else if (!strcmp(p, "pentium"))
			cpu_type = CPU086 | CPU186 | CPU286 | CPU386 | CPU486 |
				   CPU586 | CPUMMX;
		else if (!strcmp(p, "pentiumpro"))
			cpu_type = CPU086 | CPU186 | CPU286 | CPU386 | CPU486 |
				   CPU586 | CPU686 | CPUMMX | CPUSSE;
		else if (!strcmp(p, "pentium4"))
			cpu_type = CPU086 | CPU186 | CPU286 | CPU386 | CPU486 |
				   CPU586 | CPU686 | CPUP4 | CPUMMX | CPUSSE |
				   CPUSSE2;
		else if (!strcmp(p, "k6"))
			cpu_type = CPU086 | CPU186 | CPU286 | CPU386 | CPU486 |
				   CPU586 | CPUK6 | CPUMMX | CPU3dnow;
		else if (!strcmp(p, "athlon"))
			cpu_type = CPU086 | CPU186 | CPU286 | CPU386 | CPU486 |
				   CPU586 | CPU686 | CPUK6 | CPUAthlon |
				   CPUMMX | CPU3dnow;
		else if (!strcmp(p, "sledgehammer"))
			cpu_type = CPU086 | CPU186 | CPU286 | CPU386 | CPU486 |
				   CPU586 | CPU686 | CPUK6 | CPUAthlon |
				   CPUSledgehammer | CPUMMX | CPU3dnow |
				   CPUSSE | CPUSSE2;
		else if (!strcmp(p, "64")) {
