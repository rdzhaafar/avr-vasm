...
static taddr opsize(operand* p, unsigned char num_operands, section* sec,
		    taddr pc);
	...
	if (tolower((unsigned char)rp[0]) != 'r') {
		...
		if (len == 2 && tolower((unsigned char)rp[0]) == 'y') {
			...
			*p = skip(rp);
			...
			return 1;
		} else if (len == 6 && (tolower((unsigned char)rp[0]) == 'f') &&
	...
	case 2: /* format P2 */
		if (rn->type == OP_GPR) { /* Is a register */
			...
			if (m.ext.opcode == 0x4B || m.ext.opcode == 0x4C) {
				val = val >>
				      2; /* CALL/JMP does a left shift of two bits */
			}
#ifdef INSTR_DEBUG
		...
		if (rn->type == OP_GPR) { /* Is a register */
			...
			if (m.ext.opcode >= 0x27 && m.ext.opcode <= 0x28) {
				if ((base != NULL && btype == BASE_OK &&
				    ...
				    base == NULL)
					val = val - pc -
					      4; /* Relative jump/call (%pc has been increased) */
				else if (btype == BASE_OK)
					add_extnreloc_masked(&db->relocs, base,
							     val - 4, REL_PC, 0,
							     22, 0, 0xfffffc);
				else
					general_error(
