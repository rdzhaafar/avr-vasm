...
#define FL_DoubleReg                                                           \
	0x20000 /* Dm:Dn or (Rm):(Rn), where both registers
                                   are put into "reg": 0nnn0mmm */
#define FL_KFactor                                                             \
	0x40000 /* k-factor <ea>{#n} or <ea>{Dn}, which
                                   can be found in bf_offset */
#define FL_FPSpec 0x80000 /* special FPU reg. FPIAR/FPCR/FPSR only */

#define FL_CheckMask                                                           \
	0xfc000 /* bits to check, when comparing with
...
#define ANY                                                                    \
	(SIZE_BYTE | SIZE_WORD | SIZE_LONG | SIZE_SINGLE | SIZE_DOUBLE |       \
...
/* flags */
#define IIF_MASK                                                               \
	1 /* value 2^size is represented by a 0 (M_val0)
...
#define mcf_all                                                                \
	(mcfa | mcfaplus | mcfb | mcfc | mcfhwdiv | mcfmac | mcfemac |         \
	 mcfusp | mcffpu | mcfmmu)
