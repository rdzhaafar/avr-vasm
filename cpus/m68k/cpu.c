...
/* return data operand type for these number of bits */
{
	...
	cpu_error(
		38,
		OPSZ_BITS(bits)); /* data obj. with n bits size are not supp. */
	return 0;
}
...
{
	static int cpus[] = { 68000, 68010, 68020, 68030, 68040, 68060 };
	int i;
...
static void set_g2_symbol(void)
{
   ...
   */
	set_internal_abs(g2_name,
			 0xff0000 | ((phxass_cpu_num(cpu_type) - 68000) << 8));
}
...
/* set cpu options for following atoms */
{
	...
	case OCMD_FPU:
		fpu_id = arg;
		if (phxass_compat)
			set_internal_abs(fpu_name,
					 (cpu_type & mfloat) ? fpu_id : 0);
		break;
	case OCMD_SDREG:
		sdreg = arg;
		break;
...
{
	static const char* ocmds[] = { "opt generic",
				       "opt movem",
				       "opt pea",
				       "opt clr",
				       "opt st",
				       ...
				       "opt base disp",
				       "opt outer disp",
				       "opt adda/subq to lea",
				       "opt lea to addq/subq",
				       "opt immediate areg",
				       "opt for speed",
				       "opt for size",
				       "opt small code",
				       "opt small data",
				       "warn about optimizations",
				       "PIC check",
				       "type and range checks",
				       "hide all warnings" };
	static const char* cpus[32] = {
		"m68000",    "m68010",	 "m68020",   "m68030",	 "m68040",
		"m68060",    "m6888x",	 "m68851",   "cpu32",	 "CF-ISA_A",
		"CF-ISA_A+", "CF-ISA_B", "CF-ISA_C", "CF-hwdiv", "CF-MAC",
		"CF-EMAC",   "CF-USP",	 "CF-FPU",   "CF-MMU",	 NULL,
		"ac68080",   NULL
	};
	int cmd = ((optcmd*)opts)->cmd;
	int arg = ((optcmd*)opts)->arg;
   ...
   Return zero otherwise. May cause an error message on illegal symbol type. */
{
	...
	if (name = parse_symbol(start)) {
		if ((sym = find_symbol(name)) && (sym->flags & REGLIST) &&
		    sym->type == EXPRESSION) {
   ...
   --------------------------------------------- */
{
	...
	if (ISIDSTART(*s) || (elfregs && *s == '%')) {
		...
		if ((sym = find_regsym(p, s - p)) != NULL) {
			...
			if (sym->reg_type == RSTYPE_Dn ||
			    sym->reg_type == RSTYPE_An)
				reg = ((sym->reg_type == RSTYPE_An) ? REGAn :
									    0) |
   ...
   returns 0 otherwise */
{
	...
	if (getfreg(&p) >= 0) {
		...
		for (p = *start; *p; p++) {
			p = skip(p);
			if (rangemode && fpnmode &&
			    (*p >= '0' && *p <= '7')) { /* allows fp0-7 */
				reg = *p++ - '0';
	...
	if (name) {
		...
		for (i = first, len = s - name; i <= last; i++) {
			if (!strnicmp(name, SpecRegs[i].name, len)) {
				...
				op->reg =
					i; /* @@@ Warning: indexes 128-255 are stored negative! */
	...
	if ((reg = getreg(start, 0)) >= 0) {
		...
		if (*p == '-' || *p == '/' || (ot->flags & OTF_REGLIST)) {
			...
			*start = s;
			...
			if (reg >= 10)
				op->flags |=
					FL_FPSpec; /* fpiar/fpcr/fpsr list */
		...
		if (r < 0) {
			...
			if ((sym = find_regsym(p, s - p)) != NULL) {
				...
				if (sym->reg_type == RSTYPE_Dn ||
				    sym->reg_type == RSTYPE_An ||
				    ((cpu_type & apollo) &&
				     sym->reg_type == RSTYPE_Bn)) {
					r = ((sym->reg_type == RSTYPE_Dn) ?
							   0 :
							   REGAn) |
					    (signed char)sym->reg_num;
					if (sym->reg_type == RSTYPE_Bn)
...
/* fill index register, including size and scale, into format word */
{
	if (i >= 0) {
		...
		op->format |= (REGisAn(i) ? FW_IndexAn : 0) | FW_IndexReg(i) |
   ...
   from the operand's displacement value. */
{
	if (op->reg >= 0 && op->reg <= 6 && baseexp[op->reg] && op->value[0]) {
		if (find_base(op->value[0], NULL, NULL, 0) == BASE_OK) {
			expr* new = make_expr(SUB, op->value[0],
					      copy_tree(baseexp[op->reg]));

			simplify_expr(new);
			op->value[0] = new;
			op->flags |=
				FL_BaseReg; /* mark potential BASEREG expression */
		}
	}
}
...
static char* parse_immediate(char* start, operand* op, int is_float,
			     int is_quad)
   ...
   given (or 0 when missing). Returns -1 when no valid expression was found. */
{
	if ((op->value[0] = parse_expr(p)) != NULL) {
		...
		if ((disp_size = read_extension(p, 0)) != 0)
			op->flags |=
				FL_NoOptBase; /* do not optimize, when size is given */
		return disp_size;
	}
	return -1;
}
...
int parse_operand(char* p, int len, operand* op, int required)
{
	...
	if (reqflags & OTF_DATA) {
		...
		op->reg = REG_Immediate;
		p = parse_immediate(
			p, op, (reqflags & OTF_FLTIMM) != 0 && is_float_ext(),
			(reqflags & OTF_QUADIMM) != 0);
	} else {
			...
			if (*ptmp == ':') {
				...
				if ((cpu_type & apollo) &&
					...
					if (reqflags & OTF_VXRNG2) {
						...
						if (reg >= 0) {
							if (op->mode ==
							    MODE_An) {
								op->mode =
									MODE_Dn; /* make it appear as Dn/DoubleReg mode */
								op->reg |=
									REGAn; /* restore An-bit for Rm */
							}
							op->reg |=
								reg
								<< 4; /* insert Rn with 4 bits too */
							op->flags |=
								FL_DoubleReg;
							p = ptmp;
						} else
					...
					if (reg >= 0 && REGisDn(reg)) {
						op->reg |= reg << 4;
						op->flags |=
							FL_DoubleReg | FL_020up;
						p = ptmp;
					} else
						cpu_error(
							18); /* data register required */
			...
			if (*p == '-' && *(p + 1) == '(') {
				...
				if (reg < 0 && (cpu_type & apollo)) {
					if ((reg = getbreg(&ptmp)) >= 0)
						op->flags |=
							FL_BnReg; /* Apollo Core: use Bn instead of An */
				}
			...
			parse_indir:
				if (*p == '[') {
					...
					if (op->value[0]) {
						...
						if (!devpac_compat)
							cpu_error(
								6); /* warn about displacement at bad position */
					...
					if (*p ==
						...
						if ((reg = getbasereg(&p)) <
							...
							if (*p == '[')
								goto parse_indir;
							cpu_error(
								7); /* base or index register expected */
							return PO_CORRUPT;
						}
					}
				}
				...
				if (mem_indir && reg < 0) {
					...
					op->flags |=
						FL_ZBase; /* ZAn was explicitely specified */

				if (reg >=
				    0) { /* "(Rn" or "(d,Rn" or "([Rn" or "([bd,Rn" */
					int clbrk = 0;
					...
					parse_index:
							...
							if ((idx = getbasereg(
								...
								if (!mem_indir ||
									...
									if (op->value[0] ==
										...
										if ((disp_size = base_disp_and_ext(
											     op,
											     &p)) <
										    0) {
											cpu_error(
												12); /* index register expected */
											return PO_CORRUPT;
										} else {
											if (!devpac_compat)
												cpu_error(
													6); /* displacement at bad position */
											p = skip(
												p);
											goto parse_index;
										}
						...
						if (idx < 0)
							op->format &=
								~FW_Postindexed; /* index was suppressed */
					} else {
					...
					if (mem_indir) {
						...
						if (*p ==
						    ',') { /* "([bd,Rn],Xn,od" or "([bd,Rn,Xn],od" */
							/* read outer displacement */
							p = skip(p + 1);
							if (op->value[1] ==
							    NULL) {
								if ((op->value[1] = parse_expr(
									     &p)) !=
								    NULL) {
									if ((od_size = read_extension(
										     &p,
										     0)) !=
									    0)
										op->flags |=
											FL_NoOptOuter; /* do not optimize with size */
									else
										od_size =
											EXT_WORD;
									p = skip(
									...
									cpu_error(
										14); /* missing outer displacement */
							}
						}
						if (op->value[1] != NULL) {
							/* set outer displacement */
							if (od_size == EXT_WORD)
								op->format |= FW_IndSize(
									FW_Word);
							else if (od_size ==
								 EXT_LONG)
								op->format |= FW_IndSize(
				...
				if (reg >= 0) {
					if (idx >= 0) {
						if (REGisPC(idx)) {
							cpu_error(
								16); /* can't use PC register as index */
							return PO_CORRUPT;
						}
						if (REGisBn(idx)) {
							cpu_error(
						...
						if (cpu_type & mcf) {
							if (REGext(idx) != 0 &&
							    REGext(idx) !=
								    EXT_LONG)
								cpu_error(
									5); /* bad extension - only .l for ColdFire */
							idx &= ~(
								EXT_MASK
								<< REGext_Shift);
							idx |= EXT_LONG
						...
						if (REGisZero(idx))
							op->flags |=
								FL_ZIndex; /* ZRn was explicitely specified */
					}

					/* set default displacement sizes */
					if (!disp_size)
						disp_size = idx < 0 ? EXT_WORD :
									    EXT_BYTE;

					if (!mem_indir && !REGisZero(reg) &&
					    op->value[1] == NULL &&
					    ((idx < 0 &&
					      disp_size == EXT_WORD) ||
					     (idx >= 0 &&
						...
						if (idx < 0) {
							if (op->value[0]) {
								if (REGisPC(reg)) {
									op->mode =
										MODE_Extended; /* (d16,PC) */
									op->reg =
										REG_PC16Disp;
								} else {
									op->mode =
										MODE_An16Disp; /* (d16,An) */
									op->reg = REGget(
										reg);
									check_basereg(
										op);
						...
						if (REGisPC(reg)) {
							op->mode =
								MODE_Extended; /* ([bd,PC,Xn],od) */
							op->reg = REG_PC8Format;
						} else {
							op->mode =
								MODE_An8Format; /* ([bd,An,Xn],od) */
							op->reg = REGget(reg);
							check_basereg(op);
						}
						if (REGisZero(reg))
						...
						if (op->value[0]) {
							...
							op->format |= FW_BDSize(
								FW_Null); /* base disp. suppressed */
					}

					if (REGisBn(reg))
						op->flags |=
							FL_BnReg; /* Apollo Core: Bn instead of An */
				}
			}
		...
		if (*p == '{') {
			...
			if (*p == ':') {
				...
				if (*p == '}')
					...
					cpu_error(15, '}'); /* } expected */
				op->flags |=
					FL_Bitfield | FL_020up | FL_noCPU32;
			} else if (*p == '}') {
				if (absk) {
					op->bf_offset = (unsigned char)bfval;
					if (typechk &&
					    (bfval < -64 || bfval > 63))
						cpu_error(
							21); /* value from -64 to 63 required */
				} else /* k-factor is a data register */
					op->bf_offset =
						(unsigned char)(bfval & 7) << 4;
				op->flags |= FL_KFactor;
				p++;
			} else {
	...
	for (i = 0; i < 16; i++) {
		if (reqmode & (1 << i)) {
			...
			if ((op->flags & FL_CheckMask) ==
				    (reqflags & FL_CheckMask) &&
			    addrmodes[i].mode == op->mode &&
			    (addrmodes[i].reg < 0 ||
			     addrmodes[i].reg == op->reg)) {
				if (reqflags & OTF_CHKREG) {
					if ((unsigned char)op->reg <
						    optypes[required].first ||
					    (unsigned char)op->reg >
						    optypes[required].last)
						return PO_NOMATCH;
				}
#if 0 /* @@@ not used */
        if (reqflags & OTF_CHKVAL) {
          ...
          if (op->value[0]->type == NUM) {
            ...
            ierror(0);
				...
				if (*p != '\0' && p < (start + len))
					cpu_error(
						67); /* trailing garbage in operand */
				return PO_MATCH;
			}
		}
	}
	...
	for (i = 0; i < 2; i++) {
		...
		eval:
			if (!eval_expr(op->value[i], &op->extval[i], sec, pc)) {
				op->basetype[i] = find_base(
					op->value[i], &op->base[i], sec, pc);
			...
			op->extval[i] =
				0x7fffffff; /* dummy to prevent immediate opt. */
			op->flags &= ~(FL_ExtVal0 << i);
		}
	}
}
...
/* evaluate expressions in operand and try to optimize addressing modes */
{
	...
	secrel = (sec->flags & ABSOLUTE) ?
			       op->base[0] == NULL :
			       op->base[0] != NULL && LOCREF(op->base[0]) &&
				 op->base[0]->sec == sec;
	...
	if (bdopt) { /* base displacement optimizations allowed */
		...
		if (op->mode == MODE_An16Disp) {
			...
			if ((cpu_type & (m68020up | cpu32)) &&
			    ((op->base[0] && !undef) ||
			     (!op->base[0] && (op->extval[0] < -0x80 ||
					       op->extval[0] > 0x7f)))) {
				/* (d8,An,Rn) --> (bd,An,Rn) */
				op->format &= 0xff00;
				op->format |=
					FW_FullFormat | FW_BDSize(FW_Word);
				op->flags |= FL_020up;
				if (final && warn_opts > 1)
			 ...
			 !(op->format & FW_FullFormat)) {
			int pcdisp8 =
				absdpc ? (op->extval[0] >= -0x80 &&
					  op->extval[0] <= 0x7f) :
					       ((pcdisp >= -0x80 && pcdisp <= 0x7f) ||
					  undef);
			...
			} else if (op->base[0] && typechk &&
				   LOCREF(op->base[0])) {
				/* label.w --> label.l */
				op->reg = REG_AbsLong;
				if (final)
					cpu_error(
						22); /* need 32 bits to reference a program label */
			}
		}
	...
	if (op->mode == MODE_An8Format && (op->flags & FL_UsesFormat) &&
	    (op->format & FW_FullFormat)) {
		if (FW_getIndSize(op->format) ==
		    FW_None) { /* no memory indirection */
				...
				if (opt_bdisp && bdopt && !op->base[0] &&
					...
					if (final && warn_opts > 1)
						cpu_error(
							50,
							"(bd16,An,...)->(bd32,An,...)");
				} else if (opt_bdisp && bdopt && !op->base[0] &&
					   size16[0] &&
					   FW_getBDSize(op->format) ==
						   FW_Long) {
					if ((op->format & FW_IndexSuppress) &&
						...
						op->format &=
							~FW_BDSize(FW_SizeMask);
						op->format |=
							FW_BDSize(FW_Word);
						if (final && warn_opts > 1)
							cpu_error(
								49,
								"(bd32,An,Rn)->(d16,An,Rn)");
					}
				}
			...
			if (opt_bdisp && bdopt && !op->base[0] &&
			    FW_getBDSize(op->format) > FW_Null &&
			    op->extval[0] == 0) {
				/* ([0,An,...],...) --> ([An,...],...) */
				op->format &= ~FW_BDSize(FW_SizeMask);
				op->format |= FW_BDSize(FW_Null);
			...
			if (FW_getIndSize(op->format) >=
			    FW_Word) { /* outer displacement */
				if (opt_odisp && odopt && !op->base[1] &&
				    op->extval[1] == 0) {
					/* ([...],0) --> ([...]) */
					...
					if (final && warn_opts > 1)
						cpu_error(
							50,
							"([...],od16)->([...],od32)");
				} else if (opt_odisp && odopt && !op->base[1] &&
					   size16[1] &&
					   FW_getIndSize(op->format) ==
						   FW_Long) {
					/* ([...],od32) --> ([...],od16) */
...
static instruction* ip_dualop(int code, char* q, signed char mode1,
			      ...
			      uint16_t format1, expr* exp1, signed char mode2,
			      signed char reg2, uint16_t flags2,
			      uint16_t format2, expr* exp2)
{
	instruction* ip;
...
{
	return ip_dualop(code, q, mode, reg, flags, format, exp, -1, 0, 0, 0,
			 NULL);
}
	...
	if (final) {
		...
		if (m == MODE_AnIndir) {
			...
			op->value[0] = number_expr(offset);
			   ...
			   (m == MODE_Extended &&
			    (r == REG_AbsShort || r == REG_AbsLong))) {
			/* increment the expression by 'offset' */
			op->value[0] = make_expr(ADD, op->value[0],
						 number_expr(offset));
		}
	} else {
					  ...
					  taddr pc, int final)
{
	...
	if ((mnemo->ext.opcode[0] == 0x4ec0 ||
	     mnemo->ext.opcode[0] == 0x4e80) &&
	...
	for (i = 0; i < MAX_OPERANDS && ip->op[i] != NULL; i++)
		optimize_oper(ip->op[i], &optypes[mnemo->operand_type[i]], sec,
			      pc, cpc, final);
	...
	if (mnemo->ext.opcode[0] == 0x4880 && mnemo->operand_type[0] != IR &&
		...
		ip->code +=
			3; /* take matching mnemonic with swapped operands */
		mnemo = &mnemonics[ip->code];
		if (final && ip->op[0]->base[0] == NULL &&
		    ip->op[0]->value[0] != NULL &&
	...
	/* resolve register lists in FMOVEM instructions */
		...
		if (ip->op[1]->base[0] == NULL && ip->op[1]->value[0] != NULL &&
			...
			if (ip->op[1]->extval[0] & 0x1c00)
				ip->code =
					OC_FMOVEMTOSPEC; /* list contains special registers */
			...
			if (final && ip->op[0]->base[0] == NULL &&
			    ...
			    (ip->op[0]->value[0]->c.sym->flags & REGLIST))
				cpu_error(62); /* register list on both sides */
		} else if (ip->op[0]->base[0] == NULL &&
			   (ip->op[0]->extval[0] & 0x1c00)) {
...
dontswap:
	...
	if (opt_mul && abs && (oc == 0xc0c0 || oc == 0xc1c0 || oc == 0x4c00) &&
		...
		/* MULU/MULS #x,Dn */
		int muls = (oc & 0x0100) != 0 ||
			   (mnemo->ext.opcode[1] & 0x0800) != 0;
		...
		if (val == 0) {
			...
			if (ext == 'w') {
				...
				if (opt_speed) {
					...
					ip->op[0] = ip->op[1];
					ip->op[1] = NULL;
					ip->ext.un.copy.next = ip_singleop(
						OC_NEG, l_str, MODE_Dn,
			...
			if (ext == 'w' && muls && opt_speed) {
				...
				if (final) {
					...
					if (warn_opts)
						cpu_error(
							51,
							"mulu.w #x,Dn -> mvz.w Dn,Dn + lsl.l #x,Dn");
				} else
					ip2->op[0]->flags |= FL_DoNotEval;
			...
			if (opt_speed && ext == 'w') {
				...
				ip->op[1] = NULL;
				ip->ext.un.copy.next = ip2; /* append ASL */
				ip2->ext.un.copy.next =
					ip_singleop(OC_NEG, l_str, MODE_Dn,
						    ip2->op[1]->reg, 0, 0,
						    NULL);
				ip = ip2; /* make stage 2 look at the ASL */
	...
	if ((ip->code == OC_MOVE || oc == 0x0040) &&
	    ip->op[0]->mode == MODE_Extended &&
	    ip->op[0]->reg == REG_Immediate) {
		int movqabsl = opt_moveq && abs && ext == 'l' &&
		...
		if (movqabsl && val >= -0x80 && val <= 0x7f) {
			...
			if (final) {
				...
				if (warn_opts > 1)
					cpu_error(
						51,
						"move.l #x,Dn -> moveq #x,Dn + add.w Dn,Dn");
			} else
				ip->op[0]->flags |= FL_DoNotEval;
			ip->op[0]->extval[0] = val >> 1;
			ip->ext.un.copy.next =
				ip_dualop(OC_ADD,
					  (cpu_type & mcf) ? l_str : w_str,
					  MODE_Dn, ip->op[1]->reg, 0, 0, NULL,
					  MODE_Dn, ip->op[1]->reg, 0, 0, NULL);
		} else if (opt_nmovq && abs && ext == 'l' &&
			   ip->op[1]->mode == MODE_Dn &&
			   !(cpu_type & (m68040 | mcf)) && val >= 0x80 &&
			   val <= 0xff) {
			...
			if (final) {
				...
				if (warn_opts)
					cpu_error(
						51,
						"move.l #x,Dn -> moveq #y,Dn + not.b Dn");
			} else
				ip->op[0]->flags |= FL_DoNotEval;
			ip->op[0]->extval[0] = val ^ 0xff;
			ip->ext.un.copy.next =
				ip_singleop(OC_NOT, b_str, MODE_Dn,
					    ip->op[1]->reg, 0, 0, NULL);
		} else if (movqabsl && !(cpu_type & m68040) &&
			   (((val & 0xffff) == 0 && val >= 0x10000 &&
			     val <= 0x7f0000) ||
			    ((val & 0xffff) == 0xffff && val >= 0xff80ffff &&
			     val <= 0xfffeffff))) {
			...
			if (final) {
				...
				if (warn_opts > 1)
					cpu_error(
						51,
						"move.l #x,Dn -> moveq #y,Dn + swap Dn");
			} else
				ip->op[0]->flags |= FL_DoNotEval;
			ip->op[0]->extval[0] = val >> 16;
			ip->ext.un.copy.next =
				ip_singleop(OC_SWAP, w_str, MODE_Dn,
					    ip->op[1]->reg, 0, 0, NULL);
		} else if (opt_nmovq && abs && ext == 'l' &&
			   ip->op[1]->mode == MODE_Dn &&
			   !(cpu_type & (m68040 | mcf)) &&
			...
			if (final) {
				free_expr(ip->op[0]->value[0]);
				ip->op[0]->value[0] =
					number_expr(-((int16_t)val));
				if (warn_opts)
					cpu_error(
						51,
						"move.l #x,Dn -> moveq #y,Dn + neg.w Dn");
			} else
				ip->op[0]->flags |= FL_DoNotEval;
			ip->op[0]->extval[0] = -((int16_t)val);
			ip->ext.un.copy.next =
				ip_singleop(OC_NEG, w_str, MODE_Dn,
					    ip->op[1]->reg, 0, 0, NULL);
		} else if (opt_size && movqabsl && (val & 0xff) == 0 &&
			   !(cpu_type & mcf) && val >= 0x100 && val <= 0x7f00) {
			/* move.l #x,Dn --> moveq #x>>n,Dn ; lsl.w #n,Dn */
			instruction* ip2 = ip_dualop(
				OC_LSLI, w_str, MODE_Extended, REG_Immediate,
			...
			if (final && (warn_opts > 1 ||
				      (warn_opts &&
				       !(cpu_type & (m68010up | mcf | cpu32)))))
				cpu_error(51, "move #0 -> clr");
		} else if (opt_moveq && abs && ext == 'l' &&
			   (val == -1 || (val >= 1 && val <= 7)) &&
			   (cpu_type & (mcfb | mcfc))) {
		...
		} else if (opt_st && abs && ext == 'b' && !(oc & 0x0040) &&
			   (val & 0xff) == 0xff) {
			/* move.b #-1,<ea> --> st <ea> */
			...
			if (abs && val >= -0x8000 && val <= 0x7fff) {
				...
				if (final && warn_opts)
					cpu_error(51,
						  "move.l #x,-(a7) -> pea x.w");
			} else if (cpu_type & (m68000 | m68010 | m68020 |
					       m68030 | cpu32)) {
		...
		if (ip->op[o]->mode == MODE_Extended &&
		    (ip->op[o]->reg == REG_RnList ||
		     ip->op[o]->reg == REG_Immediate) &&
		    ip->op[o]->base[0] == NULL &&
		    !(ip->op[o]->flags & FL_NoOpt)) {
			...
			if (regs == 0) {
				...
				if ((opt_movem || (!(list & 0xff) && o == 1)) &&
					...
					if (final &&
						...
						cpu_error(
							51,
							"movem ea,Rn -> move ea,Rn");
				}
			} else if (regs == 2 && opt_speed &&
				   ((cpu_type & m68040) ||
				    (!(cpu_type & (m68000 | m68010)) &&
           ...
           or extended addressing modes for 68040 only. */
				...
				if ((opt_movem || (!(list & 0xff) && o == 1)) &&
					...
					if (final &&
						...
						cpu_error(
							51,
							"movem ea,Rm/Rn -> move ea,Rm + move ea,Rn");
				}
			}
		}
	}
	...
	else if (opt_gen && oc == 0x4200 && ip->op[0]->mode == MODE_Dn &&
		 ext == 'l') {
		/* CLR.L Dn --> MOVEQ #0,Dn */
		ip->code = OC_MOVEQ;
		ip->qualifiers[0] = l_str;
		...
		if ((cpu_type & (mcfb | mcfc)) && ip->op[1]->mode == MODE_Dn &&
		    ext == 'l' && (val == 0xff || val == 0xffff)) {
			...
			if (final) {
				...
				if (warn_opts > 1)
					cpu_error(
						51,
						"andi.l #$ff/$ffff,Dn -> mvz.b/w Dn,Dn");
			}
			ip->op[0] = ip->op[1];
		} else if ((val == 0xff && ext == 'b') ||
			   (val == 0xffff && ext == 'w') ||
			...
			if (final) {
				...
				if (warn_opts > 1)
					cpu_error(
						51,
						"andi.b/w/l #$ff/$ffff/$fffffff -> tst");
			}
			ip->op[0] = ip->op[1];
			ip->op[1] = NULL;
		} else if (val == 0 &&
		...
		if (ip->op[0]->mode == MODE_Extended &&
			...
			if (opt_quick && val >= 1 && val <= 8) {
				...
				if (final && warn_opts > 1)
					cpu_error(51,
						  "add/sub #x -> addq/subq #x");
			} else if ((oc & 0x90c0) == 0x90c0) { /* ADDA/SUBA */
				if (!(oc & 0x4000))
					val = -val;
				if (opt_gen && val == 0) {
					ip->code =
						-1; /* delete ADDA/SUBA #0,An */
					if (final && warn_opts > 1)
						cpu_error(
							51,
							"adda/suba #0,An deleted");
				} else if (opt_lea && val >= -0x8000 &&
					   val <= 0x7fff) {
		...
		if (ip->op[0]->mode == MODE_An16Disp && abs) {
			if (opt_gen && ip->op[0]->reg == ip->op[1]->reg &&
			    val == 0) {
				/* delete LEA (0,An),An */
				ip->code = -1;
				if (final && warn_opts > 1)
				...
				if (ip->op[0]->reg == ip->op[1]->reg) {
					...
					instruction* ip2 = ip_dualop(
						OC_ADDA, l_str, MODE_Extended,
						REG_Immediate, FL_NoOpt, 0,
						ip->op[0]->value[0], MODE_An,
						ip->op[1]->reg, FL_NoOpt, 0,
						NULL);
					ip->code = OC_MOVEA;
					ip->op[0]->mode = MODE_An;
					ip->ext.un.copy.next =
				...
				if (final)
					cpu_error(
						47); /* lea-displacement out of range, changed */
			}
		...
		if (val == 1) {
			...
			if (!(oc & 0x0e00))
				ip->op[0]->reg = ip->op[1]->reg;
			if (final &&
			    (warn_opts > 1 || (warn_opts && (oc & 0x0008))))
				cpu_error(51, "asl/lsl #1 -> add");
		} else if (opt_speed && opt_lsl && val == 2 &&
			   (ext == 'b' || ext == 'w')) {
		 ...
		 abs && mnemo->operand_type[0] == FA &&
		 mnemo->operand_type[1] == F_ &&
		 ip->op[0]->mode == MODE_Extended &&
		 ip->op[0]->reg == REG_Immediate) {
		...
		if (ext == 'x') {
           ...
           0.0 is always translated to single-precision in the next step. */
				...
				exp = ((((int)buf[0] & 0x7f) << 8) |
				       (int)buf[1]) -
				      0x3fff;
				man = readval(1, buf + 4, 8) &
				      0x7fffffffffffffffLL;
				if (exp >= -0x3ff && exp <= 0x400 &&
				    (man & 0x7ff) == 0) {
					...
					v = v | ((int64_t)(exp + 0x3ff) << 52) |
					    (man >> 11);
					if (final) {
						free_op_exp(ip->op[0]);
						ip->op[0]->value[0] = huge_expr(
							huge_from_int(v));
						if (warn_opts > 1)
							cpu_error(
								51,
								"f<op>.x #m,FPn -> f<op>.d #m,FPn");
					}
					ip->qualifiers[0] = d_str;
					ext = 'd';
					setval(1, buf, 8, v);
		...
		if (ext == 'd') {
			/* Fxxx.D #m,FPn */
			exp = ((((int)buf[0] & 0x7f) << 4) |
			       (((int)buf[1] & 0xf0) >> 4)) -
			      0x3ff;
			man = readval(1, buf, 8) & 0xfffffffffffffLL;
			if ((exp >= -0x7f && exp <= 0x80 &&
			     (man & 0x1fffffff) == 0) ||
			    (exp == -0x3ff &&
			     man == 0)) { /* also allow all zeros for 0.0 */
				/* single precision would be sufficient, so convert */
				uint32_t v = (buf[0] & 0x80) ? 0x80000000 :
								     0; /* m. sign */
		...
		if (final && (!strcmp(mnemo->name, "fdiv") ||
			      !strcmp(mnemo->name, "fsdiv") ||
			      !strcmp(mnemo->name, "fddiv") ||
			      !strcmp(mnemo->name, "fsgldiv"))) {
         ...
         which is the case when the mantissa is zero. */
			...
			if (ext == 's') {
				exp = ((((int)buf[0] & 0x7f) << 1) |
				       (((int)buf[1] & 0x80) >> 7)) -
				      0x7f;
				if ((readval(1, buf, 4) & 0x007fffffLL) == 0 &&
				    exp != -0x7f) {
					setbits(1, buf, 16, 1, 8,
						0x7f - exp); /* 8-bit exponent to offset 1 */
					free_op_exp(ip->op[0]);
					ip->op[0]->value[0] =
						number_expr(readval(1, buf, 4));
					optok = 1;
				}
			} else if (ext == 'd') {
				exp = ((((int)buf[0] & 0x7f) << 4) |
				       (((int)buf[1] & 0xf0) >> 4)) -
				      0x3ff;
				if ((readval(1, buf, 8) & 0xfffffffffffffLL) ==
					    0 &&
				    exp != -0x3ff) {
					setbits(1, buf, 16, 1, 11,
						0x3ff - exp); /* 11-bit exponent to offset 1 */
					free_op_exp(ip->op[0]);
					ip->op[0]->value[0] = huge_expr(
						huge_from_mem(1, buf, 8));
					optok = 1;
				}
			} else if (ext == 'x') {
				exp = ((((int)buf[0] & 0x7f) << 8) |
				       (int)buf[1]) -
				      0x3fff;
				if ((readval(1, buf + 4, 8) &
				     0x7fffffffffffffffLL) == 0) {
					setbits(1, buf, 16, 1, 15,
						0x3fff -
							exp); /* 15-bit exponent to offset 1 */
					free_op_exp(ip->op[0]);
					ip->op[0]->value[0] = huge_expr(
						huge_from_mem(1, buf, 12));
					optok = 1;
				}
			}
			if (optok) {
				if (!strcmp(mnemo->name, "fdiv")) {
					...
					if (warn_opts > 1)
						cpu_error(
							51,
							"fdiv #m,FPn -> fmul #1/m,FPn");
				} else if (!strcmp(mnemo->name, "fsdiv")) {
					ip->code = OC_FSMUL;
					if (warn_opts > 1)
						cpu_error(
							51,
							"fsdiv #m,FPn -> fsmul #1/m,FPn");
				} else if (!strcmp(mnemo->name, "fddiv")) {
					ip->code = OC_FDMUL;
					if (warn_opts > 1)
						cpu_error(
							51,
							"fddiv #m,FPn -> fdmul #1/m,FPn");
				} else if (!strcmp(mnemo->name, "fsgldiv")) {
					ip->code = OC_FSGLMUL;
					if (warn_opts > 1)
						cpu_error(
							51,
							"fsgldiv #m,FPn -> fsglmul #1/m,FPn");
				}
			}
		}
	}
		...
		if (val == 0) {
			...
			if (final) {
				...
				if (warn_opts)
					cpu_error(
						51,
						"divs.w #-1,Dn -> neg.w Dn + mvz.w Dn,Dn");
			}
			ip->op[0] = ip->op[1];
			ip->op[1] = NULL;
			ip->ext.un.copy.next =
				ip_dualop(OC_MVZ, w_str, MODE_Dn,
					  ip->op[0]->reg, 0, 0, NULL, MODE_Dn,
					  ip->op[0]->reg, 0, 0, NULL);
		}
	}

	else if (oc == 0x4c40) {
		if (abs && ip->op[0]->mode == MODE_Extended &&
		    ip->op[0]->reg == REG_Immediate) {
			/* DIVU.L/DIVS.L #x,Dn */
				...
				if (val == 1) {
					...
					if (final) {
						...
						if (warn_opts)
							cpu_error(
								51,
								"divu/divs.l #1,Dn -> tst.l Dn");
					}
					ip->op[0] = ip->op[1];
					ip->op[1] = NULL;
				} else if (val == -1 &&
					   (mnemo->ext.opcode[1] & 0x0800)) {
		...
		} else if (mnemo->operand_type[1] == DD &&
			   !(mnemo->ext.opcode[1] & 0x0400) &&
			   ((ip->op[1]->reg >> 4) & 0xf) ==
				   (ip->op[1]->reg & 0xf))
		...
		if (pcrelok && opt_pc && !(ip->op[0]->flags & FL_NoOpt) &&
			...
			if (lastsize == 0 || (diff == 0 && (oc & 0x40))) {
				ip->code =
					-1; /* delete a JMP to following location */
				if (final && warn_opts > 1)
		...
		if (opt_bra && ((ipflags & IFL_UNSIZED) || opt_allbra) &&
          ...
          if (diff!=-2 && done)
				...
				if (diff == 2) {
					...
					if (cpu_type &
						...
						if (oc < 0x6200) {
							...
							if (final)
								cpu_error(
									46); /* branch out of range changed to jmp */
						} else {
			...
			if (final && warn_opts > 1) {
				...
				if (ip->code == -1)
					...
					if (newsize != oldsize)
						cpu_error(newsize < oldsize ?
									53 :
									54,
							  new_ext);
				}
			}
		} else if (opt_branop && oc != 0x6100 && pcrelok &&
			   val - cpc == 0 && (ext == 'b' || ext == 's')) {
         ...
         is turned into a NOP */
			...
			if (oc < 0x6200) {
				...
				if (final) {
					/* assign "*+8" as the Bcc's expression */
					ip->op[0]->value[0] = make_expr(
						ADD, curpc_expr(),
						number_expr(phxass_compat ? 6 :
										  8));
					if (warn_opts > 1)
						cpu_error(
							52,
							"b<cc> label -> b<!cc> *+8, jmp label");
				}
			}
		}
	}
		...
		if (final && ip->op[2] != NULL && ip->op[2]->base[0] == NULL &&
			...
			cpu_error(
				64); /* An operand at level #0 causes F-line Exception */
	}

	if (opt_immaddr && abs && ext == 'l' && ip->op[0] != NULL &&
	    ip->op[0]->mode == MODE_Extended &&
	    ip->op[0]->reg == REG_Immediate && ip->op[1] != NULL &&
	    ip->op[1]->mode == MODE_An && val >= -0x8000 && val <= 0x7fff &&
   ...
   to the data created by eval_instruction. */
{
	...
	char ext = realip->qualifiers[0] ?
				 tolower((unsigned char)realip->qualifiers[0][0]) :
				 '\0';
	int i;
	size_t size;
	instruction* ip;
	...
	while (!(mnemo->ext.available & cpu_type)) {
       ...
       name and all operand-types */
		...
		mnemo++;
		if (strcmp(lastm->name, mnemo->name) ||
		    !optypes_subset(lastm, mnemo))
	...
	if (ext == '\0') {
		...
		if ((extsize & SIZE_MASK) != 0) {
			if ((extsize & S_CFCHECK) && (cpu_type & mcf))
				extsize &= ~(
					SIZE_BYTE |
					SIZE_WORD); /* SIZE_LONG for ColdFire only */
			if ((extsize & SIZE_LONG) &&
			    (cpu_type & mcf)) /* ColdFire prefers .l */
	...
	if ((mnemo->ext.size & SIZE_MASK) == SIZE_UNSIZED) {
		if (ext != '\0') {
			if (!ign_unambig_ext)
				cpu_error(
					35); /* extension for unsized instruction ignored */
			ext = '\0';
			realip->qualifiers[0] = emptystr;
		}
	} else {
		...
		uint16_t sz =
			lc_ext_to_size(ext); /* convert ext. to SIZE-code */

		if ((mnemo->ext.size & SIZE_UNAMBIG) &&
		    (mnemo->ext.available & cpu_type))
			uacode =
				realip->code; /* last mnemonic with unambiguous size ext. */
		else
			uacode = -1;
       ...
       given size extension. */
		while (!((((extsize & S_CFCHECK) && (cpu_type & mcf)) ?
			 ...
			 sz)) {
			mnemo++;
			if ((err = strcmp(mnemonics[realip->code].name,
					  mnemo->name)) != 0)
				break;
			if ((err = !optypes_subset(&mnemonics[realip->code],
						   mnemo)) != 0)
				break;
  ...
  if (realip->op[0]!=NULL && realip->op[1]!=NULL) {
	...
	if (!(extflags & IFL_RETAINLASTSIZE))
		realip->ext.un.real.last_size =
			size; /* remember size for next pass */
			...
			if ((val > (1L << (size - 1)) - 1) ||
				...
				if (val > 0 && val < (1L << size))
					cpu_error(
						...
						(1L << (size - 1)) - 1,
						val - (1L
						       << size)); /* using signed operand as unsigned */
				else
					cpu_error(
						25, val, -(1L << (size - 1)),
						(1L << (size - 1)) -
							1); /* operand value out of range */
			}
		} else {
...
/* calculate and write branch displacement of desired size, handle relocs */
{
	...
	if (op->base[0] && is_pc_reloc(op->base[0], sec)) {
		...
		add_extnreloc(&db->relocs, op->base[0], addend, REL_PC, 0, size,
			      offset);
	} else {
		...
		case 's':
			if (diff >= -0x80 && diff <= 0x7f && diff != 0)
				*(d - 1) = diff & 0xff;
				...
				cpu_error(
					28); /* branch destination out of range */
			break;
		case 'l':
			if (cpu_type & (m68020up | cpu32 | mcfb | mcfc |
					m68881 | m68882 | m68851)) {
				   ...
				   unsigned char* d, operand* op, int rtype)
{
	if (rtype == REL_ABS && op->basetype[num] == BASE_PCREL)
		op->extval[num] +=
			d - db->data; /* fix addend for label differences */
	...
	if (op->mode > MODE_Extended ||
		...
		if (op->mode == MODE_An16Disp) {
			...
			if (rtype == REL_SD && LOCREF(op->base[0])) {
				if (typechk && (op->extval[0] < 0 ||
						op->extval[0] > 0xffff))
					cpu_error(
						29); /* displacement out of range */
			} else {
				if (typechk && (op->extval[0] < -0x8000 ||
						op->extval[0] > 0x7fff))
					cpu_error(
						29); /* displacement out of range */
			}
			...
			if (op->format & FW_FullFormat) {
				...
				if (FW_getBDSize(op->format) == FW_Word) {
					...
					d = write_extval(0, 2, db, d, op,
							 rtype);
				} else if (FW_getBDSize(op->format) ==
					   FW_Long) {
					if (op->base[0]) {
						rtype = REL_ABS;
						rsize = 32;
					}
					d = write_extval(0, 4, db, d, op,
							 rtype);
				}
				if (FW_getIndSize(op->format) == FW_Word) {
					if (typechk &&
					...
					if (op->base[1]) {
						...
						if (EXTREF(op->base[1]) ||
							...
							cpu_error(
								30); /* absolute displacement expected */
					}
					d = write_extval(1, 2, db, d, op,
							 ortype);
				} else if (FW_getIndSize(op->format) ==
					   FW_Long) {
					if (op->base[1]) {
						ortype = REL_ABS;
						orsize = 32;
					}
					d = write_extval(1, 4, db, d, op,
							 ortype);
				}
			} else {
				/* (d8,An,Rn.x*s) needs one format word as extension */
				if (typechk && (op->extval[0] < -0x80 ||
						op->extval[0] > 0x7f))
					cpu_error(
						29); /* displacement out of range */
				if (op->base[0]) {
			...
			if (op->reg == REG_PC16Disp) {
				...
				if (op->base[0] != NULL &&
				    is_pc_reloc(op->base[0], sec)) {
					rtype = REL_PC;
					rsize = 16;
				} else if (op->base[0] != NULL ||
					   (sec->flags & ABSOLUTE))
					disp = op->extval[0] - pc;
				else
					cpu_error(
						no_dpc ?
							      26 :
							      68); /* absolute PC-displacement */
				if (typechk &&
				    (disp < -0x8000 || disp > 0x7fff))
					cpu_error(
						29); /* displacement out of range */
				d = setval(1, d, 2, disp);
			}
				...
				if (op->format & FW_FullFormat) {
					...
					d = setval(1, d, 2, op->format);
					roffs += 2;
					if (FW_getBDSize(op->format) ==
					    FW_Word) {
						if (op->base[0] != NULL &&
						    is_pc_reloc(op->base[0],
								sec)) {
							rtype = REL_PC;
							rsize = 16;
							op->extval[0] +=
								2; /* pc-relative xref fix */
							disp += 2;
						} else if (op->base[0] !=
								   NULL ||
							   (sec->flags &
				...
				if (typechk && (op->extval[0] < -0x8000 ||
						op->extval[0] > 0x7fff))
					cpu_error(
						32); /* absolute short address out of range */
				if (op->base[0]) {
					rtype = REL_ABS;
					rsize = 16;
				}
				...
				case 'b':
					if (op->flags & FL_ExtVal0) {
						...
						rsize = 8;
						*d++ = 0;
						d = write_extval(0, 1, db, d,
								 op, rtype);
						if (typechk &&
						    (op->extval[0] < -0x80 ||
						     op->extval[0] > 0xff))
							cpu_error(
								36); /* immediate operand out of range */
					} else
						cpu_error(
							37); /* immediate operand has illegal type */
					break;
				case 'w':
					if (op->flags & FL_ExtVal0) {
						rsize = 16;
						d = write_extval(0, 2, db, d,
								 op, rtype);
						if (typechk &&
						    (op->extval[0] < -0x8000 ||
						     op->extval[0] > 0xffff))
				...
				case 'l':
					if (op->flags & FL_ExtVal0) {
						rsize = 32;
						d = write_extval(0, 4, db, d,
								 op, rtype);
					} else if (type_of_expr(op->value[0]) ==
						   FLT) {
						if ((err = copy_float_exp(
							     d, op,
							     EXT_SINGLE)) != 0)
							cpu_error(err);
						d += 4;
		...
		if (rtype != REL_NONE) {
			if (rtype == REL_ABS && op->basetype[0] == BASE_PCREL)
				rtype = REL_PC;
			add_extnreloc(&db->relocs, op->base[0], op->extval[0],
				      rtype, 0, rsize, roffs);
		}
		if (ortype != REL_NONE) {
			if (ortype == REL_ABS && op->basetype[1] == BASE_PCREL)
				ortype = REL_PC;
			add_extnreloc(&db->relocs, op->base[1], op->extval[1],
				      ortype, 0, orsize,
				      (rtype == REL_NONE) ? roffs :
								  roffs + rsize / 8);
		}
	}
	return d;
	...
	do {
		if (ip->code >= 0) {
			...
			char ext =
				ip->qualifiers[0] ?
					      tolower((unsigned char)
							ip->qualifiers[0][0]) :
					      '\0';
			uint16_t sz = ((mnemo->ext.size & SIZE_MASK) ==
				       SIZE_UNSIZED) ?
						    SIZE_UNSIZED :
						    lc_ext_to_size(ext);
			...
			if (mnemo->ext.available & malias)
				cpu_error(
					33); /* deprecated instruction alias */
			...
			*d++ = mnemo->ext.opcode[0] & 0xff;
			...
			if ((cpu_type & mcf) && ip->code == OC_MOVE) {
				...
				if (src->mode >= MODE_An16Disp) {
					if ((src->mode == MODE_An16Disp ||
					     (src->mode == MODE_Extended &&
					      src->reg == REG_PC16Disp))) {
						if (dst->mode ==
							    MODE_An8Format ||
						    (dst->mode ==
							     MODE_Extended &&
						     dst->reg <= REG_AbsLong))
							cpu_error(
								41); /* illegal combination of CF addressing modes */
					} else {
						if (dst->mode ==
						    MODE_An16Disp) {
							if (!(cpu_type &
								...
								cpu_error(
									41); /* illegal combination of CF addr. modes */
						} else if (dst->mode ==
								   MODE_An8Format ||
							   (dst->mode ==
								    MODE_Extended &&
			...
			for (i = 0; i < MAX_OPERANDS && ip->op[i] != NULL;
			     i++) {
				operand* op = ip->op[i];
				struct oper_insert* oii =
					&insert_info[mnemo->ext.place[i]];
				...
				case M_bfea:
					if (op->flags & FL_BFoffsetDyn) {
						...
						cpu_error(
							19); /* illegal bitfield width/offset */
					}
				...
				case M_noea:
					newd = write_ea_ext(db, d, op, ext, sec,
							    pc);
					/* fall through */
...
/* Create a dblock (with relocs, if necessary) for size bits of data. */
{
	...
	default:
		cpu_error(
			38,
			bitsize); /* data objects with x size are not supported */
		break;
	}
...
int init_cpu()
{
	...
	for (i = 0, j = 0; i < mnemonic_cnt && j < code_tab_cnt; i++)
		if (!strcmp(mnemonics[i].name, code_tab[j].name))
			if ((code_tab[j].optype[0] == 0 ||
			     mnemonics[i].operand_type[0] ==
				     (int)code_tab[j].optype[0]) &&
			    (code_tab[j].optype[1] == 0 ||
			     mnemonics[i].operand_type[1] ==
				     (int)code_tab[j].optype[1]))
				*code_tab[j++].var = i;
	if (j < code_tab_cnt)
		ierror(0);
	...
	movembytes->expr =
		make_expr(MUL, new_sym_expr(movemsize),
...
static void set_cpu_type(uint32_t type, int addatom)
{
	if (type & (m68k | cpu32 | mcf_all | apollo)) {
		cpu_type =
			(cpu_type & ~(m68k | cpu32 | mcf_all | apollo)) | type;
		if (gas) {
			if (!(type & (m68020 | m68030 | cpu32)))
				cpu_type &= ~(
					m68881 |
					m68882); /* no 88x FPU when not 020 or 030 */
			if (!no_fpu && (type & (m68020 | m68030 | cpu32)))
				cpu_type |=
					m68881 |
					m68882; /* gas compatibility: always have FPU */
		}
	} else if (type & (m68881 | m68882))
		cpu_type =
			(cpu_type & ~(m68881 | m68882)) | (no_fpu ? 0 : type);
	else if (type == m68851)
		cpu_type |= m68851;
...
static uint32_t get_cpu_type(char** str)
{
	...
	for (i = 0; i < model_cnt; i++) {
		if (strlen(models[i].name) == len &&
		    !strnicmp(*str, models[i].name, len)) {
			type = models[i].type;
			break;
		}
	}
...
{
	opt_movem = opt_pea = opt_clr = opt_st = opt_lsl = opt_mul = opt_div =
		0;
...
int cpu_args(char* arg)
{
...
#ifdef OUTTOS
		tos_hisoft_dri =
			0; /* no extended symbol names unless OPT X+ is given */
#endif
		clear_all_opts();
		no_symbols = 1;
		warn_opts = 2;
	...
	nofpu:
		...
		commentchar = '|';
		set_cpu_type(m68020,
			     0); /* gas compatibility defaults to 68020/68881 */
		opt_jbra = !no_opt;
...
/* set optimizations according to PhxAss OPTC flags */
{
	...
	if (optc & 0x001) { /* N */
		opt_gen = opt_disp = opt_abs = opt_moveq = opt_bdisp =
			opt_odisp = 1;
	...
	if (optc & 0x020) /* P */
		opt_pea = 1;
	if (optc & 0x040) /* S */
		opt_clr = opt_st = opt_fconst = opt_disp = opt_bdisp =
			opt_odisp = 1;
	if (optc & 0x080) /* M */
		opt_gen = 1;
	if (optc & 0x100) /* T */
...
/* parse a devpac-style option (default) */
{
	static int opt_map[] = { /* maps OPT O<n> to a vasm option */
				 OCMD_OPTBRA,	 OCMD_OPTDISP,	 OCMD_OPTABS,
				 OCMD_OPTMOVEQ,	 OCMD_OPTQUICK,	 OCMD_NOP,
				 OCMD_OPTBRANOP, OCMD_OPTBDISP,	 OCMD_OPTODISP,
				 OCMD_OPTLEA,	 OCMD_OPTLQUICK, OCMD_OPTIMMADDR
	};
	char opt, ext, c;
	int flag = 1;
	int num;
...
#ifdef OUTHUNK
		hunk_onlyglobal =
			flag; /* only xdef-symbols in objects for Amiga */
#endif
		return s + 6;
	} else if (!flag)
		goto devpac_err;
	...
	if ((opt = tolower((unsigned char)validchar(s))) != 0) {
		...
		if (c == 0 || c == ',') {
			...
			case 'l': /* Ln : Atari only */
				...
				break;
			default:
				cpu_error(31, toupper((unsigned char)opt),
					  c); /* unkn. opt ignored */
				break;
			}
			return s;
			...
			case 'm':
				...
				break;
			case 'o':
				if (isdigit((unsigned char)ext) && num >= 1 &&
				    num <= 12) {
					...
					case 'b': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTJBRA,
							    flag);
						break;
					case 'c': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTCLR,
							    flag);
						break;
					case 'd': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTDIV,
							    flag);
						break;
					case 'f': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTFCONST,
							    flag);
						break;
					case 'g': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTGEN,
							    flag);
						break;
					case 'j': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTBRAJMP,
							    flag);
						break;
					case 'l': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTLSL,
							    flag);
						break;
					case 'm': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTMOVEM,
							    flag);
						break;
					case 'n': /* vasm-specific */
						add_cpu_opt(0, OCMD_SMALLDATA,
							    flag);
						break;
					case 'p': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTPEA,
							    flag);
						break;
					case 'q': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTNMOVQ,
							    flag);
						break;
					case 's': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTSPEED,
							    flag);
						break;
					case 't': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTST,
							    flag);
						break;
					case 'w':
						add_cpu_opt(0, OCMD_OPTWARN,
							    flag ? 2 : 0);
						break;
					case 'x': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTMUL,
							    flag);
						break;
					case 'z': /* vasm-specific */
						add_cpu_opt(0, OCMD_OPTSIZE,
							    flag);
						break;
					default:
						cpu_error(
							31, opt,
							ext); /* unknown option ignored */
						break;
					}
				}
				break;
			...
			case 'w':
				no_warn =
					!flag; /* must also be recognized during parsing */
				add_cpu_opt(0, OCMD_NOWARN, !flag);
				break;
			case 'x':
				if (flag)
...
#ifdef OUTTOS
				tos_hisoft_dri =
					flag; /* extended symbol names for Atari */
#endif
#ifdef OUTHUNK
				hunk_onlyglobal =
					flag; /* only xdef-symbols in objects for Amiga */
   ...
   cpu-specific text */
{
	...
	if (ISIDSTART(*s)) {
		...
		if ((s - name == 4 && !strnicmp(name, "near", 4)) ||
			...
			if (r >= 0) {
				if (r >= (REGAn + 0) &&
					...
					cpu_error(
						58); /* not a valid small data register */
			...
			if (sdreg >= 0) {
				...
				db->data[0] =
					0x41 |
					(sdreg << 1); /* LEA _LinkerDB,An */
				db->data[1] = 0xf9;
				memset(&db->data[2], 0, 4);
				add_extnreloc(&db->relocs,
					      new_import("_LinkerDB"), 0,
					      REL_ABS, 0, 32, 2);
				add_atom(0, new_data_atom(db, 2));
			} else
				cpu_error(
					59); /* small data mode is not enabled */
			return skip_line(s);
		}
			...
			if ((exp = parse_expr(&s)) != NULL) {
				...
				if (*s == ',') {
					...
					if (reg >= (REGAn + 0) &&
					    reg <= (REGAn + 6)) {
						if (baseexp[REGget(reg)] !=
							    NULL &&
						    !phxass_compat) {
							cpu_error(
								55,
								REGget(reg)); /* basereg already in use */
						} else {
							baseexp[REGget(reg)] =
								exp;
							eol(s);
			...
			if (reg >= (REGAn + 0) && reg <= (REGAn + 6)) {
				if (baseexp[REGget(reg)] != NULL) {
					...
					cpu_error(
						56,
						REGget(reg)); /* basereg is already free */
				...
				for (; reg <= 6; reg++) {
					if (baseexp[reg] !=
					    NULL) /* more than one register in use? */
						cpu_error(
							4); /* address register required */
				}
			} else
		...
		else if (phxass_compat && s - name == 4 &&
			 !strnicmp(name, "optc", 4)) {
		...
		if ((s - dir == 4 && !strnicmp(dir, "equr", 4)) ||
			...
			if ((r = getreg(&s, 0)) >= 0)
				new_regsym(regsymredef, 0, labname,
					   REGisAn(r) ? RSTYPE_An : RSTYPE_Dn,
					   0, REGget(r));
			else if ((r = getfreg(&s)) >= 0)
				new_regsym(regsymredef, 0, labname, RSTYPE_FPn,
					   0, r);
			else if ((r = getbreg(&s)) >= 0)
				new_regsym(regsymredef, 0, labname, RSTYPE_Bn,
					   0, r);
			else
			...
			*start = skip_line(s);
			...
			s = skip(s);
			sym = new_equate(labname,
					 number_expr((taddr)scan_Rnlist(&s)));
			sym->flags |= REGLIST;
			eol(s);
			*start = skip_line(s);
			return 1;
			...
			s = skip(s);
			sym = new_equate(labname,
					 number_expr((taddr)scan_FPnlist(&s)));
			sym->flags |= REGLIST;
			eol(s);
			*start = skip_line(s);
			return 1;
