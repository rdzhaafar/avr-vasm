...
char* cpu_copyright =
	"vasm qnice cpu backend 0.1 (c) in 2016 Volker Barthelmann";
char* cpuname = "qnice";
...
dblock* eval_instruction(instruction* p, section* sec, taddr pc)
{
	...
	if (p->op[0]) {
		...
		if (p->op[0]->type == OP_ABS) {
			...
			addr1 = absoffset(p->op[0]->offset, sec, pc, &relocs,
					  p->op[0]->reg, 16, 16);
			aflag = 1;
	...
	if (mnemonics[c].ext.encoding == 0) {
		if (p->op[1]) {
			if (p->op[1]->type == OP_ABS) {
				code |= 15 << 2;
				code |= (OP_POSTINC - 1) << 0;
				addr2 = absoffset(p->op[1]->offset, sec, pc,
						  &relocs, p->op[1]->reg, 16,
						  16);
				aflag |= 2;
