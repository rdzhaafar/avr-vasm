...
char* cpu_copyright =
	"VideoCore IV cpu backend 0.1 (c) in 2013 Volker Barthelmann";
...
static char* vflags[] = { "all", "none", "ifz", "ifnz",
			  "ifn", "ifnn", "ifc", "ifnc" };
...
static char* sru[] = { "sumu", "sums", "???", "imin",
		       "???",  "imax", "???", "max" };
...
int parse_operand(char* p, int len, operand* op, int req)
{
	...
	if (req >= OP_REG && req <= OP_LR) {
		...
		op->type = OP_REGIND;
		p = skip(p + 1);
	} else if (req == OP_IMMIND || req == OP_IMMINDSP ||
		   req == OP_IMMINDSD || req == OP_IMMINDR0 ||
		   req == OP_IMMINDS || req == OP_IMMINDPC) {
		op->offset = 0;
		if (*p == '(') {
			char* p2;
			p2 = skip(p + 1);
		...
		if (req == OP_IMMINDSP && r != 25)
			return 0;
		if (req == OP_IMMINDPC && r != 31)
			return 0;
		if (req == OP_IMMINDSD && r != 24)
			return 0;
		if (req == OP_IMMINDR0 && r != 0)
			return 0;
		if (req == OP_IMMINDS && r > 15)
			return 0;
		op->reg = r;
		op->type = OP_IMMIND;
		p = skip(p + 1);
	...
	if (req == OP_VREGMM || req == OP_IMM32M || req == OP_VIND) {
		...
		while (1) {
			...
			for (i = 0; i < sizeof(vflags) / sizeof(vflags[0]);
			     i++) {
				if ((!strncasecmp(p, vflags[i],
						  strlen(vflags[i]))) &&
...
static taddr dooffset(int rel, expr* tree, section* sec, taddr pc,
		      rlist** relocs, int roffset, int size, taddr mask)
{
	taddr val;
	if (!eval_expr(tree, &val, sec, pc)) {
		...
		if (rel != REL_PC || !LOCREF(base) || base->sec != sec) {
			add_nreloc_masked(relocs, base, addend, rel, size,
					  roffset, mask);
			return 0;
		}
	}
...
static taddr reloffset(expr* tree, section* sec, taddr pc, rlist** relocs,
		       int roffset, int size, taddr mask)
{
	/* taddr mask=size==32?0xffffffff:((((taddr)1)<<size)-1);*/
	return dooffset(REL_PC, tree, sec, pc, relocs, roffset, size, mask);
}

static taddr absoffset(expr* tree, section* sec, taddr pc, rlist** relocs,
		       int roffset, int size)
{
	/* taddr mask=size==32?0xffffffff:((((taddr)1)<<size)-1);*/
	return dooffset(REL_ABS, tree, sec, pc, relocs, roffset, size,
			0xffffffff);
}
...
static int translate(instruction* p, section* sec, taddr pc)
{
	...
	if (e == EN_ARITHI32) {
		if (p->op[2]) {
			if (!chkval(p->op[2]->offset, sec, pc, 6, 1) &&
			    (!strcmp("add", mnemonics[c].name) ||
			     !strcmp("sub", mnemonics[c].name)))
				c = replace(c, EN_ADD48);
		} else {
			if ((mnemonics[c].ext.code < 32) &&
			    (!chkval(p->op[1]->offset, sec, pc, 6, 1)))
	...
	if (e == EN_RBRANCH16) {
		symbol* base;
		if (find_base(p->op[0]->offset, &base, sec, pc) != BASE_OK ||
		    !LOCREF(base) || base->sec != sec)
	...
	if ((e == EN_MEMDISP16 || e == EN_MEMDISP32 || e == EN_MEM12DISP32 ||
	     e == EN_MEM16DISP32)) {
...
dblock* eval_instruction(instruction* p, section* sec, taddr pc)
{
	...
	case EN_ARITHI16:
		...
		code |= (p->op[0]->reg << 16) |
			(absval(p->op[1]->offset, sec, pc, 5, 0) << 20);
		break;
	case EN_FIX16:
		if (cc != 14)
			cpu_error(3);
	...
	case EN_RBRANCH16:
		code = 0x18000000;
		code |= cc << 23;
		code |= ((reloffset(p->op[0]->offset, sec, pc, &relocs, 0, 7,
				    0xfe) >>
			  1) &
			 127)
			<< 16;
	...
	case EN_RBRANCH32:
		if (code == 0x90000000)
			...
			code |= ((reloffset(p->op[0]->offset, sec, pc, &relocs,
					    8, 4, 0xF000000) >>
				  24) &
				 0xf)
				<< 24;
		code |= ((reloffset(p->op[0]->offset, sec, pc, &relocs, 16, 16,
				    0x1fffe) >>
			  1) &
			 0x7fffff);
		code |= ((reloffset(p->op[0]->offset, sec, pc, &relocs, 0, 7,
	...
	case EN_ADDCMPB32:
		...
		if (code & 0x8000) {
			...
			if (m)
				...
				code |= ((reloffset(p->op[3]->offset, sec, pc,
						    &relocs, 16, 9, 0x1ff) >>
					  1) &
					 0xff);
		} else {
		...
		if (m) {
			...
			code2 |= ((reloffset(p->op[3]->offset, sec, pc + 4,
					     &relocs, 48, 16, 0x1fffe) >>
				   1) &
				  0x7fffff);
			code2 |= ((reloffset(p->op[3]->offset, sec, pc + 4,
					     &relocs, 32, 7, 0xfe0000) >>
				   17) &
				  0x7f)
				 << 16;
		}
	...
	case EN_ADD48:
		if (cc != 14)
			cpu_error(3);
		if (p->op[2])
			code2 = absoffset(p->op[2]->offset, sec, pc, &relocs,
					  16, 32);
		else
			code2 = absoffset(p->op[1]->offset, sec, pc, &relocs,
					  16, 32);
		/* pseudo sub */
		if (code == 6)
			code2 = -code2;
	...
	case EN_LEA48:
		...
		code2 = reloffset(p->op[1]->offset, sec, pc, &relocs, 16, 32,
				  0xffffffff);
		break;
	case EN_MEM48:
		if (cc != 14)
		...
		code |= p->op[0]->reg << 16;
		if (p->op[1]->reg == 31)
			code2 = reloffset(p->op[1]->offset, sec, pc, &relocs,
					  16, 27, 0xffffffff) &
				0x7ffffff;
		else
			/* TODO: other relocs */
	...
	case EN_MEM12DISP32:
		...
		code |= (absval(p->op[1]->offset, sec, pc, 12, 1) & 0x800)
			<< 13;
		break;
	case EN_MEM16DISP32:
		if (cc != 14)
		...
		if ((code & 0xff000000) == 0xaa000000)
			code |= reloffset(p->op[1]->offset, sec, pc, &relocs,
					  16, 16, 0xffff) &
				0xffff;
		else
	...
	case EN_VREADI48:
		code = 0xF0000000 | (code << 19);
		code2 = (p->op[0]->reg << 22) |
			absval(p->op[1]->offset, sec, pc, 6, 0) | (1 << 10);
		break;
	case EN_VWRITEI48:
		code = 0xF0000000 | (code << 19);
		code2 = (p->op[0]->reg << 12) |
	...
	case EN_VARITHR48:
		...
		if (p->op[2]) {
			code2 = (p->op[0]->reg << 22) | (p->op[1]->reg << 12) |
				p->op[2]->reg;
		} else {
			code2 = (p->op[0]->reg << 22) | p->op[1]->reg;
		}
	...
	case EN_VARITHI48:
		...
		if (p->op[2]) {
			code2 = (p->op[0]->reg << 22) | (p->op[1]->reg << 12) |
				absval(p->op[2]->offset, sec, pc, 6, 0) |
				(1 << 10);
		} else {
			code2 = (p->op[0]->reg << 22) |
				absval(p->op[1]->offset, sec, pc, 6, 0) |
				(1 << 10);
		}
		break;
	/* TODO: Check these! */
	case EN_VLOAD80:
		...
		code2 = (p->op[0]->reg << 22);
		code3 = (p->op[1]->reg << 2) | (p->op[0]->dreg << 26) |
			((p->op[1]->dreg) << 22);
		if (p->op[1]->offset) {
			code2 |= (absval(p->op[1]->offset, sec, pc, 16, 1) &
				  0x7f);
			code3 |= (absval(p->op[1]->offset, sec, pc, 16, 1) >>
				  7) &
				 3;
			code3 |= ((absval(p->op[1]->offset, sec, pc, 16, 1) >>
				   9) &
	...
	case EN_VSTORE80:
		code = 0xF8000000 | (code << 19) | (p->op[1]->vecmod & 0x70000);
		code2 = (p->op[0]->reg << 12);
		code3 = (p->op[1]->reg << 2) | (p->op[0]->dreg << 20) |
			((p->op[1]->dreg) << 28);
		if (p->op[1]->offset) {
			code2 |= (absval(p->op[1]->offset, sec, pc, 16, 1) &
				  0x7f);
	...
	case EN_VREAD80:
		if (p->op[1]->offset) {
			p->op[1]->reg |=
				(absval(p->op[1]->offset, sec, pc, 9, 1) &
				 0x7f);
			p->op[1]->dreg |=
				(absval(p->op[1]->offset, sec, pc, 9, 1) >> 7) &
				3;
		}
		code = 0xF8000000 | (code << 19) | (p->op[1]->vecmod & 0x70000);
		code2 = (p->op[0]->reg << 22) | (p->op[1]->reg);
		if (p->op[1]->vecmod & (1 << 20))
			code2 |= (1 << 11);
	...
	case EN_VWRITE80:
		...
		if (p->op[1]->vecmod & (1 << 20))
			code2 |= (1 << 11);
		code3 = ((p->op[0]->dreg & 0x3f) << 20) |
			((p->op[0]->dreg & 0x3c0) << 10) | (p->op[1]->dreg);
		code3 |= (p->op[1]->vecmod & 0x3fff) << 6;
		break;
	case EN_VREADI80:
		if (p->op[1]->offset) {
		...
		if (p->op[1]->vecmod & (1 << 20))
			code2 |= (1 << 11);
		code2 = (p->op[0]->reg << 22) | (1 << 10) |
			(absval(p->op[1]->offset, sec, pc, 16, 0) & 0x3ff);
		code3 = (p->op[0]->dreg << 26) |
			((absval(p->op[1]->offset, sec, pc, 16, 0) >> 10) &
			 0x3f);
		code3 |= (p->op[1]->vecmod & 0x3fff) << 6;
		break;
	case EN_VWRITEI80:
		...
		if (p->op[1]->vecmod & (1 << 20))
			code2 |= (1 << 11);
		code2 = (p->op[0]->reg << 12) | (1 << 10) |
			(absval(p->op[1]->offset, sec, pc, 16, 0) & 0x3ff);
		code3 = ((p->op[0]->dreg & 0x3f) << 20) |
			((p->op[0]->dreg & 0x3c0) << 10) |
			((absval(p->op[1]->offset, sec, pc, 16, 0) >> 10) &
			 0x3f);
		code3 |= (p->op[1]->vecmod & 0x3fff) << 6;
		break;
	case EN_VARITHR80:
		...
		if (p->op[2]) {
			if (p->op[2]->offset) {
				p->op[2]->reg |= (absval(p->op[2]->offset, sec,
							 pc, 9, 1) &
						  0x7f);
				p->op[2]->dreg |= (absval(p->op[2]->offset, sec,
							  pc, 9, 1) >>
						   7) &
						  3;
			}
			code = 0xFC000000 | (code << 19) |
			       (p->op[2]->vecmod & 0x70000);
			code2 = (p->op[0]->reg << 22) | (p->op[1]->reg << 12) |
				(p->op[2]->reg);
			if (p->op[2]->vecmod & (1 << 20))
				code2 |= (1 << 11);
			code3 = (p->op[0]->dreg << 26) |
				((p->op[1]->dreg & 0x3f) << 20) |
				((p->op[1]->dreg & 0x3c0) << 10) |
				(p->op[2]->dreg);
			code3 |= (p->op[2]->vecmod & 0x3fff) << 6;
		} else {
			if (p->op[1]->offset) {
				p->op[1]->reg |= (absval(p->op[1]->offset, sec,
							 pc, 9, 1) &
						  0x7f);
				p->op[1]->dreg |= (absval(p->op[1]->offset, sec,
							  pc, 9, 1) >>
						   7) &
						  3;
			}
			code = 0xFC000000 | (code << 19) |
			       (p->op[1]->vecmod & 0x70000);
			code2 = (p->op[0]->reg << 22) | (p->op[1]->reg);
			if (p->op[1]->vecmod & (1 << 20))
				code2 |= (1 << 11);
			code3 = (p->op[0]->dreg << 26) | (p->op[1]->dreg);
	...
	case EN_VARITHI80:
		...
		if (p->op[2]) {
			...
			code2 = (p->op[0]->reg << 22) | (p->op[1]->reg << 12) |
				(1 << 10) |
				(absval(p->op[2]->offset, sec, pc, 16, 0) &
				 0x3ff);
			code3 = (p->op[0]->dreg << 26) |
				((p->op[1]->dreg & 0x3f) << 20) |
				((p->op[1]->dreg & 0x3c0) << 10) |
			...
			if (p->op[1]->vecmod & (1 << 20))
				code2 |= (1 << 11);
			code2 = (p->op[0]->reg << 22) | (1 << 10) |
				(absval(p->op[1]->offset, sec, pc, 16, 0) &
				 0x3ff);
			code3 = (p->op[0]->dreg << 26) |
				((absval(p->op[1]->offset, sec, pc, 16, 0) >>
				  10) &
