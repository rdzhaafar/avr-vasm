...
static int opt_inst(instruction* p, section* sec, taddr pc)
{
	...
	if (p->code == 6) {
		...
		if (tree->type == SYM && tree->c.sym->sec == sec &&
		    LOCREF(tree->c.sym) && tree->c.sym->pc - pc >= -128 &&
		    tree->c.sym->pc - pc <= 127)
			return 7;
	}
	return p->code;
}
...
static unsigned char* fill_operand(operand* p, section* sec, taddr pc,
				   unsigned char* d, rlist** relocs,
				   int roffset)
{
	taddr val;
	if (!p || p->type == OP_REG)
