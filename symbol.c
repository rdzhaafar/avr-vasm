...
{
	static const char* typename[] = { "???", "obj", "func", "sect",
					  "file" };
	int t;
...
symbol* new_abs(char* name, expr* tree)
{
	...
	if (new) {
		if (new->flags & EQUATE)
			general_error(
				67,
				name); /* repeatedly defined symbol (error) */
...
symbol* new_labsym(section* sec, char* name)
{
	...
	if (sec->flags & LABELS_ARE_LOCAL)
		name = make_local_label(sec->name, strlen(sec->name), name,
					strlen(name));
...
symbol* internal_abs(char* name)
{
	...
	if (new) {
		if (new->type != EXPRESSION ||
		    (new->flags&(EXPORT | COMMON | WEAK)))
			general_error(
				37,
				name); /* internal symbol redefined by user */
	} else {
		   ...
		   unsigned int flags, unsigned int num)
{
	...
	rsym = no_case != 0 ? find_regsym_nc(name, len) :
				    find_regsym(name, len);
	if (rsym != NULL && !redef) {
...
{
	regsym* rsym = no_case != 0 ? find_regsym_nc(name, strlen(name)) :
	...
	if (rsym != NULL) {
		if (rsym->reg_type == type) {
			...
			general_error(
				70, name); /* register symbol has wrong type */
	} else
